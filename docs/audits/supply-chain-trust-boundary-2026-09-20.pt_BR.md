...
| `project.build.outputTimestamp` tornaria o jar reprodutível em bytes? | **sim** — 3 builds limpos (mesmo caminho repetido, e outro caminho) deram o **mesmo sha256** (`30e2bca1…`) | uma propriedade padrão do Maven torna o jar do `kof-cli` byte-reprodutível; aplicada no `pom.xml` raiz e codificada em `scripts/check-reproducible-build.sh` (RED antes: digests diferentes; GREEN depois). Os arquivos de distribuição (`package.sh` tar.gz/zip com o JDK embutido) **não** foram medidos e são a lacuna de reprodutibilidade que resta |
