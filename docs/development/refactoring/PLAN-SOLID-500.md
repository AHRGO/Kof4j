...
| 3 | NativeBackend 6813 | 🟡 EM CURSO | **548 (13/09, split parcial)**: saíram `NativeSymbolMangle` 90 (mangle por assinatura SG-011B/§131) + `NativeStaticData` 116 (.data de campos estáticos bug 41) — o CRÍTICO ≥600 virou dívida tolerada (548 < 577 do pior resíduo); a fatia GC/tree-shaking em `nat/` (F3 completa ≤500) segue bloqueada pela lane GC; **baseline atualizado** (`--update-baseline`, 13 dívidas — NativeBackend saiu) |
