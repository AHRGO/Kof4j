...
| widening no VALOR do `Map.put` pinado (`mapOf(_,Long).put(_,Int)`) | `2` / `1` | DONE (bug 143 ✅ 12/09 — era VerifyError/CCE no get; coerção + paramTypes ajustado ao tipo pinado) | PARTIAL (bug 142 **ABERTO** — put roda mas o `get` da chave NOVA em mapa Long-value SIGSEGV **pré-existente ao widening** (medido HEAD Y2d/Y2e com git stash; put puro-Long também crasha); widening não é a causa) | DONE (bug 143 ✅ 12/09) | DONE (bug 143 ✅ 12/09) | `mapwiden` |
