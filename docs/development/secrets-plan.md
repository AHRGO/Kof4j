...
request). Implementation requires a rule-6 vote per face before anything touches the language or
the current `secrets.*` signatures (frozen semantics 0.2.6-beta).
