...
| 8.3 | LSP/debug/profiler por domínio | 🟡 | lane tooling/docs (.15) p/ LSP | **LSP domain-aware ✅ 19/09**: completion + hover leem o `StdCatalog` (namespace lista membros; membro no contexto `ns.` nomeia a face; solto = null honesto). **rename cross-file ✅ 19/09 (LSP-A, `LspRename`)** + fix de higiene: o scan de irmãos nunca trata /tmp ou a raiz do FS como projeto (profundidade 1 lá; 485 `.kf` alheios medidos em /tmp). **8.3-B FEITO 19/09 (`.18`)**: `workspace/symbol`/hover/definition/references cobrem deps fora do pai do arquivo via `rootUri` do initialize (`LspProject.siblings(self,root)`, dedup+ordenado; sem rootUri = comportamento antigo exato). **Pendente:** debugger/profiler por domínio (= X7 + faces futuras; profiler ainda não existe); signatureHelp por domínio — **assinaturas no hover ✅ 19/09 (LSP-A fatia 1)**: `StdCatalog.signaturesOf` (tabela no `StdCatalog`, fonte única) alimenta o hover de `db`/`http` com um overload por linha, travada COMPORTAMENTO-A-DISPATCHER (`StdCatalogSignaturesTest`: a aridade gravada binda no `staticCall` real, a proibida não). No caminho, **bug real do catálogo X10 achado pela fatia**: o `KofDb.functions()` só tinha os `case` (connect/close/transaction) — `query`/`execute` viviam nas famílias `isQuery/isExecute` e sumiam do completion/hover do namespace mais usado do plano; lista 3→5 + o lock passou a somar os literais das famílias (fonte-a-fonte). **Restante:** estender a tabela aos outros 22 namespaces (fatia a fatia, sem fingir cobertura — membro sem tabela mantém hover simples, R6) e o REQUEST `textDocument/signatureHelp` (handler novo; a fonte de dados já existe) | |
