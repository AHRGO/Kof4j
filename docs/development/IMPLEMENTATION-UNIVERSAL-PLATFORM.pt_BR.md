...
| 1.5.3 | Registry MVP | ✅ 19/09 | lane docs→plataforma | D2-A: **publish ✅** (era `b1ea1718`) + **pull ✅ S2 19/09** — `DepsRegistry`: `owner/repo[@ver]` → asset `<repo>-<ver>.tar.gz` da API GitHub Releases, `SHA256SUMS` verificado ANTES de instalar, cache `~/.kof/deps/kof/`, `latest` pinna a versão no `kofdeps`; REG001–004 honestos (R6); `DepsRegistryTest` 13/13 (shape REAL do GitHub + contrato HTTP; #564) + vizinhos `Deps*` 14/14; build/run consomem via `Deps.classpath()`; **round-trip live no GitHub ✅ 20/09** (release pública real, sem token, HOME limpo: pull por versão + `latest` + 2º resolve idempotente + REG001; era VERMELHO por #564/#565, corrigidas — evidência `docs/audits/registry-live-roundtrip-2026-09-20.pt_BR.md`); **consumo por código KOF ✅ medido 20/09 no caminho documentado** (`kof run/build --deps` + `import` + `new Classe()`, JVM; sem `--deps` dá PKG006 por desenho); **aberto (contrato, decisão da mantenedora — #566):** superfície pública de biblioteca (o `deploy` só empacota o alcançável de `main`), KOF→KOF multi-alvo e `Classe()` sem `new` em classe de classpath externo (SEM015; #568) |
