...
| 3.2 | `infra "prod" { ... }` — desugar sobre records (codegen em compile-time) | 🟡 | `.18`/9093 | **DECIDIDO 21/09 (`D-MAKEALIVE-SYNTAX`)** — puro açúcar sintático sobre `design()`: `infra` segue IDENTIFICADOR (despachado como `test`/`application`, não palavra reservada), o bloco rebaixa para `design(): Infrastructure` e não ganha semântica própria (sem HCL). **EM IMPLEMENTAÇÃO** (dono `.18`/9093) — prova `InfraSyntaxE2ETest` (gêmeo `design()` escrito à mão → plan/apply byte-idênticos, JVM==JS). R4 ✅ (hook `CodegenStep`) removeu o bloqueio de codegen. |
...
| 3.7 | Detecção de ciclo no grafo `infra` em compile-time | 🔵 | `.18`/9093 | **R4 ✅ pousou 21/09**; **DESBLOQUEADO por `D-MAKEALIVE-SYNTAX` (21/09)** — o grafo em compile-time segue a superfície 3.2 (dono `.18`/9093); até lá, a recusa em runtime embarcada no 3.1 é o contrato. |
...
De resto, o próximo trabalho da R3 é **implementar o 3.3/3.5 decididos** (`D-R3-3.3`/`D-R3-3.5`, 21/09) mais a 3.8 em curso (D6 decidido 20/09; 3.8b fatias 1–4 + D6-5 landed 20–21/09; restam bridge JS/sret Native); a ABI escalar Native (3.7) landou 20/09 e o §370/#549 (conversão de argumento de extern) fechou no mesmo dia.
