...
| 1.1.9 | `ORM001` — `kof.orm` no Native | 🟡 | `.18` (D-DB-GAPS DB-1 `DECIDIDO` 20/09) | JVM + JS fechados (JS 18/09, `KofJsOrmBridge`); rota no Native = `kof_orm_*` em asm **sobre a superfície `kof_db_*` nativa existente** (x86 primeiro, depois cross; sem atalho de embutir JVM, sem fallback silencioso). **F1a `delete_all` ✅ 20/09** (gate por-função `fnSupportedOn`; prova byte `KofOrmE2ETest` 35/0F + suíte 3183/0F); faces restantes + mysql + cross ainda `ORM001` |
