...
|---------|------|--------|----------|
| 1 | SYSTEMS (consolidação) | 🟡 em curso | sign-off GC x86 ✅ (D1-A 19/09) · NATIVE002 ✅ (19/09) — gaps honestos residuais de web/mídia/ORM (1.1) |
...
| 2.5 | Pipelines de CI/CD como **código Kof** | ⏳ | — | 2.1 (MVP aterrissou 19/09 — `dag`/`job` rodam em JVM+JS; pré-requisito pleno pede 2.1.3 retry/checkpoint) |
| 2.6 | Tooling: `kof workflow run` | ⏳ | — | 2.1 (MVP aterrissou 19/09 — `dag`/`job` rodam em JVM+JS; pré-requisito pleno pede 2.1.3 retry/checkpoint) |
...
| 8.2 | Package manager maduro | 🔵 | — | 1.5.3 ✅ (publish+pull 19/09) |
| 8.3 | LSP/debug/profiler por domínio | 🟡 | lane tooling/docs (.15) p/ LSP | **LSP domain-aware ✅ 19/09**: completion + hover leem o `StdCatalog` (namespace lista membros; membro no contexto `ns.` nomeia a face; solto = null honesto). **rename cross-file ✅ 19/09 (LSP-A, `LspRename`)** + fix de higiene: o scan de irmãos nunca trata /tmp ou a raiz do FS como projeto (profundidade 1 lá; 485 `.kf` alheios medidos em /tmp). **8.3-B FEITO 19/09 (`.18`)**: `workspace/symbol`/hover/definition/references cobrem deps fora do pai do arquivo via `rootUri` do initialize (`LspProject.siblings(self,root)`, dedup+ordenado; sem rootUri = comportamento antigo exato). **Pendente:** debugger/profiler por domínio (= X7 + faces futuras; profiler ainda não existe); signatureHelp por domínio — **assinaturas no hover ✅ 19/09 (LSP-A fatias 1–4: 14 ns, 130 membros/141 formas)**: `StdCatalog.signaturesOf` (tabela no `StdCatalog`, fonte única) alimenta o hover de `db`/`http` com um overload por linha, travada COMPORTAMENTO-A-DISPATCHER (`StdCatalogSignaturesTest`: a aridade gravada binda no `staticCall` real, a proibida não). No caminho, **DOIS bugs reais do catálogo X10 achados pela fatia**: o `KofDb.functions()` e `KofProcess.functions()` só tinham os `case`/primeiro roteamento (connect/close/transaction) — `query`/`execute` viviam nas famílias `isQuery/isExecute` e sumiam do completion/hover (query/execute 3→5; run/spawn/exit 1→3) + o lock passou a somar os literais das famílias (fonte-a-fonte). **Restante:** estender a tabela aos outros 18 namespaces (`json` sem tabela ate haver dispatcher travavel no typer); a fatia 3 trocou o lock de case-literals por um que segue vírgulas e **achou 4 drifts de catálogo ainda maiores** (net 1→8, encoding 2→8, math 19→24, strings 7→26 — 38 membros bindando no dispatcher, invisíveis no completion desde a fatia 1; todos corrigidos com trava comportamento-a-dispatcher) (fatia a fatia, sem fingir cobertura — membro sem tabela mantém hover simples, R6) e o REQUEST `textDocument/signatureHelp` (handler novo; a fonte de dados já existe) | |
