...
| X10 | LSP domain-aware (completion + ir-para-definição em pacotes) | 🟡 | lane docs→plataforma (192.168.100.15) | fatia 1 ✅ 18/09: `StdCatalog` (membros reais dos 7 namespaces do dispatch KofStd, fonte-única travada por teste contra os case-literals dos typers) + completion de membros no `kof lsp` (`rng.` → os 5 do typer; `StdCatalogTest` 3/3, `LspServerTest` 23/23; `8a2c26b7`); fatia 2 = namespaces de dispatch próprio (time/json/http/db/...); ir-para-definição em pacotes = fatia 3 |
