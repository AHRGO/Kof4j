...
| 3.6 | Segredos via `kof.security` | 🟡 | lane security | `kof.security` existe; `Secret`/`KeyHandle` pendentes (Estágio 5). **DECIDIDO 21/09 (`D-SECRETS`)**: plano promovido para `docs/development/secrets-plan.md` (EN+PT); **face 1 (tipo `Secret`) autorizada**, incremental com prova; `KeyHandle`/redação face a face. |
...
| # | Item | Status | Dono | Depende de |
|---|------|--------|------|------------|
| 5.1 | S2 — tipo `Secret` + `KeyHandle` (redação forçada) | 🟡 | lane security | R3; **DECIDIDO 21/09 (`D-SECRETS`)**: `secrets-plan.md` promovido (EN+PT), **face 1 (`Secret`) autorizada**, incremental com prova; `KeyHandle`/redação face a face |
...
| X5 | Tipos variance / sealed | 🟡 ABERTO (spec-first) | compilador/mantenedora | **decidido 21/09 (`D-TYPE-VARIANCE` = opção C): ABERTO**; spec/plano rascunhado antes de qualquer diff de parser/typer (regra 6); type-classes seguem rejeitadas. **APROVADO 21/09** (fatias incrementais, prova por fatia); **`D-X5-SURFACE` congelou a superfície v1** (`out`/`in`, `sealed` class/record + interface, projeção no sítio de uso na v1, `SEM0xx`) |
| X6 | Reflexão de interop (restrita ao interop) | 🟡 ABERTO (spec-first) | compilador/mantenedora | **decidido 21/09 (`D-INTEROP-REFLECT`): ABERTO, plano incremental exigido**; só na fronteira de interop, nunca fundação (regra 6). **APROVADO 21/09** (fatias incrementais, prova por fatia); spec X6.0 ✅ |
...
| R4 | Formalizar o codegen em compile-time (`CodegenStep`) | ✅ pousou 21/09 | `CodegenStep`/`CodegenStepPipeline` (aditivo; registro vazio = identidade, zero mudança de comportamento; `CodegenStepPipelineTest` 6/6); destrava `infra "prod" {}` (3.2, superfície ainda ⛔ regra 6); a **migração DDL/runner (2.2.3) avaliada 21/09 = descompasso de fase → DECIDIDO 21/09 opção B (`D-DESUGAR-STEP`): registry `DesugarStep` na fase de AST (livre de comportamento); o DDL permanece no lowering** |
