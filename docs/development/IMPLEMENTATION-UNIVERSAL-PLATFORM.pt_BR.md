...
| 3 | INFRASTRUCTURE (Kof Makealive) | ✅ **todas as linhas 3.1–3.8 pousadas** (3.6 secrets pousou 21/09 `32285136`) — `makealive-plan.md` movido para `docs/architecture/` | Estágio 2, R3 (FFI), R4 (hook de codegen — **✅ pousou 21/09**); **colisão de nome R1 ✅ resolvida (`kof.makealive`, plano §2.1/Q1)** |
...
| 3.6 | Segredos via `kof.security` | ✅ 21/09 | lane security | `D-SECRETS` face 1 POUSADA 21/09 (`32285136`): tipo valor `Secret` — `secrets.of`/`secrets.secret`, `reveal()` (único export cru), impressão redigida (`Secret(*** )`), `==` constant-time; JVM-primeiro (R7), JS/Native/Script/Android = `SECN008` (R6). Prova `SecretE2ETest` 4/4. `fromBytes`/buffer zerável + redação P2/`KeyHandle` (P3) = follow-ups com voto próprio (`secrets-plan.md`, EN+PT). |
...
| # | Item | Status | Dono | Depende de |
|---|------|--------|------|------------|
| 5.1 | S2 — tipo `Secret` + `KeyHandle` (redação forçada) | 🟡 | lane security | R3; **DECIDIDO 21/09 (`D-SECRETS`)**: `secrets-plan.md` promovido (EN+PT); **face 1 (`Secret`) POUSADA 21/09 (`32285136`)** — `secrets.of`/`secrets.secret` + `reveal()`/`redacted()` + `==` constant-time, JVM-primeiro, gap `SECN008` (R6); `fromBytes`/buffer zerável + P2 (redação) + P3 (`KeyHandle`) face a face com voto próprio |
