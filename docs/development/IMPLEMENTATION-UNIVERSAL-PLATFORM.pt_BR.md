...
| 3.2 | `infra "prod" { ... }` — desugar sobre records (codegen em compile-time) | 🔵 | — | **R4 ✅ pousou 21/09 (hook `CodegenStep`)**; a superfície de usuário `infra "prod"` segue ⛔ regra 6 (fora do v1, plano §5) |
...
| 3.6 | Segredos via `kof.security` | 🟡 | lane security | `kof.security` existe; `Secret`/`KeyHandle` pendentes (Estágio 5) → plano de design EN+PT pousado 21/09: `future/secrets-plan.md` (P1/P2/P3, cada um à espera de voto rule-6); zero código. |
...
| X5 | Tipos variance / sealed | 🟡 ABERTO (spec-first) | compilador/mantenedora | **decidido 21/09 (`D-TYPE-VARIANCE` = opção C): ABERTO**; spec/plano rascunhado antes de qualquer diff de parser/typer (regra 6); type-classes seguem rejeitadas |
| X6 | Reflexão de interop (restrita ao interop) | 🟡 ABERTO (spec-first) | compilador/mantenedora | **decidido 21/09 (`D-INTEROP-REFLECT`): ABERTO, plano incremental exigido**; só na fronteira de interop, nunca fundação (regra 6) |
...
| 3.3 | JVM: handles opacos / out-buffers (void*, T*, Array<Byte> como buffer) — ponteiro opaco / buffer de bytes, NÃO o ABI struct completo do D6 | 🟡 decidido 21/09 | mantenedora | **`D-R3-3.3` = opção A**: `Handle` opaco nominal (não-aritmético) + `Buffer(U8, INOUT)` (== D6-3); implementar |
...
| 3.8 | ABI struct/array completo (D6) | 🟡 | lane compilador (pós-spec) | **D6-A ✅ 19/09**: spec **rascunho escrita 19/09** (`ffi-abi-structs.md`+PT: ABIs medidas, 3 exemplos-resolução como golden, D6-1..D6-5 = decisões da mantenedora antes de 3.8, verruga `Arena.global` §1 catalogada). **3.8a ✅ 20/09 (fatia sem decisão)**: `AbiLayout.java` + `AbiLayoutTest` — engine puro de layout/classificação (size/align/offsets + classes por ABI), golden MEDIDO com GCC 13.3 nos 3 alvos e reprovado ao vivo por `_Static_assert` contra gcc/aarch64-linux-gnu-gcc/riscv64-linux-gnu-gcc; corrigiu a prosa riscv do rascunho (LP64D faz *flatten* de structs ≤2 campos — `Time(Long,Double)`→`a0`+`fa0`). Não binda nada e não decide D6-1..D6-5; 3.8b/3.7 consomem. Revisão da mantenedora das decisões D6, depois o binding |
...
escalares (array/struct/pointer) seguem `FFI002` em compilação (3.3/3.5 **decididos 21/09**; 3.8 em curso — D6 decidido, 3.8a/b landed).
