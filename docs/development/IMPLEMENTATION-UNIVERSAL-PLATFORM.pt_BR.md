...
| 3 | INFRASTRUCTURE (Kof Makealive) | 🟡 planejado 19/09 — `makealive-plan.md` (.18); 3.0.0 recon pendente, **superfície ✅ decidida (Q1–Q4, `DECISIONS.md` §D-MAKEALIVE 20/09)** | Estágio 2, R3 (FFI), R4 (hook de codegen — **✅ decidido 21/09, `D-CODEGEN-STEP`**); **colisão de nome R1 ✅ resolvida (`kof.makealive`, plano §2.1/Q1)** |
...
**R3 🟡 · R4 🟡 (decidido 21/09, `D-CODEGEN-STEP`) · R5 🟡 · R9 🟡 · R10 🔵 · R11 🟡**
...
| 3.2 | `infra "prod" { ... }` — desugar sobre records (codegen em compile-time) | 🔵 | — | **R4 ✅ decidido 21/09 (`D-CODEGEN-STEP`, hook interno)**; a superfície de usuário `infra "prod"` segue ⛔ regra 6 (fora do v1, plano §5) |
...
| R4 | Formalizar o codegen em compile-time (`CodegenStep`) | 🟡 decidido 21/09 | `D-CODEGEN-STEP` = implementar como hook INTERNO do compilador (sem sintaxe de usuário); NÃO existe no HEAD (2.2.2); destrava `infra "prod" {}` (3.2) e a migração DDL/runner |
