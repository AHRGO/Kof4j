...
| 2.2 | `kof.shell` — shell idiomático sobre `kof.process` | 🔵 | — | Estágio 1; **proposta de design em `future/shell-plan.md`** (PROPOSTO 18/09, barrado por Q1–Q3 — regra 6) |
...
| X10 | LSP domain-aware (completion + ir-para-definição em pacotes) | 🟡 | lane docs→plataforma (192.168.100.15) | fatias 1–2 ✅ 18/09: `StdCatalog` = membros reais de **18 namespaces** no completion do `kof lsp` (7 do dispatch KofStd + time/http/db/cache/process + os 6 de segurança) — fonte-única travada por teste contra os case-literals da fonte (`StdCatalogTest` 6/6, `LspServerTest` 24/24; `48633d98`, `e79a3ea0`); fatia 3 = receiver-typed restantes (json, observability, validation, tetris, ui?); ir-para-definição em pacotes = fatia 4 |
