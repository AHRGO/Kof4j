...
**Status:** **IN DEVELOPMENT — promoted from `future/` 22/09/2026** (maintainer order; `DECISIONS.md` §D-BAREMETAL-BOOT, queue 1.7) — the executable faces **B-0..B-3 + B-6 are LANDED** (the legacy BIOS path runs the real Kof `main` bare, end-to-end; `BiosBootE2ETest` 5/0F) and the **B-5 BIOS/UEFI platform bodies** (time via RTC/TSC, `sleep`, `mono`, `random`) **LANDED 24/09**. **B-4 (MCU ARM/riscv32):** the 32-bit codegen **LANDED** (`NativeMcuRiscv32` + `NativeMcuArm`; hello over UART + vector-table reset path asserted — `NativeMcuE2ETest` 7/7, `NativeMcuArmE2ETest` 6/0), but B-4 **does NOT close** yet — per **`D-BAREMETAL-MCU-GC` (24/09)** the collector `native-multiarch.md` **G-4/G-5 must be ported to the 32-bit MCU** (allocation + mark/sweep + a long-running proof under qemu) and the MCU `kof_plat_time` bodies filled (named wall refusal + SysTick monotonic). So the open code front is **B-4: allocator + collector on 32-bit**, not hello-level. **UPDATE 25/09 (lane baremetal 9092): the collector port LANDED on riscv32 (B4-GC-1..4: alloc + mark + sweep + long-running recycle proof, `NativeMcuGcRiscv32`/`NativeMcuGcRiscv32Sweep`, `NativeMcuGcTest` 8/0) and the MCU time bodies LANDED on riscv32 (B4-TIME: named wall refusal + `time`-counter mono + busy-wait sleep, `NativeMcuTimeRiscv32`, `NativeMcuTimeTest` 2/0). Remaining before closing B-4: mirror the collector/time to Cortex-M3 (`qemu-system-arm -M mps2-an385`) and wire the collector into the hello-level `NativeMcuRiscv32.emit` — the decision's "and/or" permits closing on riscv32, but full parity argues for both targets.**
