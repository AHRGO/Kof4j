...
**B-3b decomposition (recon 23/09 — the rest of B-3).** The flat image already holds the 64-bit Kof program right after the boot sector (`objcopy -O binary`), so the payload rides on the **same drive/sectors** — no second file needed. Three provable slices:
- **B-3b-1 (disk load):** the boot sector reads the payload sectors (LBA 1..N of the same image) via EDD `int 0x13, ah=0x42` into a staging buffer below 1 MiB; the build writes the payload length into a fixed header field so the boot code knows N. Proof: a per-sector progress marker on the serial and a printed length; an `int 0x13` carry-set path prints a **named failure** (never a silent hang). No long mode yet.
- **B-3b-2 (A20 + long mode):** enable A20 (`int 0x15, ax=0x2401`), build a flat GDT, set `CR0.PE`, far-jump to a 32-bit stub, set `CR4.PAE`, load PML4/PDPT/2 MiB-PD (identity-mapping the first GiB), set `CR0.PG` + `EFER.LME`, far-jump to 64-bit.
- **B-3b-3 (run the Kof payload):** the freestanding 64-bit `_start` is linked at a fixed base (e.g. `0x100000`); the boot code copies the staging buffer there in protected mode and jumps. The BIOS profile gets a `kof_plat_*` body (`RuntimePlat` branch: write = COM1 `out` 0x3F8, exit = `cli;hlt`, sync no-op) so the Kof `main` runs bare — E2E prints the Kof hello. Constraint (measured B-3a): the 64-bit program is emitted but unreachable today; B-3b needs a BIOS payload layout (load base) distinct from the MBR.
