...
| 4 | `planning-otp-supervision.md` (#83) | `EM CURSO` — 1ª fatia ✅ 11/09 (núcleo+`restartLimit`+`stop`) + **S2-JVM ✅ 13/09** (`startAll`/`lacoUnico` + wrapper de identidade) + **S2-Native x86 ✅ 15/09** (§129 fechado — chain TLS por thread + handler por worker; `KofSupervisorE2ETest` 16/16 incl. `supervisorNativeParityX86`/`supervisorNativeS2ParityX86`; riscv/aarch=OTP001) + **S2-JS ✅ 18/09** (§132 fechado — `time.sleep` async cooperativo; `OTP002` levantado; `supervisorJsParity`/`supervisorJsS2Parity`) | **DD-OTP RATIFICADAS 13/09** (opção 1a: S2 JVM; riscv/aarch PARTIAL) — ver §3 | riscv/aarch (clone cru, sem TLS) segue gate honesto (`OTP001`); JS roda o núcleo com paridade desde 18/09 |
...
| 8 | `IMPLEMENTATION-UNIVERSAL-PLATFORM.md` (+ companion de visão `docs/architecture/UNIVERSAL-PLATFORM-VISION.pt_BR.md`) | `EM CURSO` — **promovido de `future/` 17/09** (`DECISIONS.md` §D-UNIVERSAL, R12 sobreposto); dividido 17/09 em passos executáveis + companion de visão | diretriz da mantenedora 17/09: promover e implementar | **Estágios 1–8 + R1–R12 como itens executáveis** (status ✅/🟡/🔵/⛔ + lane dona + prova) — estado vivo: **R1 ✅ FEITO** (`5f1422c6` gate+ledger+CI da fronteira); **R6 ✅ gate de máquina** (`DomainGapCodesTest…` `19a740f2` + varredura do ledger `c5897cd5`); **1.5 ✅ export OTel landado** (`435b7013`; Native `OBS003`); 1.1 MEDIA = `MEDIA001/003` documentados, na fila atrás das facades HTTP da `.22`; 1.2 GC x86 = lane nat (§260 G-6(a) precisa sign-off da mantenedora ⛔ D1); 1.4 registry = decisão da mantenedora ⛔ D2. Reivindicar em `DOING.md` antes do código |
| 9 | `workflow-plan.md` (+PT) — `kof.workflow` (TIER 2.1, Stage 2) | `PLANO ASSINADO` (⛔ rule 6 → ✅ poll da mantenedora 19/09: Q1 stdlib, Q2 MVP mínimo, Q3 retry aditivo, Q4 ambas as faces de dead-letter) — **owner `.18`, zero code shipped** | entered by explicit promotion (three-states rule: an opened front is no longer "future"); it used to be the bare 🔵 tracker row 2.1 | 2.1.0 recon by `.18` (measure lexer/parser/stdlib for `job`/`dag`/`after`/`run`/`Report`); this lane only moved and classified it — content untouched |
...
| DD-OTP (restante) | `planning-otp-supervision.md` | ✅ RATIFICADAS 13/09 (opção 1a: wrapper `(id, resultado)`; riscv/aarch PARTIAL) — **S2-JVM ✅ IMPLEMENTADO 13/09** (`Supervisor.startAll`/laço selectAny único) e **S2-Native x86 ✅ IMPLEMENTADO 15/09** (§129 fechado, DECISIONS §2 opção B) e **S2-JS ✅ IMPLEMENTADO 18/09** (§132 fechado — `time.sleep` assíncrono cooperativo, `OTP002` elevado); resta riscv/aarch (`OTP001`, clone cru) |
...
| `planning-otp-supervision.md` | 1ª fatia ✅ JVM+Script; **S2-JVM ✅ 13/09** (`startAll`/`lacoUnico`); **S2-Native x86 ✅ 15/09** (§129 fechado — chain de handler por thread); **S2-JS ✅ 18/09** (§132 fechado — sleep async cooperativo; `OTP002` levantado) | riscv/aarch (`OTP001`) segue gate honesto; JS roda com paridade |
