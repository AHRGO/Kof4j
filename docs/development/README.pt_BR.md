...
| — | `db-parity-plan.md` (+PT) — adendo `D-DB-GAPS` 21/09 | `EM DESENVOLVIMENTO` — mantenedora 21/09: **paridade total de DB** (todo alvo aceita mariadb/mysql/sqlite/mongodb); matriz medida + fatias S0–S4 | frente DB/ORM (dono a nomear); docs/plataforma mantém o registro | S0 = diagnóstico interino honesto que limpa o §421; S1 `mariadb://` = alias mysql-wire (Native); S2 paridade JDBC JVM/JS/Android; S3 `mongodb://` interop-first (R9); S4 oracle |
| — | `codegen-step-2.2.3-assessment.md` (+PT) — roadmap 2.2.3 | `PROPOSTA PENDENTE (regra 6)` — sem código | medido 21/09: **descompasso de fase** (hook = IR otimizada; DDL = lowering; runner = desugar de AST) | opções A (fechar 2.2.3 como obsoleto) / B (registry `DesugarStep` na fase de AST; código após a decisão) aguardam a mantenedora |
