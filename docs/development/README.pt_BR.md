...
| — | `codegen-step-2.2.3-assessment.md` (+PT) — roadmap 2.2.3 | `DECIDIDA 21/09 opção B` (`D-DESUGAR-STEP`) — construir o registry `DesugarStep` na fase de AST (behavior-free) | medido 21/09: **descompasso de fase** (hook = IR otimizada; DDL = lowering; runner = desugar de AST) | fatias abertas: migrar os quatro desugars para `DesugarStep`s registrados (behavior-free, regra 3 de congelamento); o doc vai p/ `docs/architecture/` ao concluir |
...
**15 itens na fila aberta** (ressincronizado 21/09 ~13:1x — 20→19
...
| ~~`known-bugs.md`~~ → `docs/bugs-and-gaps/` | **15 vivos** (contagem viva — autoridade é `scripts/check_known_bugs_status.sh`; ressincronizado 21/09; 18→16 quando §192 e §358 foram fechados pela varredura de órfãos da lane nat em 21/09; 16→15 quando o §258 foi fechado 21/09 (#775 `NumericFormatterE2ETest` + #776 `KofHttp.supportedOn`); era 20 — §380 `9f383bcf`, §381 `576a1dcb`, §394 `d0464385` e §353 21/09 fechados; §400/§418/§421 catalogados/re-publicados; a fila viva do §2 é a autoridade; a contagem histórica de 14/09 era 32; §81/§163/§127-JVM, §155, §94, §157-160 e §65 fechados/NÃO-REPRODUZ 13/09) | fila viva |
