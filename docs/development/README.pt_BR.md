...
- **Pendentes (condição 3 do gate de release):** `ffi-abi-structs.md`
  ...
  plataforma) ·
  `secrets-plan.md` (Estágio 5/tracker 3.6 — **face 1 `Secret` POUSADA 21/09
  `32285136`**, JVM-primeiro; P2/P3 pendentes de voto próprio; dono lane de
  security) ·
...
| 11 | ~~`makealive-plan.md` (+PT)~~ → `docs/architecture/makealive-plan.md` — D-MAKEALIVE: infraestrutura como código tipado — **todas as linhas 3.1–3.8 POUSADAS** (core MK-1 20/09; 3.3 reconcile; 3.2 `966c86a4` `D-MAKEALIVE-SYNTAX`; 3.7 fechada runtime-only; 3.8 `D-MAKEALIVE-CLI`; **3.6 secrets pousou 21/09 `32285136`**) | ✅ **CONCLUÍDO + MOVIDO 21/09** | — (regra dos três estados; follow-ups de `secrets-plan.md` P2/P3 rastreados lá, não aqui) |
| — | `secrets-plan.md` (+PT) — `D-SECRETS`, Estágio 5/tracker 3.6 | `EM DESENVOLVIMENTO` — **face 1 (`Secret`) POUSADA 21/09 `32285136`** (JVM-primeiro; `secrets.of`/`secrets.secret` + `reveal()`/`redacted()` + `==` constant-time; gap `SECN008` em JS/Native/Script/Android; `SecretE2ETest` 4/4) | **lane security**; `fromBytes`/buffer zerável + P2 (redação forçada) + P3 (`KeyHandle`) = follow-ups com voto rule-6 próprio | — |
| — | `ffi-abi-structs.md` (+PT) — ABI de struct/array da FFI (D6) | `EM DESENVOLVIMENTO` — **D6 DECIDIDO 20/09** (`DECISIONS.md` §D-FFI-STRUCT: **D6-1 = B** (`D-FFI-STRUCT-B` 21/09, substitui A+B: `struct` mutável por-ref; records seguem por-valor read-only, `Buffer(U8)` cobre out-buffer), D6-2 só `new T[n]`, D6-3 `Buffer(U8,INOUT)`, D6-4 sret completo, D6-5 arena confinada); spec §4 = DECIDIDA; 3.8a `AbiLayout` landou; **3.8b fatia 1 landou 20/09** (`e79ea4e6` — record por valor como struct C **argumento** no JVM, D6-1) + **fatia 2 landou 21/09** (record **devolvido** por valor: registrador + sret, reconstrução pelo construtor canônico; D6-4) + **fix de arena D6-5 landou 21/09** (helpers escalares confinados por chamada + close; `FfiStructE2ETest` 10/10, `FfiE2ETest` 17/17) + **fatia 3 landou 21/09** (D6-2: `T[]` escalar→`ptr` C, **copy-in por chamada**; `FfiArrayE2ETest` 5/5) + **out-buffer D6-3 landou 21/09** (B1 `buffer.alloc`+`Buffer.bytes()` `BufferE2ETest` 4/4; B2 `Buffer(U8)` `extern` INOUT copy-in/chamada/copy-back `BufferFfiE2ETest` 4/4) | exec = dono **`jonas`** (frente FFI pós-#431) + compiler/native + decisão JS; **a lane docs só mantém o registro** | restam: retorno de struct + array/buffer no JS (cross-lane) e sret no Native (3.7) — o bridge de struct **param** no JS pousou 21/09 (`structParamByValueJsParity`, JVM==JS); o leak `Arena.global` da §1 está **CORRIGIDO** (D6-5, não mais candidata a fix) |
...
| — | `type-system-extensions-plan.md` (+PT) — X5 variância+sealed / X6 reflexão de interop | `ABERTO — implementando` — **X5/X6 APROVADOS 21/09** (`D-TYPE-VARIANCE`/`D-INTEROP-REFLECT`), promovidos de `future/`; direção travada, superfície pendente | plano aprovado; a condição 2 do gate segue `NEEDS-REVIEW` (não RED) enquanto a frente de superfície não landa | exec = lane compiler, fatias incrementais cada uma com sua prova; a lane docs mantém o registro |
...
| ~~`known-bugs.md`~~ → `docs/bugs-and-gaps/` | **16 vivos** (contagem viva — autoridade é `scripts/check_known_bugs_status.sh`; ressincronizado 21/09 — snapshot após o fix S0 do §421 + §432 corrigido pela sessão 9093 (VerifyError do `getOrDefault` JVM, tipo do arg escrito separado do V do slot) + §423–§431 catalogados + §433/§434 (vermelhos remotos FFI/JS) pelo re-pouso da gaps-db + §429 corrigido (LSP `-32601`) e §431 fechado pela lane `.18` + §435 (check_500 vermelho catalogado pela lane gaps-db) fechado pela lane `.18` (check_500 `LspServer` 601→582, split em `LspJsonRpc`); 18→16 quando §192 e §358 foram fechados pela varredura de órfãos da lane nat em 21/09; 16→15 quando o §258 foi fechado 21/09 (#775 `NumericFormatterE2ETest` + #776 `KofHttp.supportedOn`); 15→16 quando o §423 (canais no cross, NAT005) foi catalogado em 21/09; era 20 — §380 `9f383bcf`, §381 `576a1dcb`, §394 `d0464385` e §353 21/09 fechados; §400/§418/§421 catalogados/re-publicados; a fila viva do §2 é a autoridade; a contagem histórica de 14/09 era 32; §81/§163/§127-JVM, §155, §94, §157-160 e §65 fechados/NÃO-REPRODUZ 13/09) | fila viva |
