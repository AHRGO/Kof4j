...
**20 itens na fila aberta** (ressincronizado 21/09 — **21→20 quando o §435** (check_500 `LspServer` 601, lane CLI/plataforma) foi CORRIGIDO ✅ 21/09 pela lane `.18` (split em `LspJsonRpc`; `LspServer` 601→582; `check_500` rc=0); havia sido catalogado 21/09; **20→22 quando o re-pouso F2d1 da gaps-db catalogou §433/§434** (os dois vermelhos do tip remoto da lane FFI/JS, `2995d0f8`: bundle JS hello +5% do orçamento, auditoria de gate do Buffer sem JS); 23→22 quando **§429** (request desconhecida no LSP agora responde `-32601`, lane docs/plataforma) foi CORRIGIDO ✅ 21/09, 22→21 quando **§431** (tooling drift da CLI, lane `.18`) foi CORRIGIDO ✅ 21/09; 20→19
...
| ~~`known-bugs.md`~~ → `docs/bugs-and-gaps/` | **20 vivos** (contagem viva — autoridade é `scripts/check_known_bugs_status.sh`; ressincronizado 21/09 — snapshot após o fix S0 do §421 + §423–§431 catalogados + §433/§434 (vermelhos remotos FFI/JS) pelo re-pouso da gaps-db + §429 corrigido (LSP `-32601`) e §431 fechado pela lane `.18` + §435 fechado pela lane `.18` (check_500 `LspServer` 601→582, split em `LspJsonRpc`); 18→16 quando §192 e §358 foram fechados pela varredura de órfãos da lane nat em 21/09; 16→15 quando o §258 foi fechado 21/09 (#775 `NumericFormatterE2ETest` + #776 `KofHttp.supportedOn`); 15→16 quando o §423 (canais no cross, NAT005) foi catalogado em 21/09; era 20 — §380 `9f383bcf`, §381 `576a1dcb`, §394 `d0464385` e §353 21/09 fechados; §400/§418/§421 catalogados/re-publicados; a fila viva do §2 é a autoridade; a contagem histórica de 14/09 era 32; §81/§163/§127-JVM, §155, §94, §157-160 e §65 fechados/NÃO-REPRODUZ 13/09) | fila viva |
