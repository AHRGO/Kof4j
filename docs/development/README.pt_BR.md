...
| ~~`known-bugs.md`~~ → `docs/bugs-and-gaps/` | **19 vivos** (contagem viva — autoridade é `scripts/check_known_bugs_status.sh`; ressincronizado 21/09 — snapshot após o fix S0 do §421 + §432 corrigido pela sessão 9093 (VerifyError do `getOrDefault` JVM, tipo do arg escrito separado do V do slot) + §423–§431 catalogados + §433/§434 (vermelhos remotos FFI/JS) pelo re-pouso da gaps-db + §429 corrigido (LSP `-32601`) e §431 fechado pela lane `.18` + §435 fechado pela lane `.18` (check_500 `LspServer` 601→582, split em `LspJsonRpc`); 18→16 quando §192 e §358 foram fechados pela varredura de órfãos da lane nat em 21/09; 16→15 quando o §258 foi fechado 21/09 (#775 `NumericFormatterE2ETest` + #776 `KofHttp.supportedOn`); 15→16 quando o §423 (canais no cross, NAT005) foi catalogado em 21/09; era 20 — §380 `9f383bcf`, §381 `576a1dcb`, §394 `d0464385` e §353 21/09 fechados; §400/§418/§421 catalogados/re-publicados; a fila viva do §2 é a autoridade; a contagem histórica de 14/09 era 32; §81/§163/§127-JVM, §155, §94, §157-160 e §65 fechados/NÃO-REPRODUZ 13/09) | fila viva |
