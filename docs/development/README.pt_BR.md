...
| 9 | `workflow-plan.md` (+PT) — `kof.workflow` (TIER 2.1, Stage 2) | `LANDED` (poll 19/09; owner `.18`): 2.1.0 recon + 2.1.2 MVP + 2.1.3 retry/deadLetter/schedule/checkpoint todos entregues 19/09 (WorkflowE2ETest 20/20; suíte 4 módulos verde; commits `7db91735`→`285e23ef`) — residual rule-6 FECHADO 19/09: a mantenedora respondeu a pergunta ("pode ir pra fatia 3") e a 5ª face `runSupervised` (integração supervisor one_for_one) landou na fatia 3b | entered by explicit promotion (three-states rule: an opened front is no longer "future"); it used to be the bare 🔵 tracker row 2.1 | 2.1.0 recon by `.18` (measure lexer/parser/stdlib for `job`/`dag`/`after`/`run`/`Report`); this lane only moved and classified it — content untouched |
