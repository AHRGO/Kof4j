...
| — | `PLAN-BAREMETAL-BOOT.md` (+PT) — bare-metal/bootável com ring0/ring1 (faces B-0…B-6) | `EM DESENVOLVIMENTO` — **PROMOVIDO de `future/` 22/09** (`D-BAREMETAL-BOOT`, ordem da mantenedora) · **B-0..B-3 + B-6 LANDADOS** (costura `kof_plat_*` no x86+cross; link freestanding + heap/stack configuráveis + `_end`; dtoa Schubfach libc-free no x86+cross (B-1c, §448); perfil UEFI; **boot BIOS legado roda o `main` Kof REAL bare** — B-3a+B-3b, `KO-BIOS OK`/`LM64 OK`/`PAYLOAD` sob SeaBIOS, `BiosBootE2ETest` **5/0F**; ring0/ring1 com prova de `#GP` + sabotagem da GDT) | **lane `baremetal` (sessão 9092)** | restante: **B-4 (MCU ARM/riscv32)** — classificado R/H e **depende do coletor G-4/G-5** (`native-multiarch.md`); exige decisão/recorte da mantenedora (regra 6), NÃO atacado por agente sem ela. A face BIOS está end-to-end (sem fatia de código aberta) |
