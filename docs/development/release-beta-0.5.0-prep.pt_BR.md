...
| 3 | Todos os `docs/development/*.md` soltos concluídos e movidos | regra dos três estados; só fica trabalho com implementação pendente | RED (5 docs com dono em curso: `ffi-abi-structs` [jonas], `IMPLEMENTATION-UNIVERSAL-PLATFORM` [9093], `secrets-plan` [security], `db-parity-plan` [docs/plataforma], `type-system-extensions-plan` [compiler]; `makealive-plan` MOVIDO para `docs/architecture/` 21/09 — todas as linhas 3.1–3.8 pousadas) |
