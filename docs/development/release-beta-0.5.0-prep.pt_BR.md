...
| 3 | Todos os `docs/development/*.md` soltos concluídos e movidos | regra dos três estados; só fica trabalho com implementação pendente | RED (4 docs com dono em curso: `ffi-abi-structs` [jonas], `IMPLEMENTATION-UNIVERSAL-PLATFORM` [9093], `db-parity-plan` [docs/plataforma], `type-system-extensions-plan` [compiler]; `makealive-plan` MOVIDO para `docs/architecture/` 21/09 e `secrets-plan` MOVIDO 21/09 — ambos concluídos, todas as linhas/faces pousadas) |
...
| 7 | Nada pendente em bugs-and-gaps | conjunto live do `check_known_bugs_status.sh` vazio + `specification-gaps.md` 0 abertos | RED (15 live no tip — snapshot 21/09 após o fix S0 do §421 + §423–§431 catalogados, §431 fechado pela lane `.18`, §429 corrigido (LSP `-32601`), §435 fechado pela lane `.18` (check_500 `LspServer` 601→582, split em `LspJsonRpc`), §432 corrigido (VerifyError do `getOrDefault` JVM) e §428 corrigido (requests não implementadas do DAP agora falham honestamente) e §425 corrigido (`kof.config` no riscv64/aarch64 agora `CONF001` honesto) e §426 corrigido (`time.collect()` no JS agora `TIME004`) e §427 corrigido (`kof.io`/web-T1 no riscv64/aarch64 agora `NAT006`/`NAT007`); o script é a autoridade; ledger ilegível = `UNKNOWN`, nunca GREEN) |
