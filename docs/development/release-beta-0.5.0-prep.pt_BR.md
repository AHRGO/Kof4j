...
| 7 | Nada pendente em bugs-and-gaps | conjunto live do `check_known_bugs_status.sh` vazio + `specification-gaps.md` 0 abertos | RED (17 live no tip — snapshot 21/09 após o fix S0 do §421 + §423–§431 catalogados, §431 fechado pela lane `.18`, §429 corrigido (LSP `-32601`), §435 fechado pela lane `.18` (check_500 `LspServer` 601→582, split em `LspJsonRpc`), §432 corrigido (VerifyError do `getOrDefault` JVM) e §428 corrigido (requests não implementadas do DAP agora falham honestamente) e §425 corrigido (`kof.config` no riscv64/aarch64 agora `CONF001` honesto); o script é a autoridade; ledger ilegível = `UNKNOWN`, nunca GREEN) |
