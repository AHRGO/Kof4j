...
| 4 | Estabilidade total | suíte completa 0F/0E + matriz 5/5 na candidata. **Medida automaticamente** a partir de um log real da suíte via `scripts/stability-report.sh` (`KOF_SUITE_LOG=…`); o GREEN exige o `TOTAL` 0F/0E **e** o log **estampado** (`SUITE-SHA` == tip, `SUITE-DIRTY=0`) — log de outro commit ou de árvore suja é `unknown`, nunca verde falso | MEDIÇÃO GREEN em `8f459b8e` 21/09 ~06:0x (lane .18): primeiro log de `safe-suite.sh` autocertificado — `TOTAL: tests=3355 failures=0 errors=0 skipped=223`, `SUITE-SHA`==tip, `SUITE-DIRTY=0`, e o `stability-report.sh` devolveu GREEN a partir dele (o bug do mecanismo foi achado e corrigido na mesma rodada: o `safe-suite.sh` antigo imprimia o `TOTAL` só no console e nunca o anexava ao log, então NENHUMA corrida estampada podia ser certificada). O candidato ainda exige re-medição no tip final limpo na hora do corte — isso é a regra da condição 6, não dúvida desta linha. **Corrida completa nova em 21/09 no `96af9b63`** (primeira após o conserto do mecanismo): `TOTAL: tests=3413 failures=1 errors=0 skipped=223` — **RED por UM teste stale**, `CompilerDriverTest.externProducesHonestGapNotSilentDrop`, que ainda exige `FFI001` para um extern `Int[]` que o `7c6413d4` (D6-2) tornou bindável no JVM de propósito (provado por `FfiArrayE2ETest` 5/5; `String[]`/`List<T>` seguem `FFI001`). Hand-off para a frente FFI (`jonas`): reapontar a asserção para uma assinatura ainda não-ligada, não apagar. **RESOLVIDO 21/09 pela lane FFI (§422, `9d97b268`):** a asserção agora mira assinaturas genuinamente não-ligadas (`String[]`/`List<Int>`→`FFI001`, `Buffer(Int)`→`SEM096`) e um pin novo foi adicionado; `CompilerDriverTest` 259/0F/0E — o vermelho stale sumiu. O candidato ainda exige a própria re-medição estampada na hora do corte (regra da condição 6), não a dúvida desta linha. |
...
| 7 | Nada pendente em bugs-and-gaps | conjunto live do `check_known_bugs_status.sh` vazio + `specification-gaps.md` 0 abertos | RED (18 live no tip; ledger ilegível = `UNKNOWN`, nunca GREEN) |
