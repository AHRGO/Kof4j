...
| 7 | Nada pendente em bugs-and-gaps | conjunto live do `check_known_bugs_status.sh` vazio + `specification-gaps.md` 0 abertos | RED (21 live no tip — snapshot 21/09 após o fix S0 do §421 + §423–§431 catalogados, §431 fechado pela lane `.18`, §429 corrigido (LSP `-32601`) e §435 catalogado (check_500 `LspServer` 601, lane CLI/plataforma); o script é a autoridade; ledger ilegível = `UNKNOWN`, nunca GREEN) |
