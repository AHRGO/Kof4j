...
| 13 | `kof.io` file faces on cross | ✅ | ✅ x86 | 🟡 partial — stat+text+fs (`exists`/`isFile`/`isDirectory`/`readText`/`writeText`/`appendText`/`delete`/`create`/`createDirectories`/`mkdirs`/`size`/`readBytes`/`writeBytes`/`appendBytes`/`list`/`readRange`) ✅ (`NativeRiscvAsmIoStat`/`IoText`/`IoFs`/`IoMkdirs`/`IoSize`/`IoBytes`/`IoDirList`/`IoReadRange`); remaining faces (`modifiedTime`/`name`/`copy_to`/`move_to`/dir `delete`/`symlink`/`path_*`) ❌ `NAT006`; `size()` msg JVM×x86 diverge (§494) | ✅ | `NAT006`/`NAT007` | native cross lane |
