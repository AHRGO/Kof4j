...
| 3 | `ssh.cmd`/`run`/`ok` | ✅ | ✅ x86 `run`/`cmd`/`ok` 26/09 | ❌ | ❌ | `PROC001` (cross + JS) | native-cross lane (x86 ✅ 26/09) |
...
| 9 | `cache.*`/`config.*`/`log.*` cross golden; `log` interpreter | ✅ (log ⏳ interp) | ✅ x86 | `cache.*` ✅ 26/09 (`KofCacheCrossTest`, riscv64+aarch64 — `cache.ttl` −1 parity fixed, §501); `log` ✅ nível+filtro+rótulo JVM 26/09 (`NativeLogCrossTest` 6/6); timestamp ⏳ (fatia 2b); `config` ❌ `CONF001` | ✅ | `CONF001` | lane parity (cache ✅ 26/09) / stdlib lane |
