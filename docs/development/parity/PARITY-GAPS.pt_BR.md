...
| 13 | faces de arquivo do `kof.io` no cross | ✅ | ✅ x86 | 🟡 parcial — estat+texto+fs (`exists`/`isFile`/`isDirectory`/`readText`/`writeText`/`appendText`/`delete`/`create`) ✅ (`NativeRiscvAsmIoStat`/`IoText`/`IoFs`); createDirectories/bytes/listing ❌ `NAT006` | ✅ | `NAT006`/`NAT007` | lane native cross |
