...
| 9 | `cache.*`/`config.*`/`log.*` golden cross; `log` interpretador | ✅ (log ⏳ interp) | ✅ x86 | `cache.*` ✅ 26/09 (`KofCacheCrossTest`, riscv64+aarch64 — `cache.ttl` −1 paridade corrigida, §501); `log` ✅ nível+filtro+rótulo JVM 26/09 (`NativeLogCrossTest` 6/6); timestamp ⏳ (fatia 2b); `config` ❌ `CONF001` | ✅ | `CONF001` | lane parity (cache ✅ 26/09) / lane stdlib |
