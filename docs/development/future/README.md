...
| `../shell-plan.md` (moved) | `kof.shell` — idiomatic shell over `kof.process` — **RECLASSIFIED 19/09: v1 MVP landed (`34e4344f`); the doc now lives in `docs/development/`** — residual faces (pipeline JS/Native, glob/`~`/redireção v2) tracked there | — | — |
| `workflow-plan.md` | `kof.workflow` — jobs/pipelines/retry/checkpoints/dead-letter (TIER 2.1, Stage 2) — **SIGNED-OFF 19/09, owner `.18`; no code shipped yet (2.1.0 recon next)** | `kof.workflow` is still not in the lexer/parser/backends; the maintainer poll answered Q1–Q4 (stdlib form, minimal MVP, ADDITIVE retry — http migrates later, both dead-letter faces). Design turns tracker row 2.1 into an executable slice todo. It is a **composition layer** over already-shipped primitives — ABI measured in §4 across `kof.process`/`scheduler`/`orm`/`supervisor`/`concurrency` (JVM+ANDROID+JS fully green; Native = `PROC001`/`CRON001`/`ORM001` honest, inherited verbatim) |
