...
|-----|------|---------------------------|
| `type-system-extensions-plan.md` | X5 variance + sealed types · X6 interop reflection (`DECISIONS.md` §D-TYPE-VARIANCE/§D-INTEROP-REFLECT, 21/09) — spec-first | **plan only, zero code** — core type-system change / reflection surface, awaiting maintainer review (rule 6); slices X5.0–X5.5 / X6.0–X6.3 |
...
| `secrets-plan.md` | `Secret`/`KeyHandle` + enforced redaction (Stage 5, tracker line 3.6) — P1/P2/P3, each one awaits its own rule-6 vote | **zero code** — design only (21/09 audit lane); the current raw-`String` surface (`secrets.get`/`redact`, `jwt`) keeps running untouched |
