...
| `../workflow-plan.md` (moved) | `kof.workflow` — jobs/pipelines/retry/checkpoints/dead-letter (TIER 2.1) — **RECLASSIFIED 19/09: signed off by maintainer poll (Q1–Q4) and 2.1.0 recon DONE (`WorkflowPrimitivesE2ETest` 6/6 + lambda `Result` descriptor fix `8ec07214`); the doc now lives in `docs/development/`** — the MVP (2.1.2, pure-Kof stdlib) executes there |
