...
| `shell-plan.md` | `kof.shell` — idiomatic shell over `kof.process` (TIER 2.2, Stage 2) — **PROPOSED 18/09, awaiting maintainer scope decision (rule 6)** | **zero code** — `kof.shell` is not in the lexer/parser/backends; it is a design proposal turning the bare tracker row 2.2 (`🔵`, owner `—`) into an executable slice todo, gated by Q1–Q3. ABI claims are measured against the existing `kof.process` surface (JVM/JS real, Native `PROC001`) |
