...
| `shell-plan.md` | `kof.shell` — shell idiomático sobre `kof.process` (TIER 2.2, Estágio 2) — **APROVADO 18/09, dono `.18`; MVP entregue no mesmo dia** | Namespace builtin em forma de função sobre a superfície medida de `kof.process` (Q1–Q3 respondidos pela enquete da mantenedora): `cmd`/`run`/`ok` reais em JVM+JS (byte parity, `ShellE2ETest`), `pipeline` só-JVM com `PROC001` honesto em JS/Native (herdado de `process.spawn`); glob/`~`/redir ficam fora do v1 |
| `workflow-plan.md` | `kof.workflow` — jobs/pipelines/retry/checkpoints/dead-letter (TIER 2.1, Estágio 2) — **PROPOSTO 18/09, aguardando decisão de escopo da mantenedora (regra 6)** | **zero código** — `kof.workflow` não está no lexer/parser/backends; proposta de design que vira a linha solta do tracker 2.1 (`🔵`, dono `—`) numa fila de fatias executável, barrada por Q1–Q4. É uma **camada de composição** sobre primitivas já entregues — ABI medida no §4 cruzando `kof.process`/`scheduler`/`orm`/`supervisor`/`concurrency` (JVM+ANDROID+JS totalmente verdes; Native = `PROC001`/`CRON001`/`ORM001` honestos, herdados verbatim) |
