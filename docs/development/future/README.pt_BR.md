...
| ~~`secrets-plan.md`~~ **promovido 21/09 → [`../secrets-plan.md`](../secrets-plan.md)** | `Secret`/`KeyHandle` + redação forçada (Estágio 5, tracker 3.6) | não fica mais em `future/` — **face 1 autorizada** (três estados: implementação pendente) |
