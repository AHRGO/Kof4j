...
| `../shell-plan.md` (movido) | `kof.shell` — shell idiomático sobre `kof.process` — **RECLASSIFICADO 19/09: MVP v1 landou (`34e4344f`); o doc agora mora em `docs/development/`** — faces residuais (pipeline JS/Native, glob/`~`/redireção v2) rastreadas lá | — | — |
| `workflow-plan.md` | `kof.workflow` — jobs/pipelines/retry/checkpoints/dead-letter (TIER 2.1, Estágio 2) — **APROVADO 19/09, dono `.18`; nenhum código entregue ainda (recon 2.1.0 é o próximo)** | `kof.workflow` ainda não está no lexer/parser/backends; a enquete da mantenedora respondeu Q1–Q4 (forma stdlib, MVP mínimo, retry ADITIVO — http migra depois, ambas as faces de dead-letter). O design vira a linha do tracker 2.1 numa fila de fatias executável. É uma **camada de composição** sobre primitivas já entregues — ABI medida no §4 cruzando `kof.process`/`scheduler`/`orm`/`supervisor`/`concurrency` (JVM+ANDROID+JS totalmente verdes; Native = `PROC001`/`CRON001`/`ORM001` honestos, herdados verbatim) |
