...
|-----|------|---------------------------|
| `type-system-extensions-plan.md` | X5 variance + sealed types · X6 reflexão de interop (`DECISIONS.md` §D-TYPE-VARIANCE/§D-INTEROP-REFLECT, 21/09) — spec-first | **só plano, zero código** — mudança de núcleo do sistema de tipos / superfície de reflexão, aguardando revisão da mantenedora (regra 6); fatias X5.0–X5.5 / X6.0–X6.3 |
...
| `secrets-plan.md` | `Secret`/`KeyHandle` + redação forçada (Estágio 5, linha 3.6 do tracker) — P1/P2/P3, cada um espera seu próprio voto rule-6 | **zero código** — só design (21/09 lane de auditoria); a superfície crua atual (`secrets.get`/`redact`, `jwt`) segue rodando intacta |
...
| `graphics-gaming-plan.md` | superfície de intenção de 2D/3D/jogo/som/vídeo (`scene`/`frame`, sprites/tiles, malha/câmera/material, `sound.play`, painel `video`) — `DECISIONS.md` §D-GRAPHICS-GAMING + 3 adendos (20/09) | **só plano, zero código** — registrado a pedido da mantenedora 20/09; aceite é paridade TOTAL nos 4 alvos (adendo 2 — R7 não vale aqui); erradicação JavaFX medida com 0 ligações (adendo 3); stack interop-first (classe SDL/miniaudio/ffmpeg — medida, nunca prometida); frentes abrem só por promoção explícita de fatia por ela (R12 + regra 6) |
