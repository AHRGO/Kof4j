...
**Location:** `docs/development/future/` — a plan with **no code** and **non-normative**; it is not in `DECISIONS.md`.  
...
> ## MEL'S REVIEW AND APPROVAL ARE MANDATORY
>
> This document is NOT a decision already taken by the project.
>
> It gathers the current KOF contract, the real state of the repository and the
> maintainer's guidance that is published in Git or that was recorded in Git as
> orders given outside it.
>
> **No agent or contributor may ratify this document on Mel's behalf.**
>
> Only after Mel's explicit review and approval may this contract:
>
> 1. become a normative decision in `docs/development/DECISIONS.md`;
> 2. be synchronized with the release/versioning documents;
> 3. be turned into mandatory automatic gates;
> 4. guide the cut of the first 1.0 Release Candidate.
...
The repository today still shows, on `beta-0.5.0`:
...
**Metadata already diverging today (measured 09/20/2026, unrelated to a bump):** the public site shows `v0.4.1-beta` while `VERSION` and `pom.xml` are at `0.4.7-beta`; and `docs/distribution/release-naming.md` still says "Current version: 0.4.0-beta". These are documentation/site drift, not a release decision.
...
Reviewer:
Date:
Decision reference / commit:
Notes:
```

No agent fills this block on the maintainer's behalf.
...
### Reinforcement candidates — NOT RATIFIED

```text
[? MEL] Stable Surface snapshot frozen at RC1
[? MEL] 0 mandatory failures without classification
[? MEL] flaky-test policy applied to the candidate
[? MEL] candidate compared with the last accepted baseline
[? MEL] compatibility corpus executed
[? MEL] real KOF application validated with the candidate package
[? MEL] digest of the tested package == digest of the published package
[? MEL] evidence manifest per target
[? MEL] release waiver only explicit, documented and approved
```
...
**This document remains a PROPOSAL — NOT RATIFIED.** No approval block was filled in.
