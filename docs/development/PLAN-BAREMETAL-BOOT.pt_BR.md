...
**Status:** **EM DESENVOLVIMENTO — promovido de `future/` 22/09/2026** (ordem da mantenedora; `DECISIONS.md` §D-BAREMETAL-BOOT, fila 1.7) — as faces executáveis **B-0..B-3 + B-6 estão LANDADAS** (o caminho BIOS legado roda o `main` Kof REAL bare, end-to-end; `BiosBootE2ETest` 5/0F) e os **corpos de plataforma B-5 do BIOS/UEFI** (`time` via RTC/TSC, `sleep`, `mono`, `random`) **LANDARAM 24/09**. **B-4 (MCU ARM/riscv32):** o codegen 32-bit **LANDOU** (`NativeMcuRiscv32` + `NativeMcuArm`; hello pela UART + reset path da vector table asserido — `NativeMcuE2ETest` 7/7, `NativeMcuArmE2ETest` 6/0), mas o B-4 **NÃO fecha** ainda — por **`D-BAREMETAL-MCU-GC` (24/09)** o coletor `native-multiarch.md` **G-4/G-5 deve ser portado para o MCU 32-bit** (alocação + mark/sweep + prova long-running sob qemu) e os corpos de `kof_plat_time` do MCU preenchidos (recusa NOMEADA do wall + mono via SysTick). Logo a frente de código aberta é **B-4: alocador + coletor no 32-bit**, não o hello. **ATUALIZAÇÃO 25/09 (lane baremetal 9092): o port do coletor LANDOU no riscv32 (B4-GC-1..4: alocação + mark + sweep + prova long-running de reciclagem, `NativeMcuGcRiscv32`/`NativeMcuGcRiscv32Sweep`, `NativeMcuGcTest` 8/0) e os corpos de tempo do MCU LANDARAM no riscv32 (B4-TIME: recusa NOMEADA do wall + mono pelo contador `time` + sleep busy-wait, `NativeMcuTimeRiscv32`, `NativeMcuTimeTest` 2/0). Falta antes de fechar o B-4: espelhar o coletor/tempo para Cortex-M3 (`qemu-system-arm -M mps2-an385`) e integrar o coletor no `NativeMcuRiscv32.emit` (hoje hello-level).**
