...
**Decomposição do B-3b (recon 23/09 — o resto do B-3).** A imagem flat já contém o programa Kof 64-bit logo após o setor de boot (`objcopy -O binary`), então o payload viaja no **mesmo disco/setores** — não precisa de um segundo arquivo. Três fatias prováveis:
- **B-3b-1 (carga do disco):** o setor de boot lê os setores do payload (LBA 1..N da mesma imagem) via EDD `int 0x13, ah=0x42` para um buffer de staging abaixo de 1 MiB; o build grava o comprimento do payload num campo fixo do header para o boot saber N. Prova: um marcador de progresso por setor no serial e o comprimento impresso; caminho de erro do `int 0x13` (carry) imprime **falha nomeada** (nunca hang silencioso). Ainda sem long mode.
- **B-3b-2 (A20 + long mode):** habilitar A20 (`int 0x15, ax=0x2401`), montar uma GDT flat, setar `CR0.PE`, far-jump para um stub 32-bit, setar `CR4.PAE`, carregar PML4/PDPT/PD de 2 MiB (identidade do primeiro GiB), setar `CR0.PG` + `EFER.LME`, far-jump para 64-bit.
- **B-3b-3 (rodar o payload Kof):** o `_start` 64-bit freestanding é ligado numa base fixa (ex.: `0x100000`); o boot copia o staging para lá em protected mode e salta. O perfil BIOS ganha um corpo `kof_plat_*` (ramo do `RuntimePlat`: write = `out` COM1 0x3F8, exit = `cli;hlt`, sync no-op) para o `main` Kof rodar bare — E2E imprime o hello Kof. Restrição (medida no B-3a): o programa 64-bit é emitido mas inalcançável hoje; o B-3b exige um layout de payload BIOS (base de carga) distinto do MBR.
