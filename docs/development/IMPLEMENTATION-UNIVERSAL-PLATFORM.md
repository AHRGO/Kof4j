...
| X10 | Domain-sensitive LSP (completion + go-to-definition in packages) | 🟡 | docs→platform lane (192.168.100.15) | slices 1–2 ✅ 18/09: `StdCatalog` = membros reais de **18 namespaces** no completion do `kof lsp` (7 do dispatch KofStd + time/http/db/cache/process + os 6 de segurança) — single-source travado por teste contra os case-literals da fonte (`StdCatalogTest` 6/6, `LspServerTest` 24/24; `48633d98`, `e79a3ea0`); fatia 3 = receiver-typed restantes (json, observability, validation, tetris, ui?); go-to-def em packages = fatia 4 |
