...
| 2.2 | `kof.shell` — idiomatic shell over `kof.process` | 🟡 | `.18` | Stage 1; **`docs/shell-plan.md`** SIGNED-OFF 18/09 (Q1–Q3 by maintainer poll); MVP 18/09: `cmd`/`run`/`ok` on JVM+JS byte-parity, `pipeline` JVM-only (`PROC001` on JS/Native, inherited from `process.spawn`); glob/`~`/redir out of v1; 2.2.4 stdlib doc DONE 19/09 (`docs/stdlib/shell.md` +PT); **2.2.3 runWith ✅ 19/09** (`runWith(argv, cwd, env)` JVM+JS, additive env, honest `-1` failures, `ShellE2ETest` 15/15 byte-parity + Native pin); **platform prereq ✅ 19/09: JS face of `process.spawn` landed** (§355 routing bug fixed at root — handle ops never ran on ANY target; `ProcessSpawnE2ETest` 4/4 byte-parity JVM==JS); residual = `pipeline`-JS chain binding ALONE (pump threads in `KofJsProcessBridge`; public `write` stays `/dev/null`-gated — rule 6), then row flips ✅ |
