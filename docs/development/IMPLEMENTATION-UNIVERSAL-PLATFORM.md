...
| 3.2 | `infra "prod" { ... }` — desugar over records (compile-time codegen) | 🟡 | `.18`/9093 | **DECIDED 21/09 (`D-MAKEALIVE-SYNTAX`)** — pure syntactic sugar over `design()`: `infra` stays an IDENTIFIER (dispatched like `test`/`application`, not a reserved word), the block lowers to `design(): Infrastructure` and gets no semantics of its own (no HCL). **IN IMPLEMENTATION** (owner `.18`/9093) — proof `InfraSyntaxE2ETest` (hand-written `design()` twin → byte-identical plan/apply, JVM==JS). R4 ✅ (`CodegenStep` hook) removed the codegen blocker. |
...
| 3.7 | Cycle detection in the `infra` graph at compile-time | 🔵 | `.18`/9093 | **R4 ✅ landed 21/09**; **UNBLOCKED by `D-MAKEALIVE-SYNTAX` (21/09)** — the compile-time graph follows the 3.2 surface (owner `.18`/9093); until it lands, the runtime refusal shipped in 3.1 is the contract. |
...
below).
Otherwise the next R3 work is **implementing the decided 3.3/3.5** (`D-R3-3.3`/`D-R3-3.5`, 21/09) plus the in-flight 3.8 (D6 decided 20/09; 3.8b fatias 1–4 + D6-5 landed 20–21/09 — remaining = JS struct bridge/Native sret); the Native scalar ABI (3.7) landed 20/09 and §370/#549 (extern argument conversion) closed the same day.
