...
| 3.6 | Secrets via `kof.security` | 🟡 | security lane | `kof.security` exists; `Secret`/`KeyHandle` pending (Stage 5). **DECIDED 21/09 (`D-SECRETS`)**: plan promoted to `docs/development/secrets-plan.md` (EN+PT); **face 1 (`Secret` type) authorized**, incremental with proof; `KeyHandle`/redaction face by face. |
...
| R4 | Formalize compile-time codegen (`CodegenStep`) | ✅ landed 21/09 | `CodegenStep`/`CodegenStepPipeline` (additive; empty registry = identity, zero behavior change; `CodegenStepPipelineTest` 6/6); unblocks `infra "prod" {}` (3.2, surface still ⛔ rule 6); the **DDL/runner migration (2.2.3) assessed 21/09 = phase mismatch → DECIDED 21/09 option B (`D-DESUGAR-STEP`): AST-phase `DesugarStep` registry (behavior-free); DDL stays in lowering** |
