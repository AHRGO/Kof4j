...
| X10 | Domain-sensitive LSP (completion + go-to-definition in packages) | 🟡 | docs→platform lane (192.168.100.15) | slice 1 ✅ 18/09: `StdCatalog` (membros reais dos 7 namespaces do dispatch KofStd, single-sourced por teste contra os case-literals dos typers) + member-completion no `kof lsp` (`rng.` → os 5 do typer; `StdCatalogTest` 3/3, `LspServerTest` 23/23; `8a2c26b7`); slice 2 = namespaces de dispatch próprio (time/json/http/db/...); go-to-def em packages = fatia 3 |
