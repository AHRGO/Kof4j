...
| 3 | INFRASTRUCTURE (Kof Makealive) | 🟡 planned 19/09 — `makealive-plan.md` (.18); 3.0.0 recon-pending, **surface ✅ decided (Q1–Q4, `DECISIONS.md` §D-MAKEALIVE 20/09)** | Stage 2, R3 (FFI), R4 (codegen hook — **✅ decided 21/09, `D-CODEGEN-STEP`**); **name collision R1 ✅ resolved (`kof.makealive`, plan §2.1/Q1)** |
...
**R3 🟡 · R4 🟡 (decided 21/09, `D-CODEGEN-STEP`) · R5 🟡 · R9 🟡 · R10 🔵 · R11 🟡**
...
| 3.2 | `infra "prod" { ... }` — desugar over records (compile-time codegen) | 🔵 | — | **R4 ✅ decided 21/09 (`D-CODEGEN-STEP`, internal hook)**; the user-facing `infra "prod"` surface is still ⛔ rule 6 (out of v1, plan §5) |
...
| X5 | Variance / sealed types | 🟡 OPEN (spec-first) | compiler/maintainer | **decided 21/09 (`D-TYPE-VARIANCE` = option C): OPEN**; spec/plan drafted before any parser/typer diff (rule 6); type-classes stay rejected |
| X6 | Interop reflection (restricted to interop) | 🟡 OPEN (spec-first) | compiler/maintainer | **decided 21/09 (`D-INTEROP-REFLECT`): OPEN, incremental plan required**; interop boundary only, never a foundation (rule 6) |
...
below).
Otherwise the next R3 work is **implementing the decided 3.3/3.5** (`D-R3-3.3`/`D-R3-3.5`, 21/09) plus the in-flight 3.8 (D6 decided 20/09; 3.8b slice 1 landed); the Native scalar ABI (3.7) landed 20/09 and §370/#549 (extern argument conversion) closed the same day.
...
**R4 (codegen hook)** — **✅ decided 21/09 (`D-CODEGEN-STEP`), implement internal hook** — gates Stage 3 (`infra`). Within Stage 1, the remaining
