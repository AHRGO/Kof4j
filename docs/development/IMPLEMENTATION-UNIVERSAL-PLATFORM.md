...
| 3.2 | `infra "prod" { ... }` — desugar over records (compile-time codegen) | 🔵 | — | **R4 ✅ landed 21/09 (`CodegenStep` hook)**; the user-facing `infra "prod"` surface is still ⛔ rule 6 (out of v1, plan §5) |
...
| 3.3 | JVM: opaque handles / out-buffers (void*, T*, Array<Byte> as buffer) — pointer-to-opaque / byte-buffer type, NOT the full D6 struct ABI | 🟡 DECIDED 21/09 | maintainer | **`D-R3-3.3` = option A**: nominal opaque `Handle` (non-arithmetic) + `Buffer(U8, INOUT)` (== D6-3); implement |
...
below).
Otherwise the next R3 work is **implementing the decided 3.3/3.5** (`D-R3-3.3`/`D-R3-3.5`, 21/09) plus the in-flight 3.8 (D6 decided 20/09; 3.8b fatias 1–3 + D6-5 landed 20–21/09 — remaining = D6-3 out-buffer/JS/Native); the Native scalar ABI (3.7) landed 20/09 and §370/#549 (extern argument conversion) closed the same day.
