    ...
    **Slices (each with qemu proof on riscv64 + aarch64):**
    ...
    5. **`orm.find`/`all`/`where`/`where_op`/`page`** — row materialisation.
       ...
       **Slice 5c (`where`/`where_op`) DONE 24/09** as `B80` +
       ...
       list). The last face (`page`) reuses the same reader — slice 5d.
