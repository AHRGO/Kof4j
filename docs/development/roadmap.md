...
**Queue status (18/09):** PR **#438** (external fork, `fix/278-d-null-intent-atomic`) attacks **2.6.1/N1** (JVM+Script+JS) under §D-NULL-INTENT "Scope protection" — NOT draft, pending maintainer review; lanes must not open a parallel N1 front (rule 6 / one contract, one PR; CI `Build + Tests` **FAILURE at head `7dc08ccb`** — 4 reds: 3× `ConformanceMatrixTest` (cells still pin the old `null→default` fold) + 1× `KofMapSetTest.mapSetNative` (N2/Native out of the PR's N1 scope — honest split, but native went red anyway)). It also declares `Map.get/put/remove` on absent keys (I7) — subsuming the parked `#376`/`#409` map-absent face stamped by the maintainer 18/09 — do NOT open a separate N4 front for those cells while #438 is under review.
