...
| 7 | Nothing pending in bugs-and-gaps | `check_known_bugs_status.sh` live set empty + `specification-gaps.md` 0 open | RED (22 live at the tip — snapshot 21/09 after §421 S0 fix + §423–§431 catalogued + §431 closed by lane `.18`; the script is the authority; an unreadable ledger is `UNKNOWN`, never GREEN) |
