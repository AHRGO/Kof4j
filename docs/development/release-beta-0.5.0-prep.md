...
| 7 | Nothing pending in bugs-and-gaps | `check_known_bugs_status.sh` live set empty + `specification-gaps.md` 0 open | RED (16 live at the tip — snapshot 21/09 after §421 S0 fix + §423–§431 catalogued, §431 closed by lane `.18`, §429 fixed (LSP `-32601`), §435 closed by lane `.18` (check_500 `LspServer` 601→582, split into `LspJsonRpc`), §432 fixed (JVM `getOrDefault` VerifyError) and §428 fixed (DAP unimplemented requests now honest-fail) and §425 fixed (riscv64/aarch64 `kof.config` now honest `CONF001`) and §426 fixed (`time.collect()` on JS now `TIME004`); the script is the authority; an unreadable ledger is `UNKNOWN`, never GREEN) |
