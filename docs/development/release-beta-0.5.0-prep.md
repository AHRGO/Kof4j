...
|---|---|---|---|
| 1 | 100% parity between targets | per-target matrix + golden byte parity where the contract requires; divergence = bug or diagnosed `XXX00x`. **Auto-measured** by `check_release_050_gate.sh` (runs `scripts/target-matrix.sh`, EG-5, and reads its `PARITY: 100%` line) | GREEN (measured 21/09: `PARITY: 100%` on jvm/x86-64/riscv64/aarch64/JS/Script vs the JVM oracle; the tree jar was rebuilt with `scripts/build-kof-jar.sh`, which also stamps it so a rebase no longer fakes "stale"; on a rootless host the cross toolchain (binutils/qemu/libc) is set up by `scripts/setup-cross-toolchain.sh`, which extracts the `.deb`s to a local prefix and points `KOF_CROSS_SYSROOT` at it). **Re-measured 21/09 later the same day: NEEDS-MEASURE again** — the subsequent landings (makealive 3.2 `966c86a4`, §422 `9d97b268`, 3.2 tracker sync `414bc7a0`, …) re-staled `lib/kof.jar` vs source (`chain-check` names the newest source; e.g. `KofCliSupport.java`). This is the condition's own rule (re-instated by `scripts/build-kof-jar.sh` + `scripts/target-matrix.sh` at cut, condition 6), **not a parity regression** |
...
| 4 | Total stability | full suite 0F/0E + 5/5 conformance matrix on the candidate. **Auto-measured** from a real suite log via `scripts/stability-report.sh` (`KOF_SUITE_LOG=…`); GREEN requires the `TOTAL` to be 0F/0E **and** the log **stamped** (`SUITE-SHA` == tip, `SUITE-DIRTY=0`) — a log from another commit or from a dirty tree is `unknown`, never a false GREEN | MEASURED GREEN on `8f459b8e` 21/09 ~06:0x (lane .18): first self-certifying `safe-suite.sh` log — `TOTAL: tests=3355 failures=0 errors=0 skipped=223`, `SUITE-SHA`==tip, `SUITE-DIRTY=0`, and `stability-report.sh` returned GREEN from it (mechanism bug found and fixed the same round: the old `safe-suite.sh` printed `TOTAL` to the console only and never appended it to the log, so NO stamped run could ever be certified). The candidate must still be re-measured on the final clean tip at cut time — that is condition 6's own rule, not this row's doubt. **Fresh full-suite run 21/09 at `96af9b63`** (first since the mechanism fix): `TOTAL: tests=3413 failures=1 errors=0 skipped=223` — **RED by ONE stale test**, `CompilerDriverTest.externProducesHonestGapNotSilentDrop`, which still demands `FFI001` for an `Int[]` extern that `7c6413d4` (D6-2) intentionally made bindable on the JVM (proven by `FfiArrayE2ETest` 5/5; `String[]`/`List<T>` remain `FFI001`). Hand-off to the FFI front (`jonas`): repoint the assertion to a signature that is still unbound, do not delete it. **RESOLVED 21/09 by the FFI lane (§422, `9d97b268`):** the assertion now targets genuinely-unbound signatures (`String[]`/`List<Int>`→`FFI001`, `Buffer(Int)`→`SEM096`) and a new pin was added; `CompilerDriverTest` 259/0F/0E — the stale red is gone. The candidate still needs its own stamped re-measure at cut time (condition 6's rule), not this row's. |
...
| 6 | All edges closed | the FULL EG queue (EG-1..EG-10) closed + open `1.0-blocks` = 0; the 0.5.0 waits until each owner closes/moves their own work. **EG-5/EG-9/EG-10 mechanisms DONE 20/09**; open: EG-8 | RED (EG-8; an unreadable EG table is `UNKNOWN`, never GREEN; a failed `1.0-blocks` enumeration leaves that part `UNKNOWN`, never an implied 0 — `R050_OPEN_BLOCKS` can supply the count offline) |
| 7 | Nothing pending in bugs-and-gaps | `check_known_bugs_status.sh` live set empty + `specification-gaps.md` 0 open | RED (18 live at the tip; an unreadable ledger is `UNKNOWN`, never GREEN) |
