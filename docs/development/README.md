...
| 9 | `workflow-plan.md` (+PT) — `kof.workflow` (TIER 2.1, Stage 2) | `LANDED` (poll 19/09; owner `.18`): 2.1.0 recon + 2.1.2 MVP + 2.1.3 retry/deadLetter/schedule/checkpoint todos landed 19/09 (WorkflowE2ETest 20/20; suíte 4 módulos verde; commits `7db91735`→`285e23ef`) — rule-6 residual CLOSED 19/09: the maintainer answered the question ("pode ir pra fatia 3") and the 5th face `runSupervised` (one_for_one supervisor integration) landed in slice 3b | entrou por promoção explícita (regra dos 3 estados: front aberta não é mais "future"); antes era a linha crua 🔵 do tracker 2.1 | 2.1.0 recon da `.18` (medir lexer/parser/stdlib p/ `job`/`dag`/`after`/`run`/`Report`); esta lane só moveu e classificou — conteúdo intocado |
