...
| 4 | `planning-otp-supervision.md` (#83) | `IN PROGRESS` — 1st slice ✅ 11/09 (core+`restartLimit`+`stop`) + **S2-JVM ✅ 13/09** (`startAll`/`lacoUnico` + identity wrapper) + **S2-Native x86 ✅ 15/09** (§129 closed — TLS per-thread chain + per-worker handler; `KofSupervisorE2ETest` 16/16 incl. `supervisorNativeParityX86`/`supervisorNativeS2ParityX86`; riscv/aarch=OTP001) + **S2-JS ✅ 18/09** (§132 closed — cooperative async `time.sleep`; `OTP002` lifted; `supervisorJsParity`/`supervisorJsS2Parity`) | **DD-OTP RATIFIED 13/09** (option 1a: S2 JVM; riscv/aarch PARTIAL) — see §3 | riscv/aarch (raw `clone`, no TLS) remains an honest gate (`OTP001`); JS runs the core to parity since 18/09 |
...
| DD-OTP (remaining) | `planning-otp-supervision.md` | ✅ RATIFIED 13/09 (option 1a: wrapper `(id, result)`; riscv/aarch PARTIAL) — **S2-JVM ✅ IMPLEMENTED 13/09** (`Supervisor.startAll`/single selectAny loop) and **S2-Native x86 ✅ IMPLEMENTED 15/09** (§129 closed, DECISIONS §2 option B) and **S2-JS ✅ IMPLEMENTED 18/09** (§132 closed — cooperative async `time.sleep`, `OTP002` lifted); riscv/aarch (`OTP001`, raw `clone`) remains |
...
| `planning-otp-supervision.md` | 1st slice ✅ JVM+Script; **S2-JVM ✅ 13/09** (`startAll`/`lacoUnico`); **S2-Native x86 ✅ 15/09** (§129 closed — per-thread handler chain); **S2-JS ✅ 18/09** (§132 closed — cooperative async sleep; `OTP002` lifted) | riscv/aarch (`OTP001`) remains an honest gate; JS runs to parity |
