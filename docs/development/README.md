...
**22 items in the open queue** (resynced 21/09 — latest 23→22 when **§429** (LSP unknown request now answers `-32601`, lane docs/plataforma) was FIXED ✅ 21/09 (`LspServerTest` 38/38); 20→19 when
...
| ~~`known-bugs.md`~~ → `docs/bugs-and-gaps/` | **22 live** (live count — authority is `scripts/check_known_bugs_status.sh`; resynced 21/09 — snapshot after §421 S0 fix + §423–§431 catalogued + §431 closed by lane `.18` and §429 fixed (LSP `-32601`); 18→16 when §192 and §358 were closed by the nat orphan sweep 21/09; 16→15 when §258 was closed 21/09 (#775 `NumericFormatterE2ETest` + #776 `KofHttp.supportedOn`); 15→16 when §423 (cross channels, NAT005) was catalogued 21/09; was 20 — §380 `9f383bcf`, §381 `576a1dcb`, §394 `d0464385` and §353 21/09 closed; §400/§418/§421 catalogued/re-published; the §2 live set is the authority; the historical 14/09 count was 32; §81/§163/§127-JVM, §155, §94, §157-160 and §65 closed/DOES-NOT-REPRODUCE 13/09) | live queue |
