...
| `pow`/`-lm`, `roundTo`-mode | `docs/stdlib/PLAN-STDLIB-EXPANSION.md` | ✅ `pow` **DONE 13/09** (7a: `-lm`; 5 targets MATH001 cross; `stdmathpow` matrix + `powCrossArchRefused` `d736e36e`) · `roundTo` **NOT approved by 7a** (ratification = pow only; "+roundTo" was an agent note in the plan — undefined surface/signature = rule 6, awaits maintainer decision) |
