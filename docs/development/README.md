...
| 4 | `planning-otp-supervision.md` (#83) | `EM CURSO` — 1ª fatia ✅ 11/09 (núcleo+`restartLimit`+`stop` testados JVM+Script; `KofSupervisorE2ETest` 6/6; Native=OTP001 §129, JS=OTP002 §132) | **DD-OTP RATIFICADAS 13/09** (opção 1a: S2 JVM + wrapper `(id, resultado)`; riscv/aarch PARTIAL) — ver §3 | fatia S2 (selectAny N-workers) **liberada p/ implementar na JVM** (design do wrapper de identidade fechado na ratificação; §128 ✅ 12/09); promover OTP001/002 só com a face cross decidida |
