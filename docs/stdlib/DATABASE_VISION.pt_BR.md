...
- **Native x86-64:** real (`kof_orm_*` em asm sobre o `kof_db_*` nativo; SQLite + MySQL wire). **Native riscv64/aarch64:** ORM F1a+F1c+F1d+F3a+F2c3+F2a reais desde 22-23/09 (`deleteAll`/`count`/`create`/`migrate`/`count` com filtro/`delete`/`save` sobre SQLite, peças `RtB50`/`RtB51`/`RtB52`/`RtB53`/`RtB54`/`RtB55`+`RtB55Helpers`; §447 literal Bool boxeia `kof_box_bool` no Native); o restante reporta `ORM001` (gap documentado em compile-time). **JS:** FECHADO
