...
> (DB001 fechado: riscv/aarch 15/09 + JS 16/09; ORM001 fechado no JS 18/09); ORM **F1a+F1c+F1d+F3a+F2c3+F2a+F2b reais desde 22-23/09 no Native cross riscv64/aarch64** (`deleteAll`/`count`/`create`/`migrate`/`count` com filtro/`delete`/`save`/`saveAll`/`find` sobre SQLite, peças `RtB50`/`RtB51`/`RtB52`/`RtB53`/`RtB54`/`RtB55`+`RtB55Helpers`/`RtB56`/`RtB57`+`RtB57Helpers` + `NativeRiscvOrmCtors` por-programa (o resolver `kof_orm_ctors`); §447 corrigido: literal Bool boxeia `kof_box_bool` no Native; §449 corrigido: resolver x86 multi-entidade + leitura `Double`), com `ORM001` nas faces cross restantes (`all`/`where`/`page`; ORM x86-64 real desde 22/09: 13/13 faces sobre SQLite + MySQL wire, F2d1–F2d7) tracked em `docs/backend-parity.md`,
...
- **Native x86-64:** real (`kof_orm_*` em asm sobre o `kof_db_*` nativo; SQLite + MySQL wire). **Native riscv64/aarch64:** ORM F1a+F1c+F1d+F3a+F2c3+F2a+F2b reais desde 22-23/09 (`deleteAll`/`count`/`create`/`migrate`/`count` com filtro/`delete`/`save`/`saveAll`/`find` sobre SQLite, peças `RtB50`/`RtB51`/`RtB52`/`RtB53`/`RtB54`/`RtB55`+`RtB55Helpers`/`RtB56`/`RtB57`+`RtB57Helpers` + `NativeRiscvOrmCtors` por-programa; §447 literal Bool boxeia `kof_box_bool` no Native; §449 resolver x86 multi-entidade + leitura `Double`); o restante (`all`/`where`/`page`) reporta `ORM001` (gap documentado em compile-time). **JS:** FECHADO
