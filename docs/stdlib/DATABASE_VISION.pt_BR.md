...
> (DB001 fechado: riscv/aarch 15/09 + JS 16/09; ORM001 fechado no JS 18/09); ORM **F1a real desde 22/09 no Native cross riscv64/aarch64** (`deleteAll`/`count` sobre SQLite, peça `RtB50`), com `ORM001` nas faces cross restantes (ORM x86-64 real desde 22/09: 13/13 faces sobre SQLite + MySQL wire, F2d1–F2d7) tracked em `docs/backend-parity.md`,
...
- **Native x86-64:** real (`kof_orm_*` em asm sobre o `kof_db_*` nativo; SQLite + MySQL wire). **Native riscv64/aarch64:** ORM F1a real desde 22/09 (`deleteAll`/`count` sobre SQLite, peça `RtB50`); o restante reporta `ORM001` (gap documentado em compile-time). **JS:** FECHADO
