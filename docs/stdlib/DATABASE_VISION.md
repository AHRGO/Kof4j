...
> (DB001 closed: riscv/aarch 15/09 + JS 16/09; ORM001 closed on JS 18/09); ORM **F1a+F1d real since 22/09 on Native cross riscv64/aarch64** (`deleteAll`/`count`/`create` over SQLite, pieces `RtB50`/`RtB51`), with `ORM001` for the remaining cross faces (x86-64 ORM real since 22/09: 13/13 faces over SQLite + MySQL wire, F2d1–F2d7) tracked in `docs/backend-parity.md`,
...
- **Native x86-64:** real (`kof_orm_*` asm over the native `kof_db_*`; SQLite + MySQL wire). **Native riscv64/aarch64:** ORM F1a+F1d real since 22/09 (`deleteAll`/`count`/`create` over SQLite, pieces `RtB50`/`RtB51`); the rest reports `ORM001` (gap documented at compile-time). **JS:** CLOSED
