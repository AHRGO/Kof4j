...
| `kof.observability` | ✅ | `observability.health/readiness/liveness`, `counter`/`increment`/`gauge`/`histogram`/`metrics()`, `requestId()`/`correlationId()`/`traceId()`/`spanId()`/`spanStart`/`spanEnd` — JVM/Native/JS (`KofObservabilityTest` 7/7; `OBS002` closed 16/09) |
