...
| `kof.web` | ✅ | `web.app()`, rotas, middleware `app.use`, `listenSecure(port)` TLS, `status(code[, body])`/`headerSet`, `app.ws` (WebSocket RFC 6455) + `app.sse` (SSE) — JVM (Native `WEB001/002`; base JS ✅ 16/09, residual `app.ws`/`app.sse` = `WEB004`/`WEB003` compile-time) |
...
| `kof.scheduler` | ✅ | `scheduler.every(n, fn)`/`at(cron, fn)`/`cancel(id)` — JVM (ScheduledExecutor) + JS (setInterval) + Native (`SCHED001` fechado 31/08 — thread por job, cross 05/09) |
