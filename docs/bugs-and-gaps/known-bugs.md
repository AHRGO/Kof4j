...
### §185 — Script/interpretador: escrita em elemento de `Char[]` LANÇA `argument type mismatch` (crash) — JVM/Native/JS corretos — ❌ ABERTO 13/09 (achado na caça Q4 da lane bugs-and-gaps `192.168.100.15`; fix = lane `interp`, 9093)

- **Sintoma (medido 13/09, 4 targets):** o interpretador (target Script)
  **aborta** ao gravar num `Char[]`:
  ```kof
  main() {
      var c = new Char[2]
  ...
  ```
  JVM/Native/JS imprimem `65`; Script `exit=1`, `stderr="argument type
  mismatch"`, sem output. A leitura de um `Char[]` recém-alocado funciona
  (devolve `0`); o crash é só no **store**.
- **Menor repro:** o bloco acima (ou `c[0] = 65 as Char`, mesmo crash).
- **Causa raiz:** `KofInterpreterOps.arrayStore` (l.244-251) só trata
  `bool`; para `Char` cai no `Array.set(arr, idx, v)` genérico, e o valor Kof
  (`Char` = String/`Character`?) não é um `Character` — `java.lang.reflect.
  Array.set(char[], …)` lança `IllegalArgumentException: argument type
  mismatch`. O `newArray` (l.193-202) aloca `char[]` de verdade (correto); o
  que falta é a coerção do valor no store, como já existe p/ `bool`.
- **Impacto:** qualquer Kof com array de `Char` e atribuição de elemento roda
  nos 3 backends compilados e **derruba** o interpretador. Também quebra a
  paridade interpretador × JVM (gate do `KofScriptTest`/`KofInterpreterParityTest`).
- **Fix proposto (lane interp):** em `KofInterpreterOps.arrayStore`, tratar
  `char` como o `bool`: extrair o code unit do valor (se String de 1 char →
  `charAt(0)`; se número → `(char) intValue()`) e `Array.set(arr, idx, ch)`.
  Aproveitar e cobrir `Byte`/`Short` no store (hoje dependem do lowering).
- **Provas a adicionar:** `KofInterpreterParityTest.charArrayStore` (novo) +
  célula de matriz `chararr` (4 targets).
