...
## §168 — SEM025 ausente para método desconhecido em namespace `json`

> **ABERTO 13/09 (descoberto no gate WEB001-T1; dono = lane bugs-and-gaps)**

- **Sintoma:** `main() { println(json.metodoRuim()) }` compila com sucesso;
  `SemanticResolutionTest.wrongArityOnJsonNamespace` (célula J4) espera
  SEM025 "on namespace 'json'".
- **Causa raiz:** o handler semântico do `json` (`61495f69`, #126) valida
  **aridade** de `encode`/`decode`, mas não tem allow-list de métodos —
  método inexistente no namespace resolve no caminho genérico.
- **Menor repro:** `main() { println(json.metodoRuim()) }` → `success=true`
  (esperado `false` + SEM025). Confirmado standalone, classpath fresh.
- **Pré-existente:** provado no HEAD limpo (`553b3326`/`b17663d7`) com
  stash da lane development — **não é regressão do WEB001-T1** (que só
  toca o caminho `web.app()` p/ JS).
- **Fix proposto:** allow-list (encode/decode) no handler do namespace
  `json` no `SemanticAnalyzer`; SEM025 para o resto.
