...
| double println shortest-repr + scientific | `0.30000000000000004` / `1.0E7` / `1.0E-5` / `33.333333333333336` / `0.33333334` / `1.0E20` / `NaN` / `0.001` / `1.0E-4` / `3.4028235E38` / `-0.0` | DONE | DONE (bug §180 ✅ 14/09 x86_64 — `kof_dtoa` loop `%.*e`+`strtod` for the shortest round-trip + Java reformat; Float keeps its own form; `snprintf`/`strtod` need libc, so riscv/aarch stay FLT001) | DONE | PARTIAL (doc: `Number.toString` does not emit `.0`/scientific at the JDK threshold) | `doubleprint` |
