...
| `Double %` (mod of variables; + NaN/±Inf) | `1.5` / `1.0` / `0.5` / `-1.5` / `NaN` | DONE | DONE (bug 146 ✅ 12/09 `718ae5cf` — `NativeX86Arith` emits the real fmod) | DONE | DONE (§264 16/09 — was partial: JS `String(1.0)`=`1`; now `kofNumFmt` formats the JDK contract) | `doublemod` |
| cast `d as Int` / `L as Int` / `66 as Char` | `9` / `70000` / `B` | DONE | DONE | DONE | DONE | `cast` |
| float println | `0.3333333333333333` / `5.0` / `3.5` | DONE | DONE (bug 44 ✅ 10/09 x86) | DONE | DONE (§264 16/09 — was partial: JS `String(5.0)`=`5`; now `kofNumFmt` JDK contract) | `floatprint` |
| double println shortest-repr + scientific | `0.30000000000000004` / `1.0E7` / `1.0E-5` / `33.333333333333336` / `0.33333334` / `1.0E20` / `NaN` / `0.001` / `1.0E-4` / `3.4028235E38` / `-0.0` | DONE | DONE (bug §180 ✅ 14/09 x86_64 — `kof_dtoa` loop `%.*e`+`strtod` for the shortest round-trip + Java reformat; Float keeps its own form; **FLT001 CLOSED on riscv/aarch 15/09** — the same `kof_dtoa` ported as slice `RtB45` consumes libc `snprintf`/`strtod` via the on-demand dynamic link, `NativeRiscvDtoaTest` oracle both arches) | DONE | DONE (§264 16/09 — was partial: `Number.toString` does not emit `.0`/scientific at the JDK threshold; now `kofNumFmt` implements the contract) | `doubleprint` |
| infinity/NaN println+String.valueOf | `Infinity` / `-Infinity` / `NaN` | DONE | DONE (bug 44 residual ✅ 11/09 x86) | DONE | DONE (§264 16/09 — was partial: `5` vs `5.0`) | `infinityprint` |
...
| string unicode length/charAt | `4` / `é` / `café!` | DONE (D-PRINT 15/09: charAt prints the character) | DONE (bug 43 ✅ 10/09 x86) | DONE | DONE | `unicode` |
| string unicode astral (surrogate pair) | `4` / `55357` / `56832` / `98` | DONE (D-PRINT 15/09: explicit `as Int` for the code unit) | DONE (bug 43 ✅ 10/09 x86) | DONE | DONE | `unicode-astral` |
...
| store into `Char[]`/`Bool[]` element — §185 | `A` / `B` / `true` / `false` | DONE (D-PRINT 15/09: char prints the character) | DONE | PARTIAL (bug §185 — interpreter: `coerceFor` returns `Integer`; `Array.set(char[]/boolean[],…)` throws `argument type mismatch`, exit 1) | DONE | `chararr` |
...
| constructor-arity/type at call site (implicit `Class(args)`) — §362 | `SEM023 no call-site` | DONE (`57a0d5f0`) | DONE | N/D | DONE | `ConstructorPhantomE2ETest` (7/7) |
...
| stdlib kof.uuid (S3b.2: v7 — RFC 9562 time-ordered, NO matrix case; riscv/aarch slice B25b) | shape `xxxxxxxx-xxxx-7xxx-[89ab]xxx-xxxxxxxxxxxx` | ✅ assert | ✅ assert | ✅ | ✅ assert | `KofUuidTest` (v7) |
...
- **`android`** — it is not an execution backend: it is **packaging of the
  ...
  semantics. Environment requirement: `ANDROID_HOME` + build-tools (>= 35 to read Java-21 bytecode; the CLI picks the highest installed) — without the
