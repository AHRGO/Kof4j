...
| double println shortest-repr + scientific | `0.30000000000000004` / `1.0E7` / `1.0E-5` / `33.333333333333336` / `0.33333334` / `1.0E20` / `NaN` | DONE | PARTIAL (bug §180 — `%.16g` truncates to `0.3`/`10000000.0`/`1e-05`; Float becomes double expansion `0.3333333432674408`; libm `-nan` stays `-nan`) | DONE | PARTIAL (doc: `Number.toString` does not emit `.0`/scientific at the JDK threshold) | `doubleprint` |
...
| stdlib kof.math (S13b: `parse*OrDefault` — briefing §43: parse failure RETURNS the default, never throws; backends = JVM try/catch, JS wrapper, x86 wrapper w/ handler in exc_chain, riscv B41 + aarch translator; Int literal in Long param proves I2L widening of KofStd; `""` line of Double outside the golden = §169) | `42` / `-1` / `7` / `15` / `3` / `9007199254740993` / `-5` / `8` / `true` ×3 | DONE | DONE | DONE | DONE | `stdmathparseord` |
...
| stdlib kof.validation BR (S5: isCpf/isCnpj/isCep/isPis — arithmetic weights, mod-11 by subtraction; + `==true`/`==false` §93) | `true` / `false` / `true` / `false` / `true` / `false` / `true` / `false` / `true` / `true` | DONE | DONE | DONE | DONE | `stdvalidation` |
