...
### §272 — spans do `kof.observability`: JVM/JS descartavam o nome e o caminho Native diverge (x86 emite `durationMs`/sem nome; riscv64/aarch64 são stubs constantes) — 🟡 face (c) ABERTA (achado 17/09 pela lane bugs-and-gaps `192.168.100.15` ao fechar o `OBS003`; faces (a)+(b) CORRIGIDAS — (b) 18/09 lane nat `192.168.100.17`)
