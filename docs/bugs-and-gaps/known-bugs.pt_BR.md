...
- **Status:** 🔴 ABERTO — catalogado 15/09 pela lane bugs-and-gaps `192.168.100.15`. O #252 foi fechado pela lane do fixer; ESTA linha permanece: os 14 vermelhos do compiler + o retorno `Int?` que quebra o load + o gap de paridade de 3 targets continuam no tip e sem tratamento. Independente do §240 (commit diferente, raio diferente).
...
- **Status:** 🟡 RESIDUAL — catalogado 15/09 pela lane bugs-and-gaps `192.168.100.15`. O #256 fica FECHADO (o repro reportado está corrigido); isto é uma aresta mais estreita não coberta na mesma checagem. Também anotado na issue #256. Não bloqueia o portão de release (raro; nenhum teste vermelho).
