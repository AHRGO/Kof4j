...
**Ponteiro para o dono:** o contrato do teste é cancel cooperativo sobre sleeps de wall-clock; sob fome de scheduler, o `curto()` observa a flag de cancel após a janela e reporta `999` — ou margem por rodada atrelada a progresso (não wall clock) ou assumedTrue sob detecção de carga; NÃO relaxar a asserção do §117 em si (Q5). Repro: martelar o método único enquanto um build nativo roda em paralelo.
