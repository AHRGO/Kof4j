...
## §344 — gate do tip vermelho: `JsCallEmitter` 597→608 cruzou a linha CRÍTICA ≥600 (`check_500` quebra a CI) — `09e40afb` somou +11 linhas sem split — 🔴 ABERTA, rota: lane JS (`.15`/KofJS)
...
## §343 — suite vermelha no tip (o bug e de codegen; a face-gate do mesmo culpado e o §344): `09e40afb` quebrou o codegen JS de switch-case — `BackendParityTest.parityArrayAndSwitch` ICE `COMP002: unexpected op in switch case: KofConditionalJump[comparison=NE...]` — 🔴 ABERTA, rota: lane JS (`.15`/KofJS)
...
- **Leitura (hipótese para a dona, NAO diagnostico):** a recuperação reapendeu os branches de emissão perdidos; um `KofConditionalJump` como valor de case (comparação booleana/NE alimentando `switch`) agora chega no dispatch de case do emitter onde o caminho pré-recuperação esperava só operandos de table/lookupswitch — o op perdido colide com o lowering de switch-case. Mesma família de arquivo dos +11 linhas do §344 (mesmo commit culpado, duas faces: §344 = gate, §343 = codegen).
- **Regra interina (precedente §307):** outras lanes podem passar com esta ÚNICA falha documentada declarada na mensagem; ninguém fora da lane JS edita o emitter; o fix é de código com o teste voltando a verde (Q0 — nao editar o teste).
