...
- **Estado:** reproduz no `d2d025f4`; **RE-MEDIDO 15/09 ~18:40 pela lane
  ...
  §203/§166).
- **✅ CORRIGIDO 16/09 (lane issue-watcher `.22`, ordem da usuária "corrige as
  issues e fecha"):** o `ExpressionInstanceCallLowerer` agora roteia
  `toHexString`/`toBinaryString` de receptor primitivo (0 args) para os
  ESTÁTICOS JDK reais (`java/lang/Integer|Long`, `(I|J)Ljava/lang/String;`) —
  a mesma superfície do alias `toInt`/§89; o receptor empilhado vira o
  argumento da chamada estática. Receptores não-Int/Long
  (Byte/Short/Char/Float/Double/Bool) recebem **SEM052** honesto apontando o
  caminho da stdlib — nunca owner vazio (R6). Backend JS: o `JsCallEmitter`
  mapeia os estáticos para `toString(radix)` com máscara unsigned (`>>> 0`
  para Int, `BigInt.asUintN(64)` para Long) — os formatadores do JDK são
  unsigned de 32/64 bits, o `Number.toString` do JS mantém o sinal (achado na
  caça Q4 de paridade: `-42` dava `ffffffd6` JVM × `-2a` JS).
  Prova: `NumericFormatterE2ETest` 3/3 (paridade JVM×JS Int+Long edges:
  `255→ff`, `-42→ffffffd6`, `0→0`, Long `4294967296→100000000`,
  `-1→ffffffffffffffff`; receptor Double → SEM052), suíte
  **2197/0/0/190** (1900 compiler + 297 script+c+cli). Issue #148 fechada com
  essa prova.
