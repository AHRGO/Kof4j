...
### §284 — backends native: erasure-box (`kof_box`/`kof_unbox`) é NO-OP SILENCIOSO → primitivo-para-Object (`var o: Object = 99`, `h.o = 99`) SIGSEGVa no native (x86 + riscv64 + aarch64) — stub PRE-EXISTENTE catalogado durante a face B do §253 (18/09, lane `.17`) — ✅ CORRIGIDO 18/09 (lane nat; prova `NativeErasureBoxE2ETest` 3/3, x86 real + riscv/aarch sob qemu)
...
- **Prova:** `NativeErasureBoxE2ETest` (3/3: x86 real, riscv64 + aarch64 sob qemu — o programa cobre caixa de literal+campo+argumento-de-funcao, caminho feliz do `as Int`, golden do codigo de char "97", long 64-bit, passthrough bool/double/String e o diagnostico do CCE estrito no `catch(String e)`; RED pre-conserto = SIGSEGV 139 / saida vazia). md5 do programa combinado IDENTICO nos tres x86 ≡ riscv ≡ aarch (`4859ebe9…`), e o subconjunto de 8 linhas do println e byte-a-byte identico ao oraculo JVM (`a1e0e661…`). Suite completa verde no momento do commit.
...
## §304 — `Map.get` do Native para chave ABSENTE com valor primitivo retorna `0` silenciosamente — o contrato `V?` do D-NULL-INTENT (#438) não está implementado no backend nativo (medido, tip `52d5b4bf`) — 🟡 ABERTO (dono = lane nativa/mapset; catalogado pela lane `.22` ao fechar a #376)
