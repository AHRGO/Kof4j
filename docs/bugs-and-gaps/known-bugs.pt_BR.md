...
- **Cross-target:** nível parser, todos os backends herdam o furo. Não medido aqui.
- **Status:** 🔴 ABERTO 16/09 — catalogado pela lane docs/development com os casos medidos acima. Relacionado: `183cb048` (remoção do sugar — parcial), cluster I (docs ensinam `var`/`val`), bug 62 (tipo `val` no `parseVarDecl`).
