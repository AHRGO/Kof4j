...
## §295 — D-NULL-INTENT: operador **relacional** (`<` `>` `<=` `>=`) sobre **primitivo nulável** miscopila no JVM — (a) como **condição** de `if`/`while`: `if_icmp*` recebendo o `Integer` boxado → `VerifyError` no LOAD da classe ✅ CORRIGIDO 18/09 (lane `.18`, por diretriz da mantenedora); (b) como **valor** (`Bool b = v > 0`): `integer not assignable to reference` no store do local 🟡 ABERTO (dono `.22`, typing D-NULL-INTENT)
...
**Face (b) — caminho de valor (🟡 ABERTO, dono `.22`).** Reprodução:
```kof
main() {
    Int? v = 5
    Bool big = v > 100
...
**Status.** ✅ face (a) CORRIGIDA 18/09 (atalho de condição, `CompilerComparisons.emitComparisonShortcut`, `NullablePrimitiveRelationalConditionTest`). 🟡 face (b) ABERTA 18/09 (typing de valor/store — `Bool b = <rel Int?>` VerifyError; dono `.22`, typing D-NULL-INTENT #278/#438; repro + erro registrados acima). A célula de 4 targets `conformance-matrix.md` do §279 continua adiada até a face (b) landar e a `.22` fixar Native/Script para relacional-nulável.
