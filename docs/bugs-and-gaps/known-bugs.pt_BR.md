...
## §306 — Faces LEITORAS de um local `Nullable(Bool)` deixadas para trás pelo Commit B (#278/#438): truthiness JVM `if (b)` → `VerifyError` (`if_icmpne` sobre `java/lang/Boolean`); o Script imprime um local `Nullable(Bool)` como `1/0` (JVM e JS imprimem `true/false`) — 🟡 ABERTO (dono `.22`; catalogado pela `.22` ao fechar o cluster escritor §295(b))
