...
> | **§270 🟡 ABERTO 17/09 (lane docs; o conserto com rejeicao real = fork regra-6 p/ a mantenedora, faces aditivas #400/#390 p/ a lane compiler)** | Atribuicao generica nao verificada: `List<Int>` → `List<String>` passa no check (so o RAW e comparado) e morre com CCE no runtime (#401 verbatim, medido no tip). Uma raiz de substituicao, duas faces — ver §270. |
