...
- **ADENDO 15/09 (lane bugs-and-gaps `192.168.100.15`, classes frescas no tip
  ...
  - **Entrada canônica = §249** (a raiz geral: qualquer `<ident> <ident>` no
    ...
    só o veredito específico da #151.
...
- **✅ CORRIGIDO 15/09 (lane compiler, dono `192.168.100.22` — dono do `c0cf805e`): REVERT-para-gap-honesto.** O próprio registro oferecia dois caminhos ("terminar o contrato em todas as faces OU reverter ao gap honesto (R6)"); o **revert** foi escolhido como o correto, porque terminar o contrato boxed **colide de frente com o §125 CONGELADO** (decisão da mantenedora 12/09, opção A: `return null` de `Nullable(primitivo)` → default do primitivo, `0`/`0.0`/`false`, nunca `null` — `CompilerTypes.defaultValueOp` + `KofInterpreterParityTest.printNullablePrimitiveNull` + a célula `nullableprint`). Um retorno `Int?` verdadeiramente boxed teria que tornar `null` um valor de primeira classe em semântica → IR → 4 backends, desfazendo o §125 — decisão de contrato (regra 6), não patch de call-site. Meio-implementar isso (exatamente o que o §241 condena) é o que teria acontecido, então a mudança de ABI é retirada ao gap honesto em vez de embarcar um descritor que quebra o load.
  ...
  - **Residual / follow-up:** o fix real (`T?` de primitivo boxed com semântica de `null` nos 4 alvos) é um **projeto vertical** — **DECIDIDO 15/09 pela mantenedora (DECISIONS §7 D-NULL): ele NÃO reabre o §125 (null boxed `T?` nunca foi proibido; a proibição é *fabricar* o null literal num ponto sem null-safety). É agora uma fila de implementação legítima e destravada (completar o box nos 4 targets em lockstep).** `#252`/`#259`/`#266`-primário ficam como **gap conhecido honesto**, não como meia-ABI silenciosa, até essa fila landar. A parte (c) (null literal → param primitivo não-nullable = SEM048) é ✅ CORRIGIDA (§250).
...
### §249 — dois identificadores seguidos no início do statement são SILENCIOSAMENTE parseados como declaração tipada (`Type name`); um terceiro identificador cascateia numa nested function falsa que engole a chamada seguinte
...
- **Ponteiro (lane compiler):** `lookaheadTypedVarDecl` precisa exigir um
  declarador válido depois do nome (`=`, `:`, `;`, `,`, fim de bloco, ou
  fronteira de statement), e `lookaheadNestedFunction` não pode disparar
  quando a posição do "tipo de retorno" é ela mesma a cauda de um statement
  mal-parseado. Mesma família do veredito `is` do §219/#151 (que é a face
  visível desta raiz): `is` NÃO é operador de Kof (`Lexer.KEYWORDS` não tem
  `is`; a gramática documenta `instanceof`/`as`/`case Type v`), então
  `if (d is Dog)` → `PARSE029` está CORRETO; o bug é a aceitação silenciosa
  fora do `if`.
- **Estado:** 🔴 ABERTO — catalogado 15/09 pela lane bugs-and-gaps
  `192.168.100.15`. **Severidade: miscompilação silenciosa de Kof válido**
  (classe regra 1/6 do Congelamento), nenhum teste da suíte cobre (não é
  vermelho do portão). Face adiada: um operador `is` de verdade é feature
  nova → decisão da mantenedora (0.4.1); a face de perda silenciosa é bug
  puro e NÃO precisa dessa decisão.
