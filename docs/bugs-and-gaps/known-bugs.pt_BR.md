...
- **Direção do conserto (na fila, codegen nativo):** a sequência de escrita em box (`NativeMethodEmitter`/`KofStoreField` com receiver `CapturedVarBox` num frame de método) precisa manter o alinhamento no call-site: guardar o box-receiver num SLOT DE FRAME (`movq box, -N(%rbp)` + recarregar depois) em vez de um número ímpar de pushes na pilha de máquina através das chamadas, ou consertar o prólogo para que todo `call` caia em ≡0. Superfície pequena (um caminho do emitter), mas toca o lowering de atribuição nativa compartilhado — precisa da matriz acima como rede de regressão (Q3: os 4 verdes têm de continuar verdes, o vermelho tem de ficar verde; JVM/JS/SCRIPT intactos).
...
- **Estado:** 🟡 **Face A ✅ CORRIGIDA 16/09** (lane development `192.168.100.18`); face B segue 🔴 ABERTA (lane nat). O fix da face A landou as 4 costuras juntas (nunca commitar (a) sozinha — sem (d) vira leitura stale silenciosa): (a) `StatementAnalyzer` pré-define o nome da decl como UNKNOWN quando o inicializador contém lambda que o lê (self-capture); (b) NOVO gate compile-time **SEM092** em todo alvo `NATIVE*` (a leitura do handle capturado no job SIGSEGVa até a face B — erro em compile-time, nunca crash silencioso, R6); (c) `CompilerCaptureScanner` caso VarDeclStmt marca o self-name como capturado-mutável (força o box); (d) `CapturedVarBox.emit` guarda o box no slot ANTES de avaliar o inicializador (a lambda do init captura a caixa por referência; o `KofStoreField(value)` vem depois da avaliação). Prova: `KofTimeE2ETest.selfReferencingIntervalHandleCancelsItself` (jvm+js+script rodam, cancel real observado) + `selfReferencingIntervalHandleIsNativeGateSem092` (3 nativos, SEM092 asserido); suíte completa do kof-compiler 1901/0/0/190. O workaround (handle-sombra) segue válido.
