...
- **Status:** 🔴 OPEN — catalogado 15/09 pela lane bugs-and-gaps `192.168.100.15`. Triage na #261. Não corrigido aqui: toca **resolução de nomes** (regra 6) — mesma família do §179, onde a mantenedora exigiu a decisão travada antes de editar. Precisa de veredito: (a) o tipo do usuário faz shadowing do alias builtin (proposto), ou (b) nomes builtin são reservados e uma declaração de `List`/`String`/`Set`/`Map` pelo usuário é erro de diagnóstico (SEM0xx). Não bloqueia o portão de release (nenhum teste de suíte vermelho).
