...
**Follow-up de paridade (sem edicao de codigo — face medida pela lane docs, 18/09):** os 4 casos da prova cobrem JVM×3 + JS×1; as faces SCRIPT e NATIVE da mesma inferencia AINDA NAO foram medidas (o fix e de frontend compartilhado — inferencia do `BuiltinCallTyper`, mesmo IR em todos os targets — entao a expectativa e o mesmo `woof/meow`, mas expectativa nao e prova), e `conformance-matrix.md` nao tem celula para inferencia por subtype (so as linhas de if/switch-expr heterogeneos, 66–69). Dono = lane paridade/compiler: medir Script+Native + criar a celula da matriz.
