...
### §251 — tipos DECLARADOS nunca são validados: um tipo indefinido em retorno/parâmetro compila em silêncio e torna a classe incarregável (`NoClassDefFoundError`); em local/campo é ignorado em silêncio
...
- **Ponteiro (lane compiler):** criar um `declaredTypeResolves(name, sa, scope)`
  compartilhado (≈`checkThrowsClause`) aplicado nos sítios VarDeclStmt / param /
  retorno / campo. Dono = **lane compiler** (`SemanticAnalyzer`/`StatementAnalyzer`
  — arquivos quentes).
- **Estado:** 🔴 ABERTO — catalogado 15/09 pela lane bugs-and-gaps `192.168.100.15`.
  ...
  usuário). Nenhum teste da suíte cobre tipos declarados indefinidos; não é
  vermelho do portão.
