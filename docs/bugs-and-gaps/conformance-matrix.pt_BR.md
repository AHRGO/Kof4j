...
| `Double %` (mod of variables; + NaN/±Inf) | `1.5` / `1.0` / `0.5` / `-1.5` / `NaN` | DONE | DONE (bug 146 ✅ 12/09 `718ae5cf` — `NativeX86Arith` emits the real fmod) | DONE | DONE (§264 16/09 — era parcial: JS `String(1.0)`=`1`; agora `kofNumFmt` formata o contrato JDK) | `doublemod` |
| cast `d as Int` / `L as Int` / `66 as Char` | `9` / `70000` / `B` | DONE | DONE | DONE | DONE | `cast` |
| float println | `0.3333333333333333` / `5.0` / `3.5` | DONE | DONE (bug 44 ✅ 10/09 x86) | DONE | DONE (§264 16/09 — era parcial: JS `String(5.0)`=`5`; agora `kofNumFmt` contrato JDK) | `floatprint` |
| double println shortest-repr + scientific | `0.30000000000000004` / `1.0E7` / `1.0E-5` / `33.333333333333336` / `0.33333334` / `1.0E20` / `NaN` / `0.001` / `1.0E-4` / `3.4028235E38` / `-0.0` | DONE | DONE (bug §180 ✅ 14/09 x86_64 — `kof_dtoa` loop `%.*e`+`strtod` p/ o shortest round-trip + reformat Java; Float mantém a forma própria; **FLT001 FECHADO no riscv/aarch 15/09** — o mesmo `kof_dtoa` portado como fatia `RtB45` consome libc `snprintf`/`strtod` pelo link dinâmico sob demanda, `NativeRiscvDtoaTest` oracle nos 2 arches) | DONE | DONE (§264 16/09 — era parcial: `Number.toString` nao emite `.0`/cientifica no limiar JDK; `kofNumFmt` implementa o contrato) | `doubleprint` |
| infinity/NaN println+String.valueOf | `Infinity` / `-Infinity` / `NaN` | DONE | DONE (bug 44 residual ✅ 11/09 x86) | DONE | DONE (§264 16/09 — era parcial: `5` vs `5.0`) | `infinityprint` |
...
| `println(array primitivo)` formato de container (§388-B, voto 21/09) | `[65, 66]` / `[[65, 66]]` / `[]` / `[x]` — mesma gramática §107, oracle `ArrayList.toString`; passar `List` numa bytes-face é `SEM099` (§388-A) | DONE (antes era identidade `[I@…` até 20/09) | DONE (mesmo helper, via reflexão no KofRuntime) | DONE (antes `String()` sem colchetes; `kofFormat` reroteado pelo tipo estático) | DONE x86 medido 21/09 (`kof_array_to_string`); goldens riscv64/aarch64 no mesmo commit, medidos por CI/qemu (antes: só o elem[0] saía como char) | `arrayprint` |
...
| `put`/`remove` return prev (null-safe for primitive) + `set.add` | `false/true/3/true/false/1/2/2/null/0` | DONE (bug 112 ✅ 11/09 — was **VerifyError** in put and **NPE** in remove-miss; `emitPrevValueUnbox` guard. D-NULL-INTENT #278 18/09: `emitPrevValueUnbox` removed — `remove` of an absent key now returns real `null` instead of the primitive default, superseding §112/§125 opt. A) | PARTIAL (D-NULL-INTENT #278 phase 2, not covered — old representation kept: `remove` of an absent key still returns `0`, not `null`) | DONE (bug 112 ✅ 11/09 — `s.add` already-present returned **true** (add+contains) and NPE on remove-miss; `prevOrDefault` + real `HashSet.add`. D-NULL-INTENT #278: `prevOrDefault` removed — `remove`-miss now real `null`) | DONE (bug 112-JS ✅ 11/09 — was `null`: absent prev wrapped in `?? default` in the map handler + `KofPop` extended to preserve the wrapped side-effect. D-NULL-INTENT #278: the `?? default` coalesce now only fires for bare (non-Nullable) primitives — `remove`-miss goes back to real `null`, on purpose) | `mapmutret` |
| string unicode length/charAt | `4` / `é` / `café!` | DONE | DONE (bug 43 ✅ 10/09 x86) | DONE | DONE | `unicode` |
...
| wrong key as query ARG (Map/Set/List) | `null`/`false`/`false`/`null` | DONE (D-NULL-INTENT #278 18/09: the last cell — wrong-type key miss on `Map<String,Int>` — now real `null`, superseding bug 127/§125 opt. A) | PARTIAL (bug 126 ✅ partial 11/09 — was SIGSEGV on the wrong-type arg; tag is a CONJUNCTION elem×arg: String-equals only when both are String, otherwise raw cmpq = miss like the JVM. D-NULL-INTENT #278 phase 2, ALSO not covered here — old representation kept: last cell still `0`, not `null`) | DONE (D-NULL-INTENT #278: last cell now real `null`) | DONE (bug 127 ✅ 12/09 — get-miss of a primitive value wraps `?? default` in lowering; Bool-miss now `false`, not `0`. D-NULL-INTENT #278 18/09: the `?? default` coalesce now only fires for bare primitives — last cell back to real `null`, on purpose) | `wrongkey` |
| `println(f())` w/ `T? f()` null + `f()==null` direct (primitive) + null branch if/switch (`Int? f() = if(c) x else null`) + nullable `"a" + f()` | `null` / `null` / `null` / `6` / `true` ×5 + `7/null/null/null` + `anull/nullb` | DONE (D-NULL-INTENT #278 18/09 — SUPERSEDES bug 125/§125 opt. A below: `return null`/if-switch-null-branch/String-concat of `Nullable(primitive)` now carry a real boxed `null` (`ARETURN`/`ASTORE` of `Integer`/`Boolean`/etc., never the primitive default) — `foldNullablePrimBranches` only fires on Native now (phase 2); `emitWideningIfNeeded`'s Object→primitive erasure-unbox shortcut is skipped for a `Nullable(primitive)` destination (was `NoSuchMethodError Object.intValue()Integer;` on the switch-null-branch case, found by the full-suite diff). HISTORY (bug 125 ✅ 12/09, decision A, now superseded) — was VerifyError: `Int? f(){...}` emitted descriptor `I` + `ARETURN` + `aconst_null.intValue`; the return of `Nullable(primitive)` used to erase to the primitive default, same precedent as map-miss SG-008; isDoubleWidth unwraps Nullable for correct POP2 on `Long?`/`Double?`) | PARTIAL (D-NULL-INTENT #278 phase 2, not covered — old representation kept: still `0`/`false`×primitive/`7/0/0/false`/`a0/0b`, the exact pre-#278 string) | DONE (D-NULL-INTENT #278 18/09 — same supersede as JVM; HISTORY: bug 125 ✅ 12/09 — was NoSuchMethodError `Integer.valueOf/1` in the null boxing) | DONE (D-NULL-INTENT #278 18/09 — same supersede; `Boolean.valueOf(Z)` in `JsCallEmitter` now dispatches through the null-safe `kofBoolValueOf` helper instead of the raw `x ? true : false` ternary, which collapsed `null` to `false`. HISTORY: bug 125 ✅ 12/09 — printed `null`, now `0` at the time; the PRINT of `Double?` stays out of the cell = floatprint §44, correct value. bug 139 ✅ 12/09 — was COMP002 "stack underflow" in the DIRECT form `f() == null`: the `call;POP;false` fold had no counterpart in the JS parser; mid-expression discard now consumes the top and preserves the side-effect in the preamble) | `nullableprint` |
...
| store in `Byte[]`/`Short[]` OUT of range (signed 8/16-bit narrowing) — §184 | `-126` / `4464` | DONE | DONE | DONE | PARTIAL (bug §184 — `kofArraySet` writes the raw value: `b[0]=130`→`130`; silent divergence) | `narrowarr` |
| store into `Char[]`/`Bool[]` element — §185 | `A` / `B` / `true` / `false` | DONE | DONE | PARTIAL (bug §185 — interpreter: `coerceFor` returns `Integer`; `Array.set(char[]/boolean[],…)` throws `argument type mismatch`, exit 1) | DONE | `chararr` |
...
| `if` NESTADO cujo then termina em `throw`/`return` (epílogo externo deve sobreviver) | `p:y` / `c:yb` / `c:bx` / `out:y` / `mid` / `c:in` | DONE | DONE | DONE | DONE (§380 ✅ 20/09 — pilha de parsing dos falseLabels ATIVOS: o parse do else interno devolve o label do envolvente, nunca o consome; guardas §147/§149/§174 intactas) | `nestedifthrow` |
...
| reflexão de compile-time `interop.schema(R)` (X6.1/X6.2, apagada) | `2` / `name:String` / `age:Int` | DONE | DONE | DONE | DONE | `interopschema` |
