...
| stdlib kof.validation BR (S5: isCpf/isCnpj/isCep/isPis + S12c isNis — arithmetic weights, mod-11 by subtraction; + `==true`/`==false` §93) | `true` / `false` / `true` / `false` / `true` / `false` / `true` / `false` / `true` / `true` / `true` / `true` / `false` / `false` / `false` | DONE | DONE | DONE | DONE | `stdvalidation` |
