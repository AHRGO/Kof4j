...
| `SECN00x` / `GPU001` | `kof.security` / `kof.gpu` | 🟡 medido 17/09, segue aberto: o Android reuse o `JvmBackend`, mas os gates `supportedOn` de security/gpu excluem `ANDROID` — `passwords.hash` → `SECN001`, `crypto.sha512` → `SECN003`, `jwt.create` → `SECN004`, chacha → `SECN002`, `gpu.available()` → `GPU001` (aquelas pilhas ainda não rodam no Android — regra 6). `kof.db`/`kof.orm` eram over-gating e foram **corrigidos 20/09** (D-DB-GAPS DB-2: "Android É JVM" — `db.connect`/`orm.*` compilam limpos, `Main.class` byte-idêntico ao alvo JVM) — `known-bugs.md` §278 PARCIAL |
