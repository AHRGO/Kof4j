...
| `SECN00x` / `GPU001` | `kof.security` / `kof.gpu` | 🟡 measured 17/09, still open: Android reuses `JvmBackend` but the security/gpu `supportedOn` gates exclude `ANDROID` — `passwords.hash` → `SECN001`, `crypto.sha512` → `SECN003`, `jwt.create` → `SECN004`, chacha → `SECN002`, `gpu.available()` → `GPU001` (their stacks don't run on Android yet — rule 6). `kof.db`/`kof.orm` were over-gated and were **fixed 20/09** (D-DB-GAPS DB-2: "Android IS JVM" — `db.connect`/`orm.*` now compile clean, `Main.class` byte-identical to the JVM target) — `known-bugs.md` §278 PARTIAL |
