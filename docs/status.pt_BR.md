...
**20/09 — CONSOLIDACAO DA MADRUGADA 19/09→20/09 (lane docs, conferido contra `git log`/tip `1080238f`; CI `Build+Tests` verde nos tips da madrugada, tally do reator completo ~3034 testes 0F/0E na corrida das ~23:5x da `.18`).**
>
> - **D-TROOL FECHADA — `Bool` fica com 2 valores; `Troolean` (`true/false/null`) e o tipo trivalente de Kleene da linguagem** (decisao da mantenedora 19/09, roadmap 2.6.5–2.6.7 ✅, DECISIONS §D-TROOL; landed `916b9fb7`/`d61836eb`/`5f0757e8`): `Bool?` escrito pelo usuario e rejeitado (SEM095) com a seta p/ Troolean; `&&`/`||`/`!` desucaram em cadeias Kleene estritas na maquina boxed (backends intocados); `if (t)` ≡ `if (t == true)`; lei travada em `TrooleanLawE2ETest` 13/13; corpus sincronizado (`training/language/types.md`, fake-idioms, CHANGELOG, backend-parity) — EN+PT.
> - **O rio da erasure JVM landou (familias §355/§356/§357, `16f16081`/`c8d55a10`/`0aa6a307`)** — 8 issues fechadas na prova (#399/#363/#375/#385/#366/#365/#295/#368): variaveis de tipo carregam o bound ate cada site de emissao, interfaces genericas baixam com descritores apagados e pontes covariantes varrem pais E interfaces (box/unbox de primitivo na fronteira), campo `T[]`/`as T[]` apagam com honestidade, e a decisao so-JVM `SEM098` nomeia o caso array-primitivo-no-slot-apagado. O §358 grava o gap NATIVO PRE-EXISTENTE (link do `toString` de `T` sem bound no cross) p/ lane nat — medido igual no tip limpo, nao e regressao.
> - **§360 CORRIGIDO — as ops do handle de `process.spawn` eram CODIGO MORTO no JVM** (`4ed9bb2f`): o dispatcher nunca roteava um receiver `Long` para a branch de handle, entao `readLine/write/exitCode/kill/alive` baixavam para um `invokevirtual java/lang/Long.readLine` cru e morriam em EXECUCAO. Roteamento consertado; e a face JS landou (`081a48f8` + `2d20e5d4` `runWith(argv,cwd,env)` JVM+JS): spawn+handle+pipeline no JS via `KofJsProcessBridge` com paridade byte — `ProcessSpawnE2ETest` 4/4, `ShellE2ETest` 15/15; tracker 2.2 ✅ FECHADO (ultima residual era o pipeline JS).
> - **Estado real do X7 (debug info) medido: o DWARF x86 JA EXISTIA e agora os CROSS tambem** — tabelas de linha `.file/.loc` (`NativeDwarfCrossTest`, `5d9c855c`/`23b7ecf7` provaram o `.debug_line` no ELF riscv/aarch64 real) e DIEs CU/subprogram com `frame_base` por ABI (`890b58bf`); source maps JS ✅; `kof profile` honesto em nivel de processo. As linhas X7/§19.5 do tracker foram corrigidas contra o codigo (estavam abertas estando implementadas).
> - **X10 FECHADA (`27826838`)** — o catalogo de assinaturas do LSP chegou a **32/32 namespaces** (`json` na tabela com trava comportamental ao SEM025 real do `MemberCallNamespaces`), hover + `signatureHelp` + `completionItem/resolve` de UMA fonte gerada (`scripts/gen_signatures.py`), 265 membros / 282 formas estaveis.
> - **Dois bugs NOVOS catalogados com matriz de 4 alvos (lane docs; os fixes pertencem ao cluster `.22`)**: **§361** — escritas em campo nullable-primitivo guardam o inteiro cru onde o slot exige a caixa (`b.n = 42` → JVM `VerifyError` na CARGA, Native SIGSEGV, Script/JS imprimem `42`; root pointer: o gate de box do `ExpressionFieldAssignLowerer` sem `isNullablePrimitive` — o gemeo nao espelhado do §295(b)); **§362 / issue #545** — chamada a construtor INEXISTENTE aceita em silencio (`P(1,2)`, `C(1)`, `C("s")` → JVM morre em EXECUCAO, Script imprime `0`, JS imprime `1`, Native quebra no `ld` com simbolo de ctor indefinido; esperado: SEM em compile, o precedente da familia #469/#470). O fix do §361 ja esta na arvore da mantenedora (`.22` ao vivo).
> - **Melhorias de disciplina do ledger (licoes da madrugada, multi-agente)**: a regra do §NNN agora EXIGE pegar o numero do TIP REMOTO (colisao dupla no §355 renumerou duas vezes -> §359/§360 acertados com nota de procedencia); lixo de stash/marcadores de conflito commitados por uma lane foi recatado cirurgicamente (`029ac684`/`515414db`, os tres lados preservados); o `check_known_bugs_status.sh` aprendeu o token 🔓 + caso no `--selftest`; `docs-lang` mantido em 100%.
> - **Registro de decisoes cresceu (§7dfc6441, DECISIONS EN+PT)**: `D-MAKEALIVE` (compilador que compila a si mesmo), `D-KOF-AS-CLOUD` (postura de plataforma), `D-BOOTSTRAP`, `D-DB-GAPS` (fila do ORM: 13 entradas nativas `kof_orm_*` medidas faltando — recon feito, fatias planejadas) e a **Lei da Simplicidade** promovida a regra 11 do AGENTS.md (o que alcancar a superficie da linguagem tem de ser curto/idiomatico/intencional). A regra dos 3 estados foi aplicada: os planos workflow/shell concluidos foram PROMOVIDOS `development/`→`docs/` no mesmo push que os fechou.
>
...
> - **X7 (debug info) estado real medido: o DWARF x86 JA existia e agora os CROSS tambem** — tabelas de linha `.file/.loc` (`NativeDwarfCrossTest`, `5d9c855c`/`23b7ecf7` provaram `.debug_line` no ELF riscv/aarch64 real) e DIEs de CU/subprogram com `frame_base` por ABI (`890b58bf`); source maps JS ✅; `kof profile` honesto em nivel de processo. As linhas X7/§19.5 do tracker foram corrigidas contra o codigo (estavam abertas item ja implementado).
