...
- A versão vive no arquivo `VERSION` na raiz do repositório (`0.5.0-beta`).
...
- Não editar versões manualmente em vários arquivos.
 ...
 - Workflow de push na main: 2 jobs — `test-and-bump` (bump + push +
   ...
   GitHub Release por plataforma com JDK 25 embutido.
...
JDK embutido (Temurin 25; tooling API level 21).
...
- Tags seguem `kof-<versão>` (ex.: `kof-0.5.0-beta`).
