...
if (("print".equals(mc.methodName()) || "println".equals(mc.methodName())) && mc.arguments().size() == 1) {
    ...
    if (CompilerTypes.isEnumType(argType, driver.currentUnit)) {
        ...
        if (isCharPrimitive) {
            PrimitiveStringLowering.emitChar(driver, ops, argType);
            ...
            // D-NULL-INTENT: no JVM e Native, argType já Nullable(primitivo) chega
            // FISICAMENTE boxed (return/local/Map do #278) — boxPrimitive de
            // novo chamaria Integer/Boolean.valueOf sobre uma referência já
            // boxed (VerifyError). Só pula o box no JVM/Native quando já é
            ...
            boolean skipBox = (driver.target == Target.JVM || driver.target.isNative()) && argType instanceof Type.NullableType;
            if (!skipBox) {
