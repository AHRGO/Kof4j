...
1. **Zero regression.** No commit may make an existing test start to
   fail. The full suite (`mvn test`, today **2206** across the 4 modules — see
...
> is **2206 tests** (compiler 1899 + script 38 + kof-c 7 + cli 262, measurement
> 16/09 — grows with each commit): **0 regressions (1 = the intermittent §252 native flake) / 0 errors** (node now present on
...
> (`4408eb6`) + the other toolchain/external-DB guards → `2206/1/190-skip` (the 1 failure = the known intermittent §252 native flake, not a regression)
> (MEASURED 16/09 ~05:34, clean clone). With qemu, **everything executes** — the 84 cross run
> green and the total stays `2206` with the skip count dropping to the
> external-DB/`node`-env residual. Correct state TODAY (16/09 ~05:34, clean clone of `574c9419`):
