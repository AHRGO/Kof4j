...
> **PRÓXIMO PASSO (17/09 ~10:05, dono = 192.168.100.15, lane bugs-and-gaps):** varredura doc×código dos lands novos rendeu 3 commits: `79163241` (observability→`OBS002`: `histogram`/`metrics()`/`spanStart`/`spanEnd` adicionados à API; testes 3/3→7/7; `KOFUI-AUDIT` recontagem UI001/UI002) e `68713423` + `02353b6c` (land `7c1305ab` #331/#327: SG-013 estendido a CAMPOS — `classes.md` dizia 'field access is NOT checked'; `SEM063` adicionado ao `type-system.md`; `specification-status` = métodos+campos; símbolos-fantasma `accessFlagsFor:3388` e `ExpressionLowerer:198` removidos). **Varredura `java:line` em docs estáveis = LIMPA** (só restam o `NativeBackend:629` pré-split já anotado e o plano DONE `planning-switch-expr`). **Fila D-DIAG-EN (doc-sync) — ponto exato:** sincronizar docs que citam texto PT de ferramenta **por pacote traduzido**; scanner pronto. Hoje: `SEM030` (learn/04 EN já cita EN, mas o código AINDA emite PT) e `SEM033` (`known-bugs.md:5859`) = **não tocar** enquanto o código for PT; a família `SEM031/032/035` (switch, land `8edfe970`) não tem citação PT em learn/training → no-op. Próximo: (1) no próximo land i18n, sincronizar os docs do pacote; (2) re-varredura quando outra lane landar (`git log <head>..origin/beta-0.4.0`); (3) §262 face (b) repassada à lane compiler. NÃO tocar `docs/status.md` (lane `.22`).
