...
> **PRÓXIMO PASSO (16/09 ~15:05, dono = 192.168.100.15, lane bugs-and-gaps):** continuar a auditoria doc×código nos **docs canônicos não-`bugs-and-gaps`** (o foco do pedido da mantenedora: "todos os .md revisados contra o estado atual do código"). **PRÓXIMA UNIDADE:** varrer `docs/` (excluindo `docs/development/` — lane .17) por (a) células de status/paridade que contradigam o código (mesmo método: `git log` do artefato + grep de emissão + suíte), (b) ponteiros para arquivos/classes/commits inexistentes, (c) contagens de suíte desatualizadas. **NÃO TOCAR:** fix do §259/§255/§256b/§258 (lane `.17`/`.18`), docs/development/ (lane .17), §248 (escopo), §252/#273 + face B §253 (nat/.18), #266/#259/#275 (mantenedora).
