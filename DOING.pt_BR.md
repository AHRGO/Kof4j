...
> **PRÓXIMO PASSO (16/09 ~16:40, dono = 192.168.100.15, lane bugs-and-gaps):** auditoria doc×código **concluída** nesta sessão — 7 unidades (STDLIB EN header, tabelas legacy DOING, ponteiros `docs/known-bugs.md`→`bugs-and-gaps/`, suíte stale em `training/` 810→2214, SHAs mortos = 0 reais, 24 `file:line` pós-REFACTOR-500, versão corrente stale em `README`+`architecture`+`language-reference` 0.3.0→0.4.0 + 2214). **Sem drift real restante nos docs canônicos.** Re-trigger recusado até (a) regressão nova (suite red / bug reintroduzido), (b) issue nova, ou (c) trabalho destravado por fechamento de lane alheia. **NÃO TOCAR:** §259/§255/§256b/§258 (lane `.17`/`.18`), `docs/development/DECISIONS.md` (lane .17), §248 (escopo), §252/#273 + face B §253 (nat/.18), #266/#259/#275 (mantenedora).
