...
> **✅ FEITO (16/09 ~12:10, dono = 192.168.100.15, lane bugs-and-gaps): APP002 — `[server] port` no kof.toml é parseado mas nunca usado pelo `kof serve` (EN+PT).** `KofProjectConfig.java` parsea `[server] port = N` → `serverPort()`, mas `CmdServe.java` só usa `--port`/8080; zero chamadores de `serverPort()` no repo. Docs `backend-parity.md:89` descrevia APP002 como "fallback to default" (gap honesto) — agora `PARTIAL`: parse funciona, port não é usado. Não há código `APP002` emitido em lugar nenhum; o gap é de UX/implementation. **NÃO implementar** (escopo — lane da mantenedora se/quando abrir frente serve-config). Atualizado backend-parity.md(+PT) para `PARTIAL (port unused)`.> **PRÓXIMO PASSO (16/09 ~12:00, dono = 192.168.100.15, lane bugs-and-gaps):** §259 + varredura de gap codes (CONFIG001→CONF001, JS=CONF001 stale, profiles) commitados+pushados (`f06c3cb1`). **PRÓXIMA UNIDADE:** continuar a varredura de gap codes doc×código com os itens pendentes do diff — verificar (a) `APP002` (`serve` sem manifest `[server]`) tem implementação ou é claim só-docs; (b) `COL001`/`NATIVE003`/phantom-class em `docs/history/` (históricos OK se marcados); (c) códigos citados como diagnostics atuais (`CANVAS001`, `UI0xx` em KOFUI-AUDIT, `UUID002`) conferem com o código. **NÃO TOCAR:** fix do §259 (lane native/compiler `.17`/`.18`), docs/development (lane .17), §248 (escopo), §252/#273 + face B §253 + §255 + §256b (nat/.18), #266/#259/#275 (mantenedora), §258/.18.
