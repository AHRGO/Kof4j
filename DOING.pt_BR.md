...
> **PRÓXIMO PASSO (16/09 ~14:35, dono = 192.168.100.15, lane bugs-and-gaps):** **varredura de DIVERGÊNCIA EN/PT em cabeçalhos de status** — a unidade acima achou um caso (EN preso em estado antigo enquanto PT avançou). **PRÓXIMA UNIDADE:** comparar sistematicamente `**Status:**`/`**Estado:**` entre cada par canônico × `pt_BR` (o `docs-lang.sh drift` só olha mtime, não o conteúdo) e corrigir o lado atrasado contra o CÓDIGO. **DEPOIS:** reconciliar a tabela legacy "Em curso agora" (`DOING.md` ~L4310 / `DOING.pt_BR.md` ~L3775) — 7 linhas `EM CURSO` com dono por NOME (sem IP) e branch `beta-0.3.0` morta, de itens já provados concluídos no código (EDI001 `CONCLUÍDO 13/09` em `docs/tooling/`; REFACTOR-500 `CLOSED 13/09` em `docs/architecture/`; STDLIB concluído 14/09) OU já cobertos pelas linhas ativas do topo do DOING. **NÃO TOCAR:** fix do §259/§255/§256b/§258 (lane `.17`/`.18`), docs/development/DECISIONS.md (lane .17), §248 (escopo), §252/#273 + face B §253 (nat/.18), #266/#259/#275 (mantenedora).
