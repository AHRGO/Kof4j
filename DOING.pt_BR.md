...
> **✅ FEITO (18/09 ~04:00, dono = 192.168.100.15, lane kof-android): GAP REAL — flags android-only (`--apk`/`--keystore`/`--storepass`/`--keypass`/`--alias`) eram IGNORADAS em silêncio fora do alvo/sem `--apk` (R6) (código + testes + docs EN+PT).** Continuando a auditoria de flags do `CmdBuild` (2 achados anteriores: `--aab` e `--apk` sem SDK), medi: `kof build <src> --target jvm --apk` = **exit 0 sem mensagem**, `--target js --keystore x` = **exit 0**, `--target android --keystore x` (sem `--apk`) = **exit 0** — as flags de assinatura só agem dentro do pipeline `--apk`, então fora dele eram descartadas sem aviso: o usuário/CI acreditava que o APK saiu assinado. **Fix (raiz):** duas validações cedo no `CmdBuild.run` (mesmo padrão do `--fat`/`--aab`, R6): (1) `target != ANDROID` com qualquer flag android-only → erro + exit 1 (imprime o alvo); (2) assinatura (`--keystore/--storepass/--keypass/--alias`) sem `--apk` → erro + exit 1. **Prova (Q1/Q0/Q3, mesmo commit):** `CmdBuildAndroidAabTest` 7/7 — `apkOutsideAndroidIsHonestError` (agora assere exit≠0 + "android"), `signingFlagsOutsideAndroidAreHonestError`, `signingFlagsWithoutApkAreHonestError`; Q0: os 3 casos saíam 0 no código antigo (medido via CLI), agora 1; `android --apk --keystore` segue caindo no gap honesto do SDK (exit 1); `android` puro segue exit 0. **Doc:** bullet de signing em `KOFANDROID.md`(+PT) agora diz que as flags são só android e recusam com exit 1 (R6). Sem regressão: `CmdBuildFatTest` 4/4, `CmdBuildAndroidSdkTest` 5/5, `CliUsageTargetsTest` 4/4. `docs-lang.sh check` 0/0/0. **NÃO TOCAR:** `docs/development/DECISIONS.md` (lane .17); §252/§253-B (nat .18); §258 (development .18). **PRÓXIMO PASSO:** lane kof-android — restam `--aab` full (precisa bundletool real) e ícone por metadado (**decisão pendente** — `kof.toml [app] icon` vs `--icon`, regra 6); re-varrer doc×código a cada land de outra lane.

> **✅ FEITO (18/09 ~03:20, dono = 192.168.100.15, lane kof-android): GAP REAL — `--apk` sem SDK saía exit 0 SEM gerar APK (mesma classe do `--aab` silencioso) + bloco de ícone duplicado no doc (código + testes + docs EN+PT).** Medido: `kof build <src> --target android --output d --apk` sem `ANDROID_HOME` imprimia `--apk: ANDROID_HOME não definido; gere o projeto e use 'mvn verify'` e **retornava exit 0** — `runApkPipeline` era `void` + `return;`, então o `$?` do usuário/CI dizia sucesso sem APK algum (R6). **Fix (raiz):** `runApkPipeline` agora devolve `boolean` (`true` só após `APK gerado:`), `false` nos 3 caminhos de recusa (sem `ANDROID_HOME`, sem `aapt2` build-tools 34, exceção do pipeline); `CmdBuild.run` guarda o retorno em `apkOk` e faz `System.exit(1)` quando `--apk` foi pedido e não produziu artefato. Sem `--apk`, a geração do projeto segue exit 0 (não regride). **Prova (Q1/Q0/Q3, mesmo commit):** `CmdBuildAndroidAabTest` 5/5 — `apkWithoutSdkIsHonestError` (ANDROID_HOME removido do env do subprocesso → exit≠0 + mensagem `ANDROID_HOME` + pom.xml ainda gerado), `projectGenerationWithoutApkStillSucceeds` (exit 0 sem `--apk`), `apkOutsideAndroidIsHonestError`; Q0: o caso `--apk` sem SDK saía 0 no código antigo (medido via CLI), agora 1. Novo helper `cliWithEnv` no teste. **Doc:** a linha do modo standalone em `KOFANDROID.md`(+PT) agora diz que sem SDK a flag falha com exit 1 (nunca exit 0 sem APK); removido um **bloco de ícone DUPLICADO** na seção Fase 2 (as mesmas 2 linhas apareciam 2×). `docs-lang.sh check` 0/0/0. Sem regressão: `CmdBuildAndroidSdkTest` 5/5, `CliUsageTargetsTest` 4/4, `AndroidInteropE2ETest` 16/16. **NÃO TOCAR:** `docs/development/DECISIONS.md` (lane .17); §252/§253-B (nat .18); §258 (development .18). **PRÓXIMO PASSO:** lane kof-android — restam `--aab` full (precisa bundletool real) e ícone por metadado (**decisão pendente** — `kof.toml [app] icon` vs `--icon`, regra 6); re-varrer doc×código a cada land de outra lane.

> **✅ FEITO (18/09 ~02:40, dono = 192.168.100.15, lane kof-android): roadmap.md Android section stale — "Current state: Phase 1" (EN+PT).** A seção KofAndroid do `docs/development/roadmap.md` ainda dizia "🟡 Phase 1 implemented — gera projeto Maven". Como a lane kof-android implementou até a **Fase 4** + AND002 imposto, e a seção é do alvo que esta lane assume, atualizei o estado p/ "Phases 1-4 implemented" (Fase 2 label/permissions+`--apk`/`--keystore`; Fase 3 WebView responsivo `<meta viewport>`+CSS+wide-viewport do host; Fase 4 `--min-sdk`/`--target-sdk` → `<uses-sdk>`/platform jar/`d8 --min-api`; AND002 gap imposto; pendências Fases 5+ = `--aab` honesto e ícone decision-pending). Reivindicação: a seção KofAndroid do roadmap não tinha dono no DOING (sem `EM CURSO`/`dono` citando roadmap.md); as linhas PT da seção **Kof4J/KofNative/etc** são de outras lanes e **não foram tocadas**. `docs-lang.sh check` 0/0/0. **NÃO TOCAR:** `docs/development/DECISIONS.md` (lane .17); §252/§253-B (nat .18); §258 (development .18). **PRÓXIMO PASSO:** lane kof-android — restam `--aab` full (precisa bundletool real; comportamento honesto já no lugar) e o ícone por metadado (**decisão pendente** regra 6 — mecanismo `kof.toml [app] icon` vs `--icon` não fechado). Sem isso, o alvo está funcionalmente completo nas fases cobertas; re-varrer doc×código a cada land de outra lane.

> **✅ FEITO (18/09 ~02:00, dono = 192.168.100.15, lane kof-android/docs): sweep de símbolos fantasma pós-REFACTOR-500 + lands (EN+PT).** Scanner de drift (`scan.py symbols`) apontou símbolos citados em docs vivos que não existem mais no código. Corrigidos (7 arquivos × EN+PT): (1) **`refactor(compiler) c9a4060f`** moveu código — `SemanticAnalyzer.checkInterfaceImplementation` → `ImplementationChecker.checkInterfaceImplementation` (`specification-status`, `type-system`); `SemExpressionTyper.narrowedScope` → `SemNarrowing.narrowedScope` chamado de `SemExpressionTyper` (`specification-gaps`). (2) `SemanticAnalyzer.isAssignable` → **`TypeChecker.isAssignable`** (`training/language/types`). (3) `Parser.parseSwitchStatement` → **`StatementParser.parseSwitchStatement`** (`planning-switch-expr`). (4) `CompilerDriver.compile(...)` → **`CompilerDriver.compileSources(sources, outputDir, target, moduleRoot)`** (`learn/native/architecture`, o método `compile` não existe mais). Docs-lang 0/0/0. **NÃO tocar:** `docs/development/DECISIONS.md` (lane .17); §252/§253-B (nat .18); §258 (development .18); `roadmap.md` (lane alheia). **PRÓXIMO PASSO:** continuar a lane kof-android (resta `--aab` full = bundletool real; ícone = decisão pendente regra 6) e re-varrer doc×código a cada land de outra lane.

> **✅ FEITO (18/09 ~01:00, dono = 192.168.100.15, lane kof-android): drift docs do alvo Android — `kof test`, AND003 e `--aab` silencioso (código + testes + docs EN+PT).** (1) **`--aab` era IGNORADO em silêncio** (`kof build --target android --aab` = exit 0 sem AAB nem mensagem, medido) — mesma classe do AND002 (R6). Agora a flag é reconhecida: alvo não-android → erro honesto; android → diagnóstico `--aab: App Bundle (AAB) ainda não é gerado — requer bundletool (fora do build-tools)`, exit 1, projeto ainda gerado. Prova: `CmdBuildAndroidAabTest` 2/2 (novo, subprocesso). (2) **Doc `kof test` stale:** a tabela de lifecycle dizia `main()` "still exists and is testable (`kof test`)" — mas `kof test` NÃO suporta android (usa `jvm|native|js`; com android cai no ramo nativo e reporta "no binary produced"). Corrigido p/ "teste a lógica com `kof test` em jvm/js; android é empacotamento". (3) **AND003 honesto:** a tabela de restrições implicava gate de compile-time, mas a linguagem não tem superfície de reflexão própria (nenhum namespace `reflect`) — marcado como *caveat, não gate de compile-time*. Prova geral: `CmdBuildAndroidSdkTest` 5/5, `CliUsageTargetsTest` 4/4, usage strings atualizadas (`--aab`). **PRÓXIMO PASSO:** resta apenas `--aab` full (precisa bundletool real: proto-format APK + `bundletool build-bundle`) e o ícone por metadado (**decisão pendente** — regra 6). **NÃO TOCAR:** `docs/development/DECISIONS.md` (lane .17); §252/§253-B (nat .18); §258 (development .18); `roadmap.md` (lane alheia — a linha "Current state: Phase 1" segue stale, sinalizada).

> **✅ FEITO (18/09 ~00:10, dono = 192.168.100.15, lane kof-android): GAP REAL — AND002 documentado mas NÃO imposto (kof.web compilava silenciosamente no Android) + claim de CI stale (código + teste + docs EN+PT).** Análise de gaps do alvo: a tabela de restrições do `KOFANDROID.md` já listava `AND002` (servidor embutido) e `AND003`, mas **nenhum dos dois existia no código** — a checagem de gap do web (`ExpressionBuiltinInstanceCalls.lowerWeb:79`, `ExpressionMethodCallLowerer:404`) EXCLUÍA `Target.ANDROID` explicitamente, então um programa com `web.app()/get()/listen()` no Android compilava (exit 0, só o warning AND004) e gerava código de servidor que não roda num app móvel — falha silenciosa, R6. **Fix:** Android agora cai no ramo de gap com `AND002` nos 2 sítios (construtor `web.app()` em `ExpressionMethodCallLowerer` + métodos de instância em `ExpressionBuiltinInstanceCalls`, helper novo `webGap`); mensagem aponta o interop. **Prova (Q1/Q3, mesmo commit):** `AndroidInteropE2ETest.androidWebServerIsHonestGapAnd002` (16/16) — repro real via CLI `kof build --target android` = `Main.kf:3:5 error ... [AND002]` + exit 1; `--target jvm` segue exit 0 (JVM não afetado). `AND003` (reflexão) NÃO foi imposto: a linguagem não tem superfície de reflexão (nenhum namespace `reflect`); fica como caveat do doc. **Doc stale corrigido:** a lista de pendências dizia "CI ... emulator step pending", mas `.github/workflows/android.yml` JÁ tem o job `emulator-smoke` (reactivecircus/android-emulator-runner) + `interop` — reescrito como seção "CI" factual (manual, não ligado a push); `CONTRIBUTING`(+PT) deixa de listar "emulator CI" como pendente. **PRÓXIMO PASSO:** pendências restantes do alvo = `--aab` (bloqueado por bundletool) e ícone por metadado (**decisão pendente** da mantenedora — regra 6). **NÃO TOCAR:** `docs/development/DECISIONS.md` (lane .17); §252/§253-B (nat .18); §258 (development .18); `roadmap.md` (lane alheia).

> **✅ FEITO (17/09 ~23:35, dono = 192.168.100.15, lane kof-android): FASE 4 — `--min-sdk`/`--target-sdk` no `kof build --target android` (código + testes + docs EN+PT).** Implementa o design já prometido no doc do alvo ("override por flag explícita, não arquivo mágico"). **Caminho completo:** `CmdBuild` parseia `--min-sdk <n>`/`--target-sdk <n>` (com `=` também), valida (inteiro 1..99; só `--target android`; `min <= target`) e chama `CompilerDriver.setAndroidSdk(min,target)` → `CompilerPipeline` constrói `AndroidProjectWriter(min,target)` → saída em 3 lugares: (1) `AndroidManifest.xml` ganha `<uses-sdk android:minSdkVersion=... android:targetSdkVersion=.../>` (antes NÃO existia uses-sdk); (2) `pom.xml` linka `platforms/android-<targetSdk>/android.jar` e o `d8 --min-api <minSdk>` deixa de ser 24 fixo; (3) pipeline standalone `--apk` (`runApkPipeline`) usa a mesma plataforma e min-api. Defaults 24/34 preservados. **Prova (Q1/Q3, mesmo commit):** `AndroidInteropE2ETest.androidSdkOverrideThreadsToManifestPomAndReadme` (15/15) — 21/35 chegam a manifest+pom+README; `CmdBuildAndroidSdkTest` 5/5 (novo, subprocesso CLI) — 21/35 no manifest/pom, defaults 24/34, e 3 recusas honestas R6 (`--min-sdk abc`, flag fora do android, `min>target`). Sem regressão: `CmdBuildFatTest` 4/4, `CliUsageTargetsTest` 4/4. **Docs:** `KOFANDROID.md`(+PT) ganha "Phase 4 — SDK versioning by flag" e a lista de pendências encolhe (Fase 5+: `--aab`, ícone por metadado, CI de emulador); header → Fases 1-4; `CONTRIBUTING`(+PT) atualizado. **PRÓXIMO PASSO / ⚠️ DECISÃO PENDENTE (regra 6):** o override do ícone por metadado NÃO está decidido — o doc diz "metadado declarativo, não annotation", mas não fixa o MECANISMO (`kof.toml [app] icon` vs flag `--icon`); precisa de decisão da mantenedora antes de implementar. `--aab` segue bloqueado por `bundletool` (gap honesto se ausente); CI de emulador precisa de runner com emulator. **Observação de gate:** `scripts/check_500.sh` acusa 2 vermelhos PRÉ-EXISTENTES de outra lane (`SemanticAnalyzer` 719, `SemExpressionTyper` 618) — não são desta lane; os arquivos tocados aqui ficam ≤500. **NÃO TOCAR:** `docs/development/DECISIONS.md` (lane .17); §252/§253-B (nat .18); §258 (development .18); `roadmap.md` (docs/development, lane alheia).

> **✅ FEITO (17/09 ~23:10, dono = 192.168.100.15, lane kof-android): bug REAL `kof build` sem fonte estourava `ArrayIndexOutOfBoundsException` (código + teste).** Achado ao abrir `CmdBuild.run` para estender (Fase 4 Android): o guard `if (args.length < 2)` imprimia o usage mas **não retornava**, e a linha seguinte acessava `args[1]` no check de `--help` → crash medido (`Index 1 out of bounds for length 1`, `CmdBuild.java:27`, exit 1) para `kof build` sem argumento. **Raiz:** formatação malformada `} return; }` que fechava o `if` do help e o `if` de comprimento no MESMO statement, deixando o `return` sem efeito no caminho de uso. **Fix:** `return` dentro do bloco `args.length < 2` (padrão do `CmdRun`), fechando o `if` ali. **Prova (Q1, mesmo commit):** `CliUsageTargetsTest.buildWithoutSourcePrintsUsageInsteadOfCrashing` (4/4) — `assertDoesNotThrow` + usage em stderr; Q0: falhava com AIOOBE no código antigo; run real `Main build` → usage + exit 0; `build --help` preservado. **PRÓXIMO PASSO:** Fase 4 Android (`--min-sdk`/`--target-sdk`). **NÃO TOCAR:** `docs/development/DECISIONS.md` (lane .17); §252/§253-B (nat .18); §258 (development .18).

> **⚡ EM CURSO → ✅ FEITO (17/09 ~22:45, dono = 192.168.100.15, lane kof-android — reivindicada nesta sessão): FASE 3 — RESPONSIVIDADE do target Android (código + teste + docs EN+PT).** A lane kof-android não tinha dono ativo; reivindiquei por diretriz da mantenedora ("assume a lane do kof-android e implementa tudo que falta, inclusive suporte a responsividade"). **Gap real medido:** o `index.html` gerado NÃO tinha `<meta viewport>` e o host `android-host.kf` NÃO ligava `setUseWideViewPort` → o WebView do Android usa o viewport de layout de **980px** do desktop e a UI aparece encolhida/zoom-out. **Fix (raiz):** (1) `JsArtifactWriter.writeHtmlEntry` emite `<meta name=viewport content="width=device-width, initial-scale=1, viewport-fit=cover">` + bloco `@media (max-width:600px)` (`.kof-row` quebra, padding titlebar/root encolhe) — vale desktop e Android; (2) `JsRuntimeCore.kofUiSerializeHtml` (export HTML) emite a mesma meta; (3) `dev/kof/android-host.kf` liga `setUseWideViewPort(true)`+`setLoadWithOverviewMode(true)` (sem os dois o WebView IGNORA a meta); (4) `AndroidProjectWriter.patchIndexForPlatform` injeta a meta defensivamente em `index.html` custom (não duplica; âncora charset→`</head>`); (5) `fake-sdk.jar` regenerado com os 2 setters novos (javac --release 21, só a entry `WebSettings.class` substituída — 18 entries intactas). **Prova (Q1/Q3, mesmo commit):** `AndroidInteropE2ETest.androidResponsiveViewportAndWebViewWideViewport` (14/14 total) — index.html tem `name=viewport`+`width=device-width`; bytecode de `MainActivity.class` chama `WebSettings.setUseWideViewPort`/`setLoadWithOverviewMode`; Q0: as 3 asserções falham no código antigo (sem meta, sem setters). **Sem regressão:** `KofJsE2ETest` 40/40, `KofMediaE2ETest` 15/15, `WindowE2ETest` 3/3, `BackendParityTest` 19/19, `KofJsBrowserE2ETest` 26/0/23-skip (node/Chrome ausentes). **Docs:** `docs/targets/KOFANDROID.md`(+PT) ganham "Phase 3 — responsiveness" + lista honesta de pendências (Fase 4+: `--aab` precisa bundletool; `--min-sdk/--target-sdk` por flag; ícone por metadado; CI de emulador); header → Fases 1-3; `CONTRIBUTING`/`learn/30-contributing` atualizados. **NÃO editado:** `roadmap.md`(docs/development, lane alheia) — a linha "Current state: Phase 1" segue stale, sinalizada aqui. **PRÓXIMO PASSO (re-disparo):** Fase 4 item mais barato = `--min-sdk/--target-sdk` no `CmdBuild` → `AndroidProjectWriter` (threading das constantes API_LEVEL/min-api do pom+manifest, com teste `AndroidInteropE2ETest` novo); depois ícone por metadado; `--aab` só com bundletool disponível (gap honesto senão). Re-varredura doc×código a cada land. **NÃO TOCAR:** `docs/development/DECISIONS.md` (lane .17); §252/§253-B (nat .18); §258 (development .18); #266/#259/#275 (mantenedora).

> **✅ FEITO (17/09 ~11:20, dono = 192.168.100.15, lane bugs-and-gaps): varredura de versão corrente stale 0.2.6-beta → 0.4.0-beta + placeholders riscv/aarch + HTTP002 Native (EN+PT, 16 arquivos, `fbc8673a` + este commit).** Claim de ESTADO ATUAL corrigido em: `CONTRIBUTING`(header/testes 810→2218/CLI 26/backend full core/stdlib/in-dev+planned), `AGENTS`(syntax 0.3.0→0.4.0-beta), `architecture.md`+`performance.md`(marcadores de seção; exemplo de baseline 810 rotulado histórico), `stdlib/*`(security §7 810→2218, config/database/logging/web/stdlib/http/observability), `comparison/kof-vs-java`(tabela resumo: `native.risc/arm` full core, spawn/await CONC001/CONC003, Native HTTP, observability; header), `runtime/CURRENT_STATE`+`MEMORY_MODEL`, `learn/33`(exemplo 0.4.0-beta). **Segundo achado:** `learn/30-contributing`+`learn/README`(EN+PT) ainda diziam `riscv64/aarch64 placeholders` e `kof.http (JVM+JS, HTTP002 Native)` → full core (free-list+mark-sweep) e `kof.http` JVM+Native+JS (HTTP002 é branch MORTO, `KofHttp.supportedOn` sempre true; https/DNS seguem gaps). Notas datadas "Updated (0.2.6-beta, 31/08)" preservadas como provenance (data de introdução). Prova: `scripts/docs-lang.sh check` 0 par / 0 switcher / 0 órfão.
...
> **PRÓXIMO PASSO (16/09 ~23:35 local, dono = 192.168.100.15, lane bugs-and-gaps):** D-DIAG-EN AVANÇOU: 4 batches de código landed (`9bc9f60f` parser PARSE073/076/078/085/094, `8edfe970` switch SEM031/032/035, `d993a6f4` runtime/CLI await/handle/frame-diagnostics/bench, `621ce9fd` coleções SEM029/033/034/055/056 + TargetMatrix WASM001) + doc-sync da linha que estava nomeada (`known-bugs.md:5859` SEM033 → EN, `13343543`). **Batch 6 = próximo escopo:** `ExpressionInstanceCallLowerer` (8 strings PT) + `PrimitiveNumericFormatters` ("Kof não tem método") + `CompilerImports` ("não corresponde ao diretório do import"/"não encontrado no módulo") + `ReturnPathAnalyzer` ("declara retorno X mas pode terminar sem return/throw") — antes de cada arquivo: grep de testes asserindo o texto PT (padrão: todos os 5 batches até aqui tiveram zero-test-assert-casando OU assert reforçado); depois doc-sync EN das citações que mudarem; prova = 1 assert EN/ausência-PT por família; suíte completa + `docs-lang.sh check` antes do push. NÃO COLIDIR com footprint `.22`/`.17`: MemberCallTyper/SemExpressionTyper/StatementAnalyzer/SymbolTableBuilder/TypeChecker/MethodCallTyper/ExpressionTyper FORA (typer cluster #334/#331/#327); Translate*/CmdEditor/LspHover CLI (160+ strings) = unidades próprias depois. `SEM030` (learn/04): código ainda PT em outro sítio — verificar no batch 6. Política de citação PT-vs-EN nos docs `.pt_BR` (3588/5853/3939 citam tooling PT) = **rule 6, adjudicar com a mantenedora**, não mudar unilateralmente. NÃO tocar `docs/status.md` (lane `.22`).
> **🔀 HANDOFF → lane compiler (17/09 ~08:30, dono = 192.168.100.15, lane bugs-and-gaps): §262 face (b) `nullableRecord == nullableRecord` (sem literal `null`) NPE — ENTREGUE à lane compiler.** Face (a) (literal `null`) está FIXA (`RecordNullableNullEqE2ETest` 6/6). Face (b) foi medida cross-target nesta sessão: JVM `NullPointerException: Cannot invoke "Point.equals(Object)"`, JS `TypeError: Cannot read property 'equals' of null`, SCRIPT red — bug real de paridade. Fix = igualdade de conteúdo null-safe no ramo de record (`ExpressionBinaryLowerer` ~l.326-345; hoje emite `KofCall(recordType,"equals")` sem guarda de receptor nulável); precisa de helper/desugar compartilhado nos 4 alvos (`kofValEq` existe no runtime JS, `Objects.equals` no JVM, sem equivalente Native). **Bloqueado nesta máquina por falta de qemu** (Native não provável) → por isso NÃO foi fixado aqui (regra 5: nada de fix JVM-only). Dossier completo: `docs/bugs-and-gaps/known-bugs.md` §262 (repro + raiz + medição). Lane compiler assume com o toolchain nativo para provar os 4.

> **✅ FEITO (17/09 ~06:10, dono = 192.168.100.15, lane bugs-and-gaps): §325 doc×código + símbolos `Parser.*` pós-split (EN+PT).** Ao revisar os lands novos, achei drift real: (1) `docs/language-reference/classes.md`/`grammar.md` mostravam a ordem `implements` ANTES dos componentes do record (ordem java), mas a forma canônica do Kof (e o que o #325 passou a aceitar) é componentes primeiro — EBNF corrigida + bullet "(#325)"; (2) **símbolos `Parser.*` obsoletos** após o REFACTOR-500: `Parser.parseTypeDeclaration`/`parseModifiers`/`parseRecordBody`/`parseEntity` → `TypeDeclarations.*`; `StatementParser.parseSwitch` → `parseSwitchStatement`; `ExpressionParser.parseSwitchExpr` → `parseSwitchExpression`; `TypeParser.parseFunctionType` → `parseFunctionTypeRef`; `CompilerDriver.selectBackend` → `CompilerPipeline.selectBackend`; `CompilerDriver.expandKofImports` → `CompilerImports.expandKofImports`; `JvmBackend.arrayTypeForType` → `JvmLiteralEmitter.arrayTypeForType`; `Optimizer.foldUnary` → `FieldConstantFolder.foldUnary`; `JvmOpCollections.emitRuntimeCall` → `emitKofRuntimeCall`. Prova: varredura de símbolos `Classe.metodo` em docs/learn/training × declaração real = 0 residuais dessa lista; `docs-lang.sh check` 0/0/0.
...
> **NEXT STEP (fila da lane docs/development `192.168.100.22`, atualizado 16/09 ~14:10):** ao re-trigger: (1) `git fetch` + `git diff --name-only 9572949f..origin/beta-0.4.0 -- '*.java' '*.c'` — se o tip moveu **código** (novo @Test/remoção), re-medir a suíte em CLONE FRESCO (`git clone -s`, nunca worktree — armadilha skip-worktree, anti-pattern 2ª face) e sincronizar os 12 registros EN+PT (AGENTS×2, status×2, backend-parity×2, development/README×2, ecosystem-coverage×2, known-bugs×2) + linha neste mesmo commit. Base: 2218 = 1911+38+7+262 (clone limpo tip `9572949f`, 15:54, 0 falhas; flake §252 calado 3x (11:38/15:09/15:54), ~1/4). (2) Se **fechamento alheio** limpar um registro (gate CodeQL verde → §258/#773/#775/#776; §252 nativo; D-NULL-INTENT `.22` commitado c/ testes), atualizar o registro na mesma unidade. (3) Se nada disso e nenhum gap-documental real (varredura R6 de 16/09 esgotou CRON001/SG-002-sealed/GC/UUID/DB001-DB002/learn18/learn30/learn28/orderBy-where (architecture:141)/status-Targets-GC/DECISIONS-frentes56/§258-#777/CLUSTER-NULL-CONTRATO-A-F (AGENTS,learn13,idioms,records,§262,type-system,fake-idioms,training/language*,migration,learn04,statements,syntax; resíduo-zero no grep `= null`) — candidatos verificados HONESTOS e não-tocados: kof.concurrent.Queue planned (zero código), kof.rest planned, 26/26 snapshot datado, SUP001/AND001/MQ001/ORM001/PROC001 vivos no código, tabela 3.8 Batch scheduling — próximos candidatos só com MOVIMENTO de código), **RECUSAR o re-trigger** (condição STABILITY da AGENTS.md): registrar "estável — re-trigger recusado em <data>" e `scripts/auto-loop.sh stop`.
> **✅ FEITO (16/09 ~18:20, dono = 192.168.100.22, lane issues/parsers): ESCOPO NOVO (pedido da mantenedora) — TRIAGEM DA ENXURRADA DE ISSUES + #330 FIXADO.** 60 issues abertos 16/09 (56 jvm/48 bug). **Triagem sistemática:** extraí o reproducer de CADA issue e rodei contra o jar de clone limpo da ponta (`9572949f`; código == `3e4f0da6`) — `/tmp/opencode/issue-triage/{issues,repros,results,evidence}.json`. **21 FECHADOS como `kof-agent-worker[bot]` com evidência medida** (não reproduzíveis na ponta: #283 #286 #287 #288 #290 #292 #299 #300 #301 #303 #305 #306 #308 #309 #311 #312 #316 #320 #329 #335 + #330 abaixo) — relatórios feitos contra build antigo; os fixes (D-PRINT 58561d1f, D-ENUM207 fatias, §231 e4f1fb65, erasure/checkcast/boxing) já estavam na branch. **⚠️ COLLISION MAP (para o agente do merge nullable):** o worktree compartilhado tem EM ANDAMENTO exatamente o cluster nullable-primitive (#259 #266 #278 #279 #280 #281 #289 #296 #297) — NUNCA toquei nesses 17 arquivos `.java` staged nem nos issues do cluster. Meu fix #330 foi em **clone isolado** (`/tmp/opencode/kof-tip5`, branch sync-beta/issue-330) sem pisar no merge alheio. **FIX #330 (`80b04b10`):** `var fn/fun/func = 42` dava PARSE037 genérico; agora `expectId` emite PARSE085 canônico na posição do keyword (spec SG-001 já pedia — só faltava o caminho). Prova: FunctionSyntaxTest 15/15 (+3 testes novos no MESMO commit); suíte compiler completa sem falha nova (só flake §252). Fechado com comentário técnico. **ABERTOS RESTANTES (39, por área):** parser-diags (#324 AST-toString+PT em msgs, #325 record+implements PARSE007, #315 cast-C-style SEM011), semantic-checks silenciosos (#321 #323 #326 #327 #331 #333 — aceitam e quebram no runtime; regra 6? NÃO: emitirem diagnóstico SEM é aditivo — freeze só proíbe MUDAR código que compila hoje), lowering JVM (#293 #294 #295 #298 #302 #304 #310 #313 #328 #332 #334 #336), docs/feature (#275 #273 flake §252). Próximas unidades candidatas (sem colisão com nullable-lane): #324 → #325 → #333 → #327/#331 (par access-checks).\n> **✅ CONCLUÍDO (16/09 ~18:20, dono = 192.168.100.22, lane issues/parsers): ESCOPO NOVO (pedido da mantenedora) — TRIAGEM DA ENXURRADA DE ISSUES + #330 CONSERTADO.** 60 issues abertos 16/09 (56 jvm/48 bug). **Triagem sistemática:** extraí o reproducer de CADA issue e rodei contra o jar de clone limpo da ponta (`9572949f`; código == `3e4f0da6`) — `/tmp/opencode/issue-triage/{issues,repros,results,evidence}.json`. **21 FECHADOS como `kof-agent-worker[bot]` com evidência medida** (não reproduzíveis na ponta: #283 #286 #287 #288 #290 #292 #299 #300 #301 #303 #305 #306 #308 #309 #311 #312 #316 #320 #329 #335 + #330 abaixo) — relatórios feitos contra build antigo; os fixes (D-PRINT 58561d1f, fatias D-ENUM207, §231 e4f1fb65, erasure/checkcast/boxing) já estavam na branch. **⚠️ MAPA DE COLISÃO (para o agente do merge nullable):** o worktree compartilhado tem EM ANDAMENTO exatamente o cluster nullable-primitive (#259 #266 #278 #279 #280 #281 #289 #296 #297) — NUNCA toquei nesses 17 `.java` staged nem nos issues do cluster. Meu fix #330 foi em **clone isolado** (`/tmp/opencode/kof-tip5`, branch sync-beta/issue-330) sem pisar no merge alheio. **FIX #330 (`80b04b10`):** `var fn/fun/func = 42` dava PARSE037 genérico; agora `expectId` emite PARSE085 canônico na posição do keyword (a spec SG-001 já pedia — só faltava o caminho). Prova: FunctionSyntaxTest 15/15 (+3 testes novos no MESMO commit); suíte compiler completa sem falha nova (só flake §252). Fechado com comentário técnico. **ABERTOS RESTANTES (39, por área):** parser-diags (#324 AST-toString+PT em msgs, #325 record+implements PARSE007, #315 cast-C-style SEM011), checagens semânticas silenciosas (#321 #323 #326 #327 #331 #333 — aceitam e quebram no runtime; regra 6? NÃO: emitir diagnóstico SEM é aditivo — o freeze só proíbe MUDAR código que compila hoje), lowering JVM (#293 #294 #295 #298 #302 #304 #310 #313 #328 #332 #334 #336), docs/feature (#275 #273 flake §252). Próximas unidades candidatas (sem colisão com a lane nullable): #324 → #325 → #333 → #327/#331 (par de access-checks).\n> **✅ FEITO (16/09 ~17:45, dono = 192.168.100.22, lane docs/development): UNIDADE I-D + §263 CATALOGADO — a verdade do cluster let/const: remoção PARCIAL (furo geral do parser).** Medição nova no jar do tip (clone limpo `9572949f`): `let x = 5` morre (SEM011 — como a unidade I ensinou), MAS `let x: Int = 5` **IMPRIME `5`** — e `Klaxon x: Int = 5`/`Banana q: String = "z"` também. Raiz: `parseVarDecl` aceita QUALQUER IDENT como tipo (`StatementParser:413-415`) e a anotação `: Tipo` (:417-421) **descarta o prefixo em silêncio** (R6 violado); `async f(): Int` passa no check. Sem anotação falha certo → o teste `jsSugarIsRejected` é verde só por falta de anotação. Catalogado **§263** (EN+PT, tabela+corpo, 6 casos medidos; conserto = lane compiler, regra 6 — mudança de contrato é da mantenedora, não toquei o parser). Células I-D corrigidas: `training/reference/{compiler,targets}` EN+PT (claims "let/const → KofScriptGlobals" → forma real var/val + nota §263). NOTA: §260 gdb-3 chegou só na EN (`cf953a59`, lane `.17`) — PT espelha na prox unidade docs. ⚠️ incidente desta unidade: 3 tool-calls de escrita PARALELOS no MESMO known-bugs.md corromperam o arquivo (3 linhas §263 PT duplicadas + corpos perdidos) — reparado no mesmo commit; LIÇÃO: NUNCA mutar o mesmo arquivo em chamadas paralelas. Prova: r1-r10+kf1-kf4 medidos (/tmp/opencode/nulltest/ks5); docs-lang 0/0/0. Commit com pathspec .md (índice compartilhado com .22).\n> **✅ FEITO (16/09 ~16:55, dono = 192.168.100.22, lane docs/development): UNIDADE I-B+C — cluster let/const-sugar KofScript COMPLETO (18 células, 9 arquivos EN+PT).** Depois do learn/04 (A/`3281def0`): learn/00:48, learn/02:134-148 (DEMO INTEIRO reescrito: let/const+main → var/val+statement solto — forma provada `Mel 3.14`/`Mel`), learn/27:12, learn/glossary:90 (+ leak null cluster :92 `String? x = null` → API — o filtro != null da unidade F deixou passar; corrigido aqui), learn/README:163, learn/native/README:73, learn/29-compiler-internals :217/:227/:239. BÔNUS: `docs/decisions/planning-mutability.md:156` (linha de plano que PROMETIA o alias let→var — nunca existiu; pós-183cb048) ganhou NOTA DE REGISTRO (histórico preservado, não reescrito — padrão datado). NÃO-tocados com razão: learn/32:83 (diz a verdade), KofScriptTest (código). Resíduo-zero no grep let/const-vivo fora das anotações. Prova: i1.ks=Mel, w1.ks=Mel 3.14, t2.ks=PARSE011/SEM011 medidos no jar do tip; git show 183cb048; docs-lang 0/0/0.
> **✅ FEITO (16/09 ~17:45, dono = 192.168.100.22, lane docs/development): UNIDADE I-D + §263 CATALOGADO — a verdade do cluster let/const: remoção PARCIAL (furo geral do parser).** Medição nova no jar do tip (clone limpo `9572949f`): `let x = 5` morre (SEM011 — como a unidade I ensinou), MAS `let x: Int = 5` **IMPRIME `5`** — e `Klaxon x: Int = 5`/`Banana q: String = "z"` também. Raiz: `parseVarDecl` aceita QUALQUER IDENT como tipo (`StatementParser:413-415`) e a anotação `: Tipo` (:417-421) **descarta o prefixo em silêncio** (R6 violado); `async f(): Int` passa no check. Sem anotação falha certo → o teste `jsSugarIsRejected` é verde só por falta de anotação. Catalogado **§263** (EN+PT, tabela+corpo, 6 casos medidos; conserto = lane compiler, regra 6 — mudança de contrato é da mantenedora, não toquei o parser). Células I-D corrigidas: `training/reference/{compiler,targets}` EN+PT (claims "let/const → KofScriptGlobals" → forma real var/val + nota §263). NOTA: §260 gdb-3 chegou só na EN (`cf953a59`, lane `.17`) — PT espelha na prox unidade docs. ⚠️ incidente desta unidade: 3 tool-calls de escrita PARALELOS no MESMO known-bugs.md corromperam o arquivo (3 linhas §263 PT duplicadas + corpos perdidos) — reparado no mesmo commit; LIÇÃO: NUNCA mutar o mesmo arquivo em chamadas paralelas. Prova: r1-r10+kf1-kf4 medidos (/tmp/opencode/nulltest/ks5); docs-lang 0/0/0. Commit com pathspec .md (índice compartilhado com .22).\n> **✅ FEITO (16/09 ~16:55, dono = 192.168.100.22, lane docs/development): UNIDADE I-B+C — cluster let/const-sugar KofScript COMPLETO (18 células, 9 arquivos EN+PT).** Depois do learn/04 (A/`3281def0`): learn/00:48, learn/02:134-148 (DEMO INTEIRO reescrito: let/const+main → var/val+statement solto — forma provada `Mel 3.14`/`Mel`), learn/27:12, learn/glossary:90 (+ leak null cluster :92 `String? x = null` → API — o filtro != null da unidade F deixou passar; corrigido aqui), learn/README:163, learn/native/README:73, learn/29-compiler-internals :217/:227/:239. BÔNUS: `docs/decisions/planning-mutability.md:156` (linha de plano que PROMETIA o alias let→var — nunca existiu; pós-183cb048) ganhou NOTA DE REGISTRO (histórico preservado, não reescrito — padrão datado). NÃO-tocados com razão: learn/32:83 (diz a verdade), KofScriptTest (código). Resíduo-zero no grep let/const-vivo fora das anotações. Prova: i1.ks=Mel, w1.ks=Mel 3.14, t2.ks=PARSE011/SEM011 medidos no jar do tip; git show 183cb048; docs-lang 0/0/0.
> **✅ FEITO (16/09 ~16:35, dono = 192.168.100.22, lane docs/development): UNIDADE I — NOVO CLUSTER `let`/`const`-sugar KofScript (A/4: learn/04, EN+PT).** Varredura que começou na célula null do learn/04 achou um SEGUNDO cluster de ensino-morto: o sugar `let`/`const` do KofScript foi REMOVIDO em `183cb048` (06/09, "KofScript é Kof puro, sem sugar de outra língua") mas ~16 células no `learn/` ainda o ensinam como funcional. MEDIDO no jar do tip (`9572949f`): `let x=5` em modo script → `SEM011`/`PARSE011` (morto); a forma real `var nome=...` solto no topo → wrapper gera `class KofScriptGlobals{static...}` + `main()` (i1.ks→`Mel`, i2.ks→`3`, ambos verdes). CONTRADIÇÃO interna: `learn/32-cli-tooling:83` e o teste `KofScriptTest.jsSugarIsRejected` (verde na suíte) já dizem a verdade — as outras células são os outliers. learn/04 corrigido (3 blocos EN+PT: intro :11, seção let/const→"NÃO existem", status :182). ⚠️ também medido: a classe citada em todo o corpus é `KofScriptGlobals` (inner class gerada no wrapper) — NAO `ScriptGlobals` (a classe pública real em kof-script/src/main é `dev.kof.script.ScriptGlobals` com MAP; o `KofScriptGlobals` do doc é o nome da classe gerada — coerente, não corrigi). FILA do cluster (próximas unidades): B) learn/00:48 + learn/02:136 + learn/27:12 + learn/glossary:90 + learn/README:163 + learn/native/README:73 (claims "let/const viram KofScriptGlobals"); C) learn/29-compiler-internals :217/:227/:239 (tabela lexer/KofScript); D) fake-idioms/tabelas que listam let/const como válidos (conferir). NÃO-tocar: learn/32, KofScriptTest. Prova: i1/i2/t2/t1 medidos + git log 183cb048; docs-lang 0/0/0.
> **✅ FEITO (16/09 ~16:10, dono = 192.168.100.22, lane docs/development): CORREÇÃO — meu commit `4cd0217a` contaminou a beta com o teste órfão da `.22` (regra Q0, prioridade zero; transparência de erros — inclui meu erro).** Root: o commit da re-medição rodou com um `.java` de `src/test/java` da lane `.22` (D-NULL-INTENT N1, feature NÃO-fundida, branch `pr277`) JÁ staged no índice compartilhado; meu `git stash` protegia só `main/java/dev/kof/compiler/`, NÃO `src/test/java` → o `git commit` capturou o órfão. Efeito: beta com `NullablePrimitiveE2ETest` = 7 testes, 4 VERMELHOS (o main boxed não está na beta) → contradiz o '2218/0 falhas' sincronizado = build quebrado no remoto. Conserto: `git rm --cached` do órfão (o ARQUIVO FICA NO DISCO p/ a `.22` continuar; blob `dbce44e7` permanece alcançável em `4cd0217a` no remoto + a `.22` tem `7d2317ab` em `pr277`). A árvore volta a ser exatamente o conjunto testado em `9572949f` = 1911 compiler = 2218 válidos (docs não-addem teste; NÃO re-medido: a árvore restaurada É a medida às 15:54, e remover 1 teste standalone não recompila os outros). LIÇÃO PARA O CONTRATO: ANTES de commitar, `git diff --cached --name-only` TEM de listar SÓ os `.md` meus — se aparecer `.java`, tirar do índice (`git restore --staged`). O índice é compartilhado entre as sessões do MESMO worktree; `git add <pathspec.md>` não 'protege' contra um `.java` alheio já staged por outra sessão.
> **✅ FEITO (16/09 ~15:54, dono = 192.168.100.22, lane docs/development): RE-MEDIÇÃO #2 no tip `9572949f` → 2218, 12 registros EN+PT sincronizados (contrato passo 1 disparado por `92d11a03`).** O tip moveu CÓDIGO pós-`97425541`: G-6b GC x86 (`92d11a03`, +3 @Test NativeX86GcMarkScopeTest sem guard → roda no host) + #773 bound-check (`c2300df6`, teste sem delta @Test: 6→6). Clone FRESCO `/tmp/opencode/kof-tip5` (disco=HEAD): **2218 run (1911+38+7+262), 0 falhas / 0 erros / 192 skip, BUILD SUCCESS** — previsão 2215+3 exata (delta reconciliado). §252: flake CALADO pela 3ª corrida (09:44 disparou; 11:38/15:09/15:54 calou → frequência revista ~1/4, não ~1/2 — atualizado na bullet §252 EN+PT + linhas AGENTS×2/status×2/parity×2/README×2/ecosystem×2 + base do NEXT STEP). Docs-only. Prova: tip-suite5.log; docs-lang 0/0/0.
> **✅ FEITO (16/09 ~15:35, dono = 192.168.100.22, lane docs/development): UNIDADE H — sincronização §260 pós-G-6b (contrato passo 2, EN+PT).** `92d11a03` (.17) fechou a causa-1 do §260 (G-6b: mark x86 varre a pilha inteira da thread, guard `NativeX86GcMarkScopeTest` 3/3) e atualizou o CORPO EN mas deixou (a) a LINHA DA FILA (linha 20, escrita por mim na unidade E como causa-única-registrador) desatualizada e (b) o PAR PT inteiro defasado. Sincronizado: fila EN+PT vira 🟡 PARTIAL (causa-1 CLOSED `92d11a03`, causa-2/G-6(a) + gatilho OFF abertos) + bloco CAUSA REAL/Árvore copiado p/ a PT (a .17 redigiu o update já em português no EN — copia fiel, sem inventar). Título da seção NAO alterado (conteúdo da .17). Prova: diff do commit lido + grep dos 3 tokens novos nos dois lados; docs-lang 0/0/0. ⚠️ CONTRATO passo 1 pendente: `92d1a03` moveu CÓDIGO pós-medição 2215 (+3 @Test NativeX86GcMarkScopeTest sem guard → roda no host; previsão 2218). Próximo tick: re-medir clone limpo do tip novo.
> **✅ FEITO (16/09 ~15:25, dono = 192.168.100.22, lane docs/development): UNIDADE G — gate re-rodado pós-fechamento #773 (contrato passo 2): o fix do #777 era INSUFICIENTE, #780 no MESMO sítio.** `c2300df6` (.18) fechou #773/#777 → gate re-rodado: agora **3 abertos** — #775/#776 (mesmos) + **#780 novo em `KofWebJsE2ETest:307`**. Root: o guard `(?i)[0-9a-f]+` do fix #777 aceita token de >8 hex dígitos → `Integer.parseInt(...,16)` lança NFE sem catch — GOLDEN medido com java real (`"fffffffff"` matches + NFE "under radix 16"), mesma classe do #777. Catalogado (fila linha 17 + bloco UPDATE da seção §258, EN+PT) com dono = `.18`/SSE (regra 8: arquivo alheio; o #777 foi "fix" que não cobriu overflow). NÃO consertado aqui. Registro de fechamento alheio §258 já estava sincronizado pela `.18` (linha 17+corpo EN+PT) — só faltava o novo alerta. Prova: `codeql-gate.sh --fast` rodado + linha 307 lida + `javac/java` T.java; docs-lang 0/0/0.
> **✅ FEITO (16/09 ~15:09, dono = 192.168.100.22, lane docs/development): RE-MEDIÇÃO no tip `97425541` → 2215, 12 registros EN+PT sincronizados (contrato passo 1).** O tip moveu CÓDIGO pós-`803eeef4`: `4ea099b3` (§261 window-bind, +1 @Test no KofJsBrowserE2ETest 25→26) e `06e77e94` (§255, guard de sysroot → KofDbE2ETest skip 3→4). Medido em CLONE FRESCO (`/tmp/opencode/kof-tip4`, disco=HEAD): **2215 run (1908 compiler + 38 script + 7 kof-c + 262 cli), 0 falhas / 0 erros / 192 skip, BUILD SUCCESS** — previsão 2214+1 exata (delta reconciliado por report: KofJsBrowser run+1/skip+1, KofDb skip+1). §252 flake CALADO de novo (3ª corrida: 11:38 e 15:09 silenciosas, 09:44 disparou → ~1/2 mantém). Sincronizados AGENTS×2 (:729,:1091-1118), status×2 (:87/:579), backend-parity×2 (:13), development/README×2 (:6-10), ecosystem-coverage×2 (:14), known-bugs×2 (nova bullet §252 15:09); histórico datado (11:38/09:44/03:20) PRESERVADO. Bonus drift consertado: a linha do status/README dizia "0 regressions (1 failure = flake)" implicando falha nesta corrida (0) — reescrita p/ "0 regressions / 0 errors, flake intermittent ~1/2 silent this run". NÃO tocado: KofDb.java javadoc (858d8c36=doc-only, já validado). Prova: tip-suite4.log + reports KofDb/KofJsBrowser (grep); docs-lang 0/0/0.
> **✅ FEITO (16/09 ~17:05, dono = 192.168.100.22, lane docs/development): UNIDADE F — RESIDUÁRIO do cluster null-contrato no training/learn/language-reference (12 células, 8 arquivos EN+PT; regra Q4/5 — varredura exaustiva pós-cluster).** O grep `? x = null` no corpus vivo achou o que A–D não cobria: `training/language/{syntax,strings,types}`, `anti-patterns/{runtime-workarounds,fake-idioms}`, `migration/java-to-kof`, `learn/04`, `docs/language-reference/{statements,syntax}` — todas ensinavam o literal proibido (SEM048 desde 10/09, medido na unidade A). Reescritas para a forma API (`mapOf().get()`/`find()`) + nota SEM048. BÔNUS medido (F1–F3 no jar do tip): o erro de ATRIBUIR `T?` p/ `T` sem check é **SEM021** (não SEM014 como 4 células do training alegavam); narrowing só vale DENTRO do then-branch (G3 → `abc` ok; fora → SEM021). SEM014 legítimo (aridade/args, tabela type-system:327) intocado. Prova: F1=SEM021, F3=SEM021, G3=abc, G1/G2=SEM021; docs-lang 0/0/0. RESIDUÁRIO ZERO no grep. NÃO-ESCOPO achado e registrado (não corrigido): o bloco KofScript de `learn/04` usa `let x = 5` + `s.length()` — o modo `script` do tip dá PARSE011 até em `let x = 5` puro (medido ks2) → a seção `KofScript let/const (0.2.0)` pode estar pré-contrato; é frente da lane KofScript/compiler, não da doc — fila candidata p/ sweep futuro com medição própria.
> **✅ FEITO (16/09 ~16:45, dono = 192.168.100.22, lane docs/development): UNIDADE E — sincronização de registro §260 (fila EN+PT + seção na PT).** A lane `.17` comitou `4b2b14e2` (catalogação §260) com duas lacunas: (a) linha de fila OPEN ausente em AMBOS os cabeçalhos; (b) seção só no arquivo EN (que aliás está escrita em PT — a `.17` redigiu a medição em português dentro do known-bugs EN; reescrita p/ EN = trabalho de conteudo da lane autora, NAO fiz — a PT recebeu a copia fiel da medicao, que ja e PT). Corrigido: linha §260 na fila EN+PT (antes de §261) + secao completa na PT (com Status traduzido). Prova: `grep -c §260` PT = 2 antes era 0; docs-lang 0/0/0.
> **✅ FEITO (16/09 ~16:30, dono = 192.168.100.22, lane docs/development): UNIDADE D/4 — FECHA O CLUSTER null-contrato (type-system.md + fake-idioms, EN+PT).** D1) `docs/language-reference/type-system` §5 Nullable: "Deref de T? sem narrowing NÃO é erro/advisory (SG-005)" → **MENTIRA desde 10/09** — medido hoje no jar do tip: `var s:String?="x"; s.length` → `SEM049 receiver is nullable; narrow first` (SG-005 consertado em `9436da12`). "`Int? == null` falha em runtime (NPE)" → **MENTIRA hoje** — medido: map-miss `Int?` dá `not-null` (DOBRA silencioso — bug aberto #259/D-NULL-INTENT, §125 mede a dobra; §241 revertido p/ gap honesto); apontado + §262 (record-null `== null` → NPE, renumerado da colisão §261/window-bind no rebase). "literais `T?`" como fonte → **MENTIRA desde 10/09** (SEM048 medido) — linha reescrita: fontes = API + `return null` de função `T?`. D2) `fake-idioms:29` "Int? null safety ✅" sem ressalva → nota ⚠️ dobra #259/§125 + record-NPE §262 ("✅" = caminho String/class). CLUSTER A–D FECHADO (A=AGENTS, B=learn/13, C=idioms+§262, D=type-system+fake-idioms). RESTO DA FILA da lane docs: tradução §260 faltando na PT (drift da `.17` `4b2b14e2`) + sweep de novas células `= null`/advisory que aparecerem com movimento de código. Prova: D1/D2 medidos no jar do tip (SEM049; not-null dobra) + probes anteriores (SEM048, Q1 NPE, Q2/P1 ok); docs-lang 0/0/0.
> **✅ FEITO (16/09 ~16:00, dono = 192.168.100.22, lane docs/development): UNIDADE C/4 do cluster null-contrato — idioms/strings + idioms/records (EN+PT) + NOVO BUG §262 catalogado (com repros medidos).** Medições no jar do tip (clone limpo): `String? s = null`/`Point? maybe = null` → SEM048; `s.length` sem narrowing → SEM049 (a célula strings dizia "SEM014" — pré-10/09); formas API provadas: `mapOf("k","abc").get("k")`+narrowing → `3`, `m.get("k")` record → `7`. DESCOBERTA (regra Q4 — caçar o bug ao reescrever): **`Point? maybe = m.get("z")` (record MISS) + `== null`/`!= null` → NPE no JVM** ("Cannot invoke Point.equals(Object) because maybe is null") — raiz: `==` de record baixa para `.equals()` sem guarda de null (`CompilerComparisons:28,337-342` bug 188); narrowing de class/String ok (if_acmp). Catalogado §262 (EN+PT — colisao de numero: a lane `.18` tomou §262 para window-bind em `4ea099b3` no mesmo intervalo; meu registro foi renumerado no rebase, 4 casos medidos HIT/MISS/class/String; conserto = lane compiler `CompilerComparisons`, regra 8 respeitada — não toquei o lowering). Célula records.md EN+PT reescrita: forma HIT ensinada + nota ⚠️ MISS→§262. strings.md EN+PT: forma API + SEM049. ⚠️ OUTRO DRIFT achado: **§260 existe só na EN** (a lane `.17` comitou `4b2b14e2` sem espelhar PT) — fila para unidade própria (tradução fiel da medição; não fiz aqui pra não estourar a unidade). Unidade D pendente: type-system.md:143-151+fake-idioms:29 claims pré-10/09. Prova: 6 probes medidos (E1=3,E2=7,F1=SEM049,Q1=NPE,Q2=is-null,P1/P2 ok) + git log §260; docs-lang 0/0/0.
> **✅ FEITO (16/09 ~15:45, commit `5ecdfae1`, dono = 192.168.100.22, lane docs/development): UNIDADE B/4 do cluster null-contrato — `learn/13-nullability` (EN+PT).** O bloco "A solução" (:25) mostrava `String? apelido = null // pode ser null` como didática — SEM048 desde 10/09 (medido no jar do tip). Corrigido p/ `findNickname()` (forma API, o idioma real do capítulo — :33/:44 já usam `obterNome()`) + comentário "NUNCA via `= null`". A linha 17 (`String nome = null;`) é o exemplo JAVA do problema — preservada. Docs-only; linha DOING da unidade A (cd3e5334) já trazia a fila; esta linha fecha o B no mesmo padrão. docs-lang 0/0/0.
> **✅ FEITO (16/09 ~15:20, dono = 192.168.100.22, lane docs/development): UNIDADE A/4 do cluster null-contrato — `AGENTS.md` (EN+PT) ensinava código que dá ERRO hoje.** O guia obrigatório dos agentes (bloco Variáveis, :834-835) mostrava `String? nome2 = null` / `var idade: Int? = null` como idiomáticos — MAS o null-literal foi PROIBIDO pela decisão da mantenedora (SG-008, implementada 10/09 em `9436da12` como SEM048; D-NULL-INTENT 15/09 reafirma: null nunca chega silencioso). Medido hoje no jar do tip (clone limpo): `= null` dá `error: null cannot be assigned [SEM048]` em AMBOS os exemplos; a forma API (`String? x = find(k)`) roda (`Mel`/`30`). Reescrita: exemplos viram API + nota ⚠️ SEM048/D-NULL-INTENT. Bloco Null safety (:954) já estava correto (usa `find(key)` — não tocado). FILA DO CLUSTER: B) learn/13; C) idioms/strings:117 + records:160; D) type-system.md:143-151 + fake-idioms:29 (claims pré-10/09). Prova: medições no jar do tip (SEM048/SEM049/API-form narrowing-hit `8`) + git log -S (célula 6f40ac76 01/09; fix 9436da12 10/09); docs-lang 0/0/0.
> **✅ FEITO (16/09 ~14:45, dono = 192.168.100.22, lane docs/development): TICK NÃO-INVENTIVO (recusa registrada).** Contrato rodado de ponta a ponta: (1) tip sem código novo (só o javadoc KofDb já validado); (2) gate = os MESMOS 4 alertas de donos alheios (#773/.18, #775/.22, #776/§259, #777/SSE), nenhum fechamento desde 13:20; issues #278-#284 (novas, nullable/if-expr/lambda-crash) = código das lanes compiler `.22`/watcher — regra 8, batidas contra as células do corpus SEM overclaim na lane docs (ex.: nenhuma célula diz "lambda reassign ✅" nem "if-expr todos os targets" — a E2ETest IfExprBraces cobre o que claima e está verde); (3) contagem 2214 sem resíduo (grep 2211/1904 = 0 fora dos 12 registros). Nenhuma unidade nova sem violar regra 6/8 — o próximo trabalho REAL da lane só aparece com movimento de código ou fechamento alheio. Heartbeat MANTIDO (STABILITY global não se aplica: §252/§253/§255/§258/§259 seguem abertos em outras lanes).
> **✅ FEITO (16/09 ~14:05, dono = 192.168.100.22, lane docs/development): tabela "Targets" do `docs/status.md` (EN+PT) — célula "GC mark pending" corrigida (era FALSA como estado atual; escrita 27/08 em b52a509d sem data na linha).** Mark-sweep landou 03/09 (`KofGcE2ETest` 3/3, stdlib.md:119); reescrita: "free-list alloc + mark-sweep GC (03/09; auto-coleta pendente — §260)". NÃO tocada com razão: "26/26" nas linhas riscv/aarch = snapshot datado (02-05/09, honesto — a contagem atual 2×42 já vive em AGENTS/README/§252 com data). kof.rest ⏳ planned verificado honesto (zero código KofRest). Docs-only; prova: stdlib.md:119+§260+grep do código antes; docs-lang 0/0/0. Linha neste mesmo commit.
> **✅ FEITO (16/09 ~13:55, dono = 192.168.100.22, lane docs/development): `training/idioms/architecture` (EN+PT) — célula "orderBy/multiple where pending" removida (era FALSA).** Ambos implementados 02/09 (`a5b25cc3`: `CompilerOrmSupport:153-167` emite ORDER BY + concat AND de wheres; `KofOrmE2ETest.queryDslFiltersOrdersAndLimits`/`queryDslMultipleWhereAnds` verdes nos 1907 do compiler medidos hoje 11:38). A célula de 14/09 contradizia `database.md:26` (que já ensina orderBy como existente) — auto-contradição no corpus resolvida. Não-tocado: roadmap §5 "User.query { orderBy }" (visão conceitual futura, ok). Docs-only; prova: código+testes lidos antes; docs-lang 0/0/0. Linha neste mesmo commit.
> **✅ FEITO (16/09 ~13:45, dono = 192.168.100.22, lane docs/development): frentes 5/6 do DECISIONS.md (D-DEV-PRIORITY) dessincronizadas do código — gap-codes corrigidos (EN+PT).** A célula :1024-1026 (decisão `ded04b6f`, 01:25) apontava "ws/sse/TLS/path-params = **WEB001**" num bucket só; o canônico da própria mantenedora (`backend-parity:92`, atualizado por ela HOJE `803eeef4` 11:14) e o gate real (`ExpressionBuiltinInstanceCalls:69-92`) enumeram por feature: ws=**WEB004**, TLS=**WEB002**, sse=**WEB003** (JS handler-scoped ✅ hoje `7cd69a7b`; push pós-return é o residual), path-params/keep-alive=**WEB001**. Reescrita alinhada ao canônico com ponteiro para a linha. NÃO é mudança de decisão (a fila e a ordem da mantenedora intactas) — só o catálogo de gap-codes que a decisão citava de memória. Docs-only; prova: gate + backend-parity:92 lidos antes; docs-lang 0/0/0. Linha neste mesmo commit.
> **✅ FEITO (16/09 ~13:30, dono = 192.168.100.22, lane docs/development): §258 re-catalogado — 4º alerta NOVO no gate (#777).** Contrato (2): `codeql-gate.sh --fast` no tip revelou **#777 `java/uncaught-number-format-exception` em `KofWebJsE2ETest:302`** — o helper de-chunk do teste SSE introduzido HOJE por `7cd69a7b` (lane SSE da mantenedora) faz `Integer.parseInt(hex)` sem catch em input lido da rede. Catalogado (tabela :17 + corpo da seção §258, EN+PT) com ponteiro de dono = mantenedora/SSE; NÃO consertado (regra 8 — arquivo alheio, teste verde hoje; conserto = 1 linha in-file). Bônus: drift pré-existente na linha "Estado" PT sincronizado com a EN. Contagem 2214 permanece válida (único diff de código desde 803eeef4 = javadoc de KofDb.java em 858d8c36 — verificado). ⚠️ engano registrado: o commit `6585da5f` alegou na mensagem "linha DOING no mesmo commit" mas um SyntaxError no script de inserção deixou o DOING de fora — esta linha corrige no follow-up (histórico empurrado não se emenda). Prova: gate rodado + linha 302 lida + blame `7cd69a7b`; docs-lang 0/0/0.
> **✅ FEITO (16/09 ~13:15, dono = 192.168.100.22, lane docs/development): `training/idioms/database` (EN+PT) — célula "db no JS → DB001" atualizada pós-fechamento.** DB001-JS foi FECHADO 16/09 (esta lane, `574c9419`; `KofDb.java:24-25` "JS soporta kof.db não-tipado (DB001 fechado 16/09; tipado → DB002)"), mas a célula de gaps do idiom ainda listava `db` no JS como gap DB001. Reescrita: `db.query<T>` tipado no JS → `DB002` (não-tipado funciona; canônico backend-parity:94). `gapCode()` DB001 verificado VIVO para outros paths (ExpressionDbCallLowerer:43 — driver fora do SQLite) — por isso a célula DB001 não foi apagada, apenas reposicionada. Docs-only. Linha neste mesmo commit; docs-lang 0/0/0.
> **✅ FEITO (16/09 ~13:05, dono = 192.168.100.22, lane docs/development): `learn/30-contributing` + `learn/README` (EN+PT) — células de "em desenvolvimento/gaps" desenterradas.** As duas listas de frente carregavam 3 staleness: (1) "GC mark-sweep pendente/completo" → landou 03/09 (`KofGcE2ETest` 3/3), auto-coleta pendente é §260; (2) "scheduler Native (SCHED001)" → FECHADO 31/08; (3) "HTTP002 (HTTP client Native)" → http/1.1 funciona 03/09 (`NativeHttpRuntime`), o que resta é https/DNS + timeout/retry/circuit silentes (§259); "WEB002 (web server nativo)" → server base roda 03/09 (`backend-parity:92`), WEB002 é TLS no Native. Formato riscado/✅ dos já-fechados mantido (mesmo estilo das células DB001/CONC003). Não-tocadas: PROC001 (código de lacuna vivo `ExpressionProcessCallLowerer:28`), ORM001, AND00x (verificadas — ainda abertas). Prova: backend-parity:92/93, §259 (`known-bugs:18`), stdlib.md:119 lidos antes; docs-lang 0/0/0. Docs-only, linha neste mesmo commit.
> **✅ FEITO (16/09 ~12:50, dono = 192.168.100.22, lane docs/development): `training/idioms/stdlib` (EN+PT) — célula UUID corrigida: "v7/ulid ainda não existem" era FALSO para v7.** `uuid.v7()` existe nos 5 targets (`KofUuid.java:42-43`, `KofUuidTest.uuidV7CrossArch` sob qemu fatia B25b 10/09; a conformance-matrix foi reafirmada HOJE pela lane `.15` em `9ed559ed` "UUID002 phantom"). Reescrita: v7 documentado com shape (dígito 14='7', 19∈{8,9,a,b}) + "ulid ainda não existe" mantido (único que ainda é verdade). learn/39 (EN+PT:164) corrigido no MESMO commit: "riscv64/aarch64 ficam atrás do gap UUID001 (fatia B pendente)" era FALSO — UUID001 fechou 09/10 (fatia B + tradutor, `uuidV4CrossArch`/`isUuidCrossArch` sob qemu; canônico PLAN-STDLIB-EXPANSION:45 "UUID001 closed"). Docs-only. Linha DOING neste mesmo commit; docs-lang 0/0/0.
> **✅ FEITO (16/09 ~12:40, dono = 192.168.100.22, lane docs/development): `learn/28-language-design` (EN+PT) — célula GC do Native atualizada.** Dizia "free-list GC, mark-sweep **pendente**" — o mark-sweep landou 03/09 (`KofGcE2ETest` 3/3, canônico stdlib.md:119); reescrita: free-list **com mark-sweep** + auto-collect na exaustão ainda PENDENTE (safe-points, §260 — recém-catalogado 16/09 pela `.17`). MySQL "em progresso" preservado (confere com stdlib-database:93 WIP). Docs-only. Prova: lidos stdlib.md:119 + §260 antes de reescrever; linha DOING neste mesmo commit; docs-lang 0/0/0.
> **✅ FEITO (16/09 ~12:30, dono = 192.168.100.22, lane docs/development): `fake-idioms.md` (EN+PT) ensina a armadilha `sealed`/`permits` (hábito Kotlin).** O cluster SG-002 corrigiu as células que AFIRMAVAM que sealed existe; faltava o anti-pattern: um agente traduzindo Kotlin escreve `sealed class X permits...` e trava. Adicionada linha ao bloco "não compila" (EN+PT): não-é-palavra-chave (SG-002 12/09), `sealed` → IDENTIFIER perdido → `PARSE010`, alternativa real `record`+`enum`+`interface`. Prova do estado: `CompilerDriverTest.deadTokensGiveCleanLexerError` (caso sealed→PARSE010, verde na suíte 2214 de hoje). Docs-only, linha neste mesmo commit, docs-lang 0/0/0.
> **✅ FEITO (16/09 ~12:25, dono = 192.168.100.22, lane docs/development): exatidão AGENTS (EN+PT).** A linha de medição dizia "0 regressões (1 = o flake do §252)" — implicava falha NESTA corrida, que foi 0 falhas. Reescrita: "0 regressões / 0 erros (a única falha que a suíte já mostrou é o flake intermitente do §252 — calado nesta corrida, disparou às 09:44)". Docs-only, 2 linhas.
> **✅ FEITO (16/09 ~12:15, dono = 192.168.100.22, lane docs/development): `learn/10-inheritance` (EN+PT) — célula `sealed` corrigida (regra "linha no MESMO commit", cumprida aqui).** A célula ~:88 dizia "a palavra-chave existe no lexer, mas o parser ainda não consome" — FALSO pós-SG-002 (12/09): `sealed`/`permits` foram REMOVIDOS do lexer (roadmap §2.5: DECIDIDO ADIAR). Reescrita (EN+PT): não-é-palavra-chave + `PARSE010` travado pelo teste + adiado por decisão + bump. learn/15 e learn/00 verificados e NÃO tocados (dizem a verdade). Prova: `CompilerDriverTest:4660-4664` (teste verde no lock da remoção, parte da suíte 2214) + SG-002 "APLICADA (provado 12/09)" + `grep -n sealed parser/Lexer.java` = só comentário; docs-lang 0/0/0.
> **✅ FEITO (16/09 ~12:05, commit `997d96c9`, dono = 192.168.100.22, lane docs/development): célula cross de `learn/18` (EN+PT) — gap da CI FECHADO 12/09 + contagem 66→84.** (Follow-up: a linha DOING entrou separada do commit — correção da regra "mesmo commit"; próximo tick anexa.) A célula ~:213 foi escrita 11/09 e ficou para trás: (1) dizia "instalar qemu na CI fecharia essa lacuna" — a lacuna foi FECHADA em 12/09 (`f2fee2f8`, job `cross-native` roda as duas suítes sob qemu); (2) "66 provas" → 2×42=84 na medição de 16/09 (+ nota "cresce a cada commit"); a execução datada 11/09 preservada como histórico. Prova: lido o job no `ci.yml` (80-111) + `grep -c @Test` (44+44, guard 2×42) antes de reescrever; docs-lang 0/0/0.
> **✅ FEITO (16/09 ~11:55, dono = 192.168.100.22, lane docs/development): re-medição da suíte no tip `803eeef4` → 2214, registros EN+PT sincronizados.** Contrato (3): o tip moveu código pós-`98ac26ed` (`7cd69a7b` feat SSE-JS, +3 @Test no compiler). Re-medido em CLONE FRESCO (`/tmp/opencode/kof-tip3`, disco=HEAD, guardas off): **2214 run = 1907 compiler + 38 script + 7 kof-c + 262 cli, 0 regressões / 0 erros / 190 skip, BUILD SUCCESS** — exatamente a previsão 2211+3. NOVO DADO DO §252: o flake ficou CALADO nesta corrida completa (0 falhas) — disparou às 09:44, calou às 11:38 → frequência ~1/2 registrada em §252 (EN+PT, nota nova acima da linha 09:44). 12 registros sincronizados (AGENTS×2, status×2, backend-parity×2, README development×2, ecosystem-coverage×2, known-bugs×2 — as duas últimas contêm também o dado do flake); README EN/PT reconciliado para "0 falhas nesta corrida". Prova: log da suíte (`/tmp/opencode/tip-suite3.log`) + `KofConcurrency2Test.txt` 41/0/3; docs-lang 0/0/0. Timestamps corrigidos para a hora REAL do fim da suíte (11:38, mtime do log) — não ~12:20. WIP alheio (9 .java da `.22`/D-NULL-INTENT) deixado intacto e NÃO-commitado (regra 8).
> **✅ FEITO (16/09 ~11:29→commit 739302de, dono = 192.168.100.22, lane docs/development): célula de ensino `CRON001` no idiom + security.md (EN+PT).** O registro CRON001 (`at(cron)` = stub 60s, cron IGNORADO nos 5 alvos — `RuntimeScheduler:18` "MVP igual JVM: ignora o cron, roda a cada 60s", confirmado no código) landou nos canônicos (status/backend-parity/stdlib/roadmap) pela `.18` em `daf20ceb`, mas a **célula de ensino** de `training/idioms/concurrency` (a que esta lane corrigiu ~11:20 p/ SCHED001) listava `scheduler.at("0 3 * * *") { ... }` como funcional SEM a ressalva — induzindo um agente a confiar no cron. Corrigida (EN+PT): at() marcado stub CRON001 + workaround idiomática no padrão do repo (`spawn { time.sleep(ms); job() }` — mesma forma do workaround §253/Toast). Espelho: célula "Scheduling" de `docs/stdlib/security.pt_BR.md` (dizia `every/at/cancel` ok e Native SCHED001 como gap) → SCHED001 fechado + ressalva CRON001. NÃO TOCADAS: `ecosystem-coverage:280` "scheduling PLANNED" é da tabela **3.8 Batch** (agendamento de pipelines, não kof.scheduler — correta); `PLAN-UNIVERSAL-PLATFORM:349` é future/ (ok). Docs-only. Prova: docs-lang 0/0/0. ⏳→✅ Re-medição concluída às ~11:55 (entrada acima): 2214, 0 falhas.
> **✅ FEITO (16/09 ~11:55, dono = 192.168.100.22, lane docs/development): sync pós-fechamento SSE-JS (`7cd69a7b`) — `security.md` corrigido (EN+PT).** O `.15`/`7cd69a7b` sincronizaram README/backend-parity/ecosystem-coverage(tabela)/status/stdlib-web, mas a tabela de capacidades de `docs/stdlib/security.md` (linhas 57–58, EN+PT) ficou para trás: (a) a célula **WebSocket** mapeava `JS WEB003` — errado: o gate (`ExpressionBuiltinInstanceCalls:69-88`) emite **WEB004** p/ ws no Native E no JS; (b) a célula **SSE** dizia "Native/JS WEB003" sem o handler-scoped JS de 16/09. Corrigidas p/ o canônico `stdlib-web:336`/`backend-parity:69`: ws = "Native/JS `WEB004`"; sse = "JVM + JS handler-scoped ✅ 16/09 (`7cd69a7b`); Native `WEB003`, resíduo de push pós-return no JS `WEB003`". Witness: só `kof_web_sse_route` liberado no gate JS; rota ws segue gated. Prova: `docs-lang.sh check` 0 par/switcher/orfão; grep ws∧WEB003 sem WEB004 separado = 0 (todos os hits corretos). Docs-only. ⚠️ `JvmTypeMapper.java` na árvore = WIP alheio NÃO-COMITADO da `.22` (D-NULL-INTENT N1, #259/#266) — DEIXADO INTACTO, o commit é cirúrgico só nos meus docs (regra 8: não é meu; a dona commita com a prova dela).
> **✅ FEITO (16/09 ~11:20, dono = 192.168.100.22, lane docs/development): sweep R6-documental continuado — 3 células stale corrigidas (EN+PT, 2 arquivos: idiom + stdlib), adjudicadas contra o CÓDIGO (regra: golden = medição real).** (a) `training/idioms/concurrency` (EN+PT, célula OTP): dizia "NATIVE = OTP001" e citava §129 como bloqueio — mas §129 foi **CORRIDO 15/09 (x86)** (`CompilerSupervisor:58-67` só emite OTP001 em riscv/aarch, mensagem "Nucleo OTP disponivel em JVM, Script e Native x86"); corrigida p/ "JVM+Script+Native x86 entregam o núcleo (✅15/09); riscv/aarch=OTP001, JS=OTP002". Witness interno: `stdlib:71` e `roadmap:248` já corretos — a célula de ensino era o outlier. (b) `docs/stdlib/stdlib` :119 (EN+PT, célula GC): afirmava "**auto-collect na exaustão** ✅ 03/09" — FALSO contra o commit `461ec3b4` ("sweep funcional, **auto-collect desligado**"), a NOTA do próprio `KofGcE2ETest` ("collect automático no alloc fica pendente — requer safe-points; sem root-map = double-free") e `backend-parity:98` ("auto-collect desligado"). Corrigida p/ "✅ sweep real 03/09 (`kof_gc_mark`+`kof_gc_sweep`+`kof_gc_collect_now` manual) ⚠️ auto-collect na exaustão ainda PENDENTE (safe-points)" — a linha :124 (mark-sweep riscada) está correta (refere à operação, não à automaticidade — mesma distinção do witness `status:725`). NÃO TOCADAS (com razão): `status`/`backend-parity`/`overview:94`/`learn/18` já espelham o estado; roadmap:248 = delta datado. Nenhum guard-test pinna essas células (só `ConcurrencyGapsDocTest` lê `learn/18`, intocado). Prova: `docs-lang.sh check` = 0 par/switcher/orfão; varredura final "exhaustion✅ sem pendente" = 0 hits em docs vivos. Docs-only.
> **✅ FEITO (16/09 ~10:55, dono = 192.168.100.22, lane docs/development): varredura R6-documental do blast-radius de fechamentos antigos — 16 células stale `TIME001`/`SCHED001`/`§253-A` corrigidas (EN+PT, 9 arquivos).** Células de ensino/índice afirmavam no PRESENTE que `time.interval`/`cancel` era "apenas JVM" e `scheduler` "JVM+JS, Native SCHED001 = gap" — mas o código fecha ambos (KofTime `supportedOn`=true + comentários "TIME001 fechado": JS 02/09 `5b12152c`, cross 05/09; SCHED001 Native 31/08 — `status:771`/`roadmap:268` "thread por job", cross 05/09; `KofTimeE2ETest.jsIntervalRunsPeriodicallyUntilCancelled`), e as células-espelho já corretas (status:361, backend-parity:53, stdlib:57) são a PROVA do drift. Corrigidas (todas com data+prova no texto): `training/idioms/concurrency` (célula GOOD + linha scheduler + NOVA nota self-cancel §253-A: jvm/js/script ✅, native SEM092 até a face B), `docs/stdlib/stdlib` (linha kof.scheduler + resumo "scheduling JVM+JS"→3 targets), `ecosystem-coverage` (linhas 64+166 "interval/cancel (JVM)" — contradiziam o próprio arquivo que diz 3 targets), `training/language/overview` (linha 93 na tabela "Planned/Unavailable" → riscada + ✅ 31/08), `docs/language-reference/concurrency:43` ("JVM+Native" → +JS TIME001), `docs/development/roadmap` 1.1 (fila de paridade listava 7 códigos FECHADOS como abertos → riscada + data, 🟡 mantido pelos abertos HTTP002/MEDIA/ws-sse). NÃO TOCADAS (com razão): `docs/history/*` (snapshots datados 02/09), `conformance-matrix`/`memory-model` (semântica, não-alvo), bp:144/tabela gap-codes (lista de prefixos que EXISTEM, não de abertos). Prova: `docs-lang.sh check` = 0 par/switcher/orfão; varredura final "interval apenas JVM" = 0 hits. Docs-only, nenhum código tocado.
> **✅ FEITO (16/09 ~10:25, dono = 192.168.100.22, lane docs/development): contagem autoritativa re-medida no CLONE LIMPO do tip `98ac26ed` — 2206→2211 (EN+PT, 12 registros).** Contrato (3) disparou: o tip moveu CÓDIGO pós-`574c9419` — `78b733fa` (+3 `NumericFormatterE2ETest`) e `7b38d0d4` (+2 `KofTimeE2ETest` §253-face-A) = +5 compiler. Medição (4 módulos, clone fresco `98ac26ed`, disco==HEAD==índice): **2211 run (1904 kof-compiler + 38 script + 7 kof-c + 262 cli), 1 fail, 0 erros, 190 skip**. A 1 falha é o **flake §252 RE-DISPARADO** (`spawnWorkerThrowPropagatesThroughSelectAnyNative`, "array index out of bounds", idêntico a 15/09) — **NÃO regressão**, frequência em suíte completa ≈ 2/~10. Atualizados com prova: `docs/development/README`×2, `AGENTS`×2 (nº + "Estado correto HOJE" → 09:44/`98ac26ed`/re-fired), `status`×2, `backend-parity`×2, `ecosystem-coverage`×2, `known-bugs`×2 (nova sub-bala §252 09:44 suíte completa). Paridade EN/PT 0/0/0. **§258**: re-catalogado — #774 CLOSED pela `.17` (`2a60b426`); a análise CodeQL do GitHub trouxe à tona +2 no tip (16/09 ~10:30): **#775** `NumericFormatterE2ETest:35` relative-`java` (dona `.22`, `78b733fa` — mesma família #774) + **#776** `KofHttp.supportedOn:57` unused-param (= guard morto do §259, `@SuppressWarnings` não vale p/ CodeQL) → gate = 3 abertos. Linha+seção+fila EN+PT, ponteiros de dono; nada corrigido in-file (regra 8). `check_500` OK. Rebase limpo sobre `bfdbe81f`. **PRÓXIMO PASSO:** idem fila TICK 06:30 (fechar §258 só quando #773 sumir do `codeql-gate --fast`; re-medir se tip mover código; §252 fecha só pela lane nat). NÃO TOCAR: #773 (`.18`), §252/#273 (nat), §253-face-B (nat), #266/#259/#275 (mantenedora/.22), §259 (`.15`), `docs/development` WIP `.17`.
> **✅ FEITO (16/09 ~07:55, dono = 192.168.100.15, lane bugs-and-gaps): `backend-parity` ws/sse cell mapeava Native→`WEB002` (TLS) em vez de `WEB004`/`WEB003` (EN+PT).** A divisão `WEB003`/`WEB004` landou em `dc941b81` (30/08), mas a linha 69 seguia com o código ANTIGO `WEB002` (que hoje é exclusivo de TLS `listenSecure`) e `WEB001 residual` no JS. O gate único não-JVM (`ExpressionBuiltinInstanceCalls`) usa `KofWeb.gapCode`: ws→`WEB004`, sse→`WEB003` em Native E JS. Corrigido nas duas células. Prova: `docs-lang.sh check` 0 par/switcher/orphan; confronto com `KofWeb.gapCode` + o gate. Docs-only. **PRÓXIMO PASSO:** idem.
...
> **✅ FEITO (16/09 ~11:00, dono = 192.168.100.15, lane bugs-and-gaps): catalogação §259 — `http.timeout/retry/circuit` no Native são no-ops silenciosos (R6/Q7).** Achado: `ecosystem-coverage.md:270` citava `HTTP003` fantasma ("não silencioso: debug syserr") — código emite zero ocorrências de `HTTP003`, só `HTTP002`. Configuradores são `ret` puro (NativeHttpCore:369-380, NativeRiscvHttpCore:317-324), sem output de debug em lugar nenhum. `KofHttp.supportedOn` devolve `true` para todos os targets → compila silencioso, não faz nada. Docs corrigidos: `ecosystem-coverage.md` + `.pt_BR.md` (HTTP003 → pure silent no-ops); `backend-parity.md` + `.pt_BR.md` (ref §259). Seção adicionada ao `known-bugs.md` + `.pt_BR.md` (tabela fila §259 + seção completa), guard docs-lang 0. NÃO tocar: §258/.18, §252/#273/B/§255/§256b (nat/.18), #266/#259/#275 (mantenedora).
...
> **✅ FEITO (16/09 ~07:40, dono = 192.168.100.15, lane bugs-and-gaps): sync de corpus WEB001 — `stdlib-web` linha 332 mapeava ws/sse imprecisamente (EN+PT).** A fatia de honestidade WEB001 (`555d2afe`) mapeia por FUNÇÃO: `app.ws`→`WEB004`, `app.sse`→`WEB003` no Native E no JS (`KofWeb.gapCode`). A linha dizia "Native `WEB004`, JS `WEB003`", sugerindo que o JS só emite WEB003 p/ ambos. Corrigido p/ "Native/JS: `app.ws`→`WEB004`, `app.sse`→`WEB003` (reportados por função em compile-time)". Prova: `docs-lang.sh check` 0 par/switcher/orphan; confronto com `KofWeb.gapCode` (código). Docs-only. **PRÓXIMO PASSO:** idem.
...
> **✅ FEITO (16/09 ~07:10, dono = 192.168.100.15, lane bugs-and-gaps): §211 leading-Status corrigido (EN+PT) — padrão "linha de Status descreve o estado pré-fix".** A varredura header↔corpo↔fila (84 seções) achou 1 drift: §211 (enum identity, D-ENUM207) tem título/fila/fatias ✅ FECHADO, mas a PRIMEIRA linha `**Estado:**` ainda lia "reproduz no `0ab25887` … REABERTA" sem marcador de resolução — um leitor podia achar que o bug segue aberto. Corrigido: a linha vira `✅ FECHADO 15/09 (ambas as fatias)` + prefixo `*(Histórico)*` na re-medição pré-fix (a trilha é mantida, só o estado atual fica explícito); EN+PT. Prova: `docs-lang.sh check` 0 par/switcher/orphan; scan header-vs-body 84 seções = 0 drift restante. Docs-only, nenhum código tocado. **PRÓXIMO PASSO:** idem acima — repetir a varredura quando outra lane landar fix; §258 (#773 CodeQL) segue o único bloqueador de push repo-wide (dono .18, unidade DB001 já fechada — candidato a orphan/claim). NÃO TOCAR: docs/development (.17), §248 (escopo), §252/#273 + face B §253 (nat/.18), #266/#259/#275 (mantenedora).
> **✅ FEITO (16/09 ~12:10, dono = 192.168.100.15, lane bugs-and-gaps): APP002 — `[server] port` no kof.toml é parseado mas nunca usado pelo `kof serve` (EN+PT).** `KofProjectConfig.java` parsea `[server] port = N` → `serverPort()`, mas `CmdServe.java` só usa `--port`/8080; zero chamadores de `serverPort()` no repo. Docs `backend-parity.md:89` descrevia APP002 como "fallback to default" (gap honesto) — agora `PARTIAL`: parse funciona, port não é usado. Não há código `APP002` emitido em lugar nenhum; o gap é de UX/implementation. **NÃO implementar** (escopo — lane da mantenedora se/quando abrir frente serve-config). Atualizado backend-parity.md(+PT) para `PARTIAL (port unused)`.> **✅ FEITO (16/09 ~12:40, dono = 192.168.100.15, lane bugs-and-gaps): `UUID002` fantasma no conformance-matrix (EN+PT).** As 2 linhas de `uuid.v7()` (S3b.2) citavam `UUID002` como gate riscv/aarch — esse código **não existe no compilador** (o único code de v7 é `UUID001` em `KofUuid.gapCode`, hoje inalcançável pois `supportedOn`=true) e a própria célula dizia `✅` (auto-contradição). v7 roda nos 5 alvos desde 10/09 (fatia `RtB25b` + tradutor; `KofUuidTest.uuidV7CrossArch` sob qemu) — nota corrigida para refletir isso. Sem novo § (claim stale documental). Guard 0.> **✅ FEITO (16/09 ~12:50, dono = 192.168.100.15, lane bugs-and-gaps): varredura de gap codes — LOG001 + HTTP002 stale claims (EN+PT).** (1) `stdlib/http.md` (+PT) dizia "Native reporta HTTP002" para retry/circuit — o ramo `HTTP002` em `ExpressionHttpCallLowerer` é morto (`KofHttp.supportedOn`=true; §259); corrigido para "no-ops silenciosos, §259". (2) `status.md` (+PT): `kof.log` tabela célula JS `LOG001` → `✅ (LOG001 fechado 01/09)`; nota "JS reporta LOG001" → "JS console.* (LOG001 fechado)". `KofLog.supportedOn` inclui JS; `KofLogE2ETest` testa JS. Guard 0.> **✅ FEITO (16/09 ~13:00, dono = 192.168.100.15, lane bugs-and-gaps): javadoc stale do `KofDb` + célula `ecosystem-coverage` DB001 (EN).** `KofDb.java:24`/`:50` ainda diziam "JS reporta DB001 em compile-time" — `supportedOn` inclui `Target.JS` desde `eb9140cb` (DB001 fatia B); no JS só `db.query<T>` tipado falha (`DB002`). Comentário corrigido (no-op de comportamento, compile -q verde) + `ecosystem-coverage.md:458` "`kof.db` no JS ✅ 16/09 (`DB001`)" → "(DB001 fechado)". NÃO toquei `docs/development/native-multiarch.md` (lane .17 — a linha "JS mantém DB001" fica para ela).> **✅ FIX (16/09 ~13:10, dono = 192.168.100.15, lane bugs-and-gaps):** espaço líder no `**Status:**` de §258 em `known-bugs.md` (linha 9073). Sem impacto de conteúdo.> **✅ FEITO (16/09 ~13:20, dono = 192.168.100.15, lane bugs-and-gaps): `HTTP003` fantasma removido do `backend-parity.md:59` (+PT).** A célula do cliente `kof.http` ainda dizia "a face síncrona em JS puro fica HTTP003" — mas `HTTP003` não é emitido por nenhum módulo (confirmado na varredura §259); a §133 registra a face síncrona JS como **HTTP004** (nota de paridade, não código). Ambas as línguas corrigidas. Guard 0.> **PRÓXIMO PASSO (16/09 ~13:30, dono = 192.168.100.15, lane bugs-and-gaps):** VARREDURA DE GAP CODES ESGOTADA — sem drift real restante nos canônicos vivos. **Resumo da sessão (10:15–13:30):** §259 catalog (Native HTTP configurators = no-ops silenciosos + phantom HTTP003); CONFIG001→CONF001 (4 cells + 2 backend-parity); stale "JS=CONF001/LOG001/HTTP003/HTTP002" (7 cells); profiles Native y (prova); APP002 resid. (port não usado); UUID002 fantasma; HTTP003 phantom em backend-parity. **Itens restantes legítimos:** APP002/CRON001 são códigos de escopo, não-emitidos (doc-only). **NÃO tocar:** fix §259 (lane `.17`/`.18`), docs/development (lane .17), §248 (escopo), §252/#273 + face B §253 + §255 + §256b (nat/.18), #266/#259/#275 (mantenedora), §258/.18. **Estabilidade:** recusar re-trigger até regressão ou issue nova; portão = 1 flake (§252), o resto verde.> **✅ FEITO (16/09 ~13:40, dono = 192.168.100.15, lane bugs-and-gaps): `stdlib-web` Native `HTTP002` stale corrigido (EN+PT).** A seção de limitações dizia "`kof.http` ... Native `HTTP002` pendente" — o cliente HTTP Native está implementado (asm HTTP/1.1; `KofHttpE2ETest#nativeCompilesAndServesGet` verde) e o código `HTTP002` está morto; corrigido para "Native ✅ (configuradores no-op silenciosos, §259)". Guard 0.> **✅ FEITO (16/09 ~13:50, dono = 192.168.100.15, lane bugs-and-gaps): overclaim Native `status`/`headerSet` em `backend-parity.md:70` (+PT).** A linha dizia `✅ 03/09 (NativeWebRuntime responde status/body)` — FALSO: `KofWeb.contextNativeSupported` só aceita `kof_web_body`; `status()`/`headerSet()` são context-fns sem símbolo no `.s` nativo e emitem **WEB001 em compile-time** (`ExpressionStaticCallLowerer:101`; confirmado: `grep kof_web_status` nos fontes `nat/` = vazio; `KofWebNativeE2ETest` só asserta 200). `status.md:768` e `stdlib-web.md:335` já estavam corretos — a matriz era a outlier. Corrigida para `WEB001`. Guard 0.> **✅ FEITO (16/09 ~14:00, dono = 192.168.100.15, lane bugs-and-gaps): drift de fila EN em §180.** A linha 28 da OPEN Queue EN dizia "❌ OPEN 13/09" mas o cabeçalho da seção já dizia "✅ FIXED 15/09 x86_64" (same state that PT already had). §180 = residual shortest-round-trip/scientific/Float double-expansion; fixou em 15/09 por lane development `.18`. Linha corrigida. Guard 0.> **PRÓXIMO PASSO (16/09 ~14:10, dono = 192.168.100.15, lane bugs-and-gaps): AVALIAÇÃO DE ESTABILIDADE — lane bugs-and-gaps SEM trabalho próprio pendente.** As 3 condições de estabilidade NÃO se cumprem globalmente (known-bugs tem §255/§256b/§258/§259 OPEN — TODOS de outras lanes: `.18`, nat, `.17`), mas **nesta lane a varredura de drift doc×código está ESGOTADA** (15 commits nesta sessão): gap-codes fantasma (HTTP003/CONFIG001/UUID002), stale "JS reporta X" (CONF001/LOG001/DB001/HTTP002), overclaim Native status/headerSet, javadocs de código, fila vs cabeçalho (§180), APP002 residual, leading-Status §258. **Verificações finais:** `docs-lang.sh check` 0/0/0; scans automáticos (gap-code doc×código, fila↔cabeçalho EN+PT, header↔Status) sem drift real novo. **NÃO TOCAR:** fix §259/§255/§256b/§258 (lane `.17`/`.18`), docs/development (lane .17), §248 (escopo), §252/#273 + face B §253 (nat/.18), #266/#259/#275 (mantenedora). **RECUSO o re-trigger** até (a) regressão nova, (b) issue nova, ou (c) fechamento alheio que destrave verificação nesta lane. Cron: manter apenas se houver watcher de issues ativo.> **PRÓXIMO PASSO (16/09 ~12:25, dono = 192.168.100.15, lane bugs-and-gaps):** APP002 registrado+pushado (`acec3ffb`) — `[server] port` parseado mas não consumido. **PRÓXIMA UNIDADE:** continuar a varredura de gap codes doc×código com os itens restantes do diff — verificar (a) `UUID002` (conformance: gate riscv/aarch) tem emissão real; (b) `CANVAS001`/`UI0xx` em KOFUI-AUDIT conferem com o código; (c) os códigos citados como "current" (`APP001` etc.) têm emissão honesta. **NÃO TOCAR:** fix do §259 + APP002-implement (lane native/compiler/platform), docs/development (lane .17), §248 (escopo), §252/#273 + face B §253 + §255 + §256b (nat/.18), #266/#259/#275 (mantenedora), §258/.18.
> **✅ FEITO (16/09 ~14:30, dono = 192.168.100.15, lane bugs-and-gaps): `PLAN-STDLIB-EXPANSION.md` (EN) — status contradizia o par PT e o índice (EN+PT).** O cabeçalho EN dizia `Status: IN PROGRESS — S7d CLOSED 11/09` enquanto o par PT dizia `CONCLUÍDO (14/09)` e `docs/development/README.md:50` dizia `CONCLUDED 14/09 (S0–S13 implemented and validated on 5 targets)`. O EN ficou preso no estado de 11/09 (pré-conclusão). Além disso os 2 links do switcher (`plan-stdlib-expansion.md`/`.pt_BR.md`) apontavam para arquivos minúsculos que **não existem** — os reais são `PLAN-STDLIB-EXPANSION.md`/`.pt_BR.md`. Ambos corrigidos: header EN → `CONCLUDED (14/09)` (histórico de 11/09 preservado como bloco de traceabilidade) + switcher com os nomes reais. **Varredura de links do corpus (1290 links, 3 worktrees):** só esses 2 estavam mortos (os 4 `I18N.md → X.md` são a convenção documentada). Guard `docs-lang.sh check` 0/0/0.
> **✅ FEITO (16/09 ~15:00, dono = 192.168.100.15, lane bugs-and-gaps): reconciliação das tabelas históricas do DOING (EN+PT).** Três frentes num commit: (a) **divergência EN/PT de cabeçalhos de status** — varredura dos 112 pares: os 61 "divergentes" eram quase todos tradução (`implemented`×`implementado`, `CLOSED`×`FECHADO`); sem drift semântico real além do já corrigido no `PLAN-STDLIB-EXPANSION`; (b) **tabelas legacy "Em curso agora"/"Frentes de validação"** (snapshot 03–13/09, dono por NOME sem IP, branch `beta-0.3.0` morta) — banner de SNAPSHOT HISTÓRICO adicionado (conteúdo preservado, regra 8) + estados reconciliados contra o CÓDIGO: `STDLIB`/`EDI001`/`REFACTOR-500`/`KOFSCRIPT` → `FEITO` (provas em `docs/stdlib/`, `docs/tooling/`, `docs/architecture/`, `kof-script/`); (c) **tabela "Abertos"** — `CANVAS001` e `EDI001` provados concluídos (`known-bugs.md` §CANVAS001 `CLOSED`; `PLAN-EDITOR-INTEGRATION` `CONCLUÍDO 13/09`), `HTTP003` fantasma trocado pelo item real §259, residual web re-mapeado por função (`WEB002`/`WEB003`/`WEB004`). **Ponteiros mortos `docs/known-bugs.md`** (5 em DOING, canônico é `docs/bugs-and-gaps/known-bugs.md` desde 13/09) corrigidos; os 2 do `CHANGELOG` são registro histórico de 02/09 (path correto à época). Guard `docs-lang.sh check` 0/0/0.
> **✅ FEITO (16/09 ~15:35, dono = 192.168.100.15, lane bugs-and-gaps): auditoria completa — suíte stale training (810→22.214), SHAs mortos (0 reais), test-class refs (todas explicadas), status headers EN/PT (convergentes), backend-parity spot-check (correto).** (a) `training/` (10 arquivos EN+PT): header dizia versão 0.4.0--beta mas contagem de 02/09 (810→22.214, autoritativo em AGENTS. md:1091); (b) varredura de SHAs mortos em 422 arquivos docs/: 41 tokens hex-evalidados = 0 reais (UUIDs IEEE-754 endereços); o único real (4408eb66 typo) já corrigido 09:25; (c) 8 refs de test-class ausentes: todas explicadas (1=nome de arquivo abandonado em Pointer, 3=narrativa de revert, 4=pointers para lanes de compilador); (d) gap codes stale corrigidos na sessão (CONFIG001/HTTP003/UUID002/DB001/LOG001/HTTP002/APP002 resid.); (e) STDLIB EN header + dead switcher (Unit 1); (f) tabelas legacy DOING reconciliadas (Units 3+4); (g) ponteiros mortos docs/known-bugs.→bugs-and-gaps/ (5 em DOING; CHANGELOG=histórico). Guard docs-lang. 0/0/0.

> **✅ FEITO (16/09 ~16:05, dono = 192.168.100.15, lane bugs-and-gaps): ponteiros `file:line` stale nos docs canônicos (EN+PT, 24 refs em 19 arquivos).** A varredura pós-REFACTOR-500 achou 24 referências `Classe.java:linha` apontando para além do EOF (ou para arquivo inexistente) porque os splits moveram código. Corrigidas por medição (grep do símbolo real + `wc -l`): `JvmRuntime.java:370`→`JvmRuntimeWebServer.java:50` (SSLServerSocketFactory), `JvmWebRuntime.java:238`→`JvmWebHttpRuntime.java:112` (KOF_HTTP_CLIENT_INSECURE), `KofWeb.java:84`→`KofWeb.java:142` (kof_web_listen_secure), `JvmRuntime.java:489`→`JvmRuntimeWebDispatch.java:173` (kof_web_dispatch), `JvmWebRuntime.java:22`→`JvmWebCoreRuntime.java:20` (KOF_WEB_STATUS), `KofWeb.java:107`→`KofWeb.java:248` (kof_web_status), `CompilerDriver.java:3972`→`CompilerTypes.java:423` (substituteTypeVariableIn) em 8 ocorrências (architecture/comparison/stdlib/status), `CompilerDriver.inferExprType`→`ExpressionTyper.inferExprType` (`ExpressionTyper.java:14`) + `kof_int_to_string`→`NativeRiscvCrossOps.java:201`, `Main.java:694/839`→`Main.java:211`/`CmdScript.java:100` (init/repl), `SemanticAnalyzer.java:1033`→`MemberResolver.java:65` (isObjectMethod), `AstNodes.java:351`→`VarDeclStmt.java:4` (só `type`/`name`/`initializer`), `NativeBackend.java:1725`→`NativeBackend.java:369-372` (mangle de import) em 3 arquivos (known-bugs EN+PT + learn/19), `NativeBackend.java:3650`→`NativeAarch64Translator.java:16` (translateRiscvToAarch64), `Main.java:364/365`→`KofCliSupport.java:91/92` (NATIVE_RISCV64/AARCH64), `KofCcompiler.java`→`KofCCompiler.java` (typo de caixa; arquivo real em `kof-c-compiler/src/main/java/dev/kof/c/`). Prova: re-scan automatizado dos 422 `.md` de `docs/`+`learn/`+`training/` (excl. `docs/history/`) = **0 refs past-EOF / NO-FILE** (antes: 24); `scripts/docs-lang.sh check` 0/0/0. Docs-only.

> **✅ FEITO (16/09 ~16:35, dono = 192.168.100.15, lane bugs-and-gaps): versão corrente stale nos docs canônicos (EN+PT, 13 arquivos).** O `VERSION` da raiz e `pom.xml <revision>` são **`0.4.0-beta`** (autoridade: `training/releases/versioning.md` §version), mas o **`README`** raiz (EN+PT) dizia `0.3.0-beta` **e** a contagem de suíte de 02/09 (810/793+8+5+4) — duas verdades desatualizadas no doc mais visível do repo. Corrigido para `0.4.0-beta` + **2214** (1907+38+7+262). No mesmo padrão de header (versão corrente), `docs/architecture/architecture` (EN+PT, 3 sits cada: header + 2 inline do KofScript) e `docs/architecture/compiler-architecture` (EN+PT) declaravam `0.3.0-beta`; `docs/language-reference/` (README, specification-status, syntax; EN+PT) idem. Todos atualizados para `0.4.0-beta`, **preservando a proveniência** (`language-reference/README`: "initial extraction branch `beta-0.3.0`, 06/09/2026; current branch `beta-0.4.0`"). Convenção já usada por `specification-gaps.md:12` ("0.4.0-beta (pom `revision`; was 0.3.0-beta in the audit)"). Escopo consciente: `CHANGELOG`/`docs/history/`/§-registros do DOING citam `0.3.0-beta` como **histórico datado** — intocados. Guard `docs-lang.sh check` 0/0/0.

> **✅ FEITO (16/09 ~16:50, dono = 192.168.100.15, lane bugs-and-gaps): contagem de suíte stale em claims de ESTADO ATUAL (EN+PT, 4 arquivos).** Pós-varredura de todas as ocorrências `810` nos canônicos: classifiquei cada uma como snapshot datado (intocável) vs claim de estado atual (stale). Corrigidos na 2ª categoria: (a) `learn/30-contributing` (EN+PT) — header atualizado p/ `0.4.0-beta` em 14/09 mas as 2 contagens seguiam 810 (golden + "Tests: 810"); (b) `docs/philosophy` (EN+PT) — presente-do-indicativo Build `mvn test` 810 → 2214; (c) `docs/stdlib/DATABASE_VISION` (EN+PT) — `VERSION` 0.2.6-beta; build 810 tests → 0.4.0-beta + 2214. Intocados (snapshot datado correto): `CONTRIBUTING` (02/09 0.2.6-beta), `performance` (08/27), `complexity-audit` (snapshot 09/02), `kof-vs-java` (31/08), `security` (31/08), `ecosystem-coverage` (audit date 08/27), `specification-gaps` (§SG-E2 fala da contagem de 02/09 como histórico + “1270” é a medida de 10/09, correto para o §), `known-bugs` (810 = apenas em SHAs/floats). Guard `docs-lang.sh check` 0/0/0.

> **✅ FEITO (16/09 ~17:05, dono = 192.168.100.15, lane bugs-and-gaps): CLI `18 commands` → **26** (EN+PT, 16 arquivos).** A varredura de claims de estado atual achou que o `Main.java` tem **26** comandos no switch (`build/run/serve/check/test/script/repl/c/fmt/config/bench/profile/inspect/decompile/translate/compare/migrate/debug/info/lsp/install/deps/editor/init/new/version`), mas os canônicos diziam **18**. Os 8 que faltavam são REAIS (classe + testes): `decompile`/`translate`/`compare`/`migrate` (`02faca02`), `deps` (`dfce9115`), `editor` (`527dcfa7`), `new` (`2a255f8b`); `init`/`config` já existiam. Corrigidos: `philosophy`, `ecosystem-coverage` (2 células), `roadmap`, `PLAN-UNIVERSAL-PLATFORM`, `learn/README` (2 sítios), `learn/29`, `learn/30`, `training/reference/compiler` (+ linhas da tabela CLI ganharam os 8 comandos). `architecture.md:291` intocado (snapshot datado `0.2.6-beta, 31/08/2026`, correto para a data). Guard `docs-lang.sh check` 0/0/0.

> **✅ FEITO (16/09 ~18:10, dono = 192.168.100.15, lane bugs-and-gaps): help do CLI vs parser — targets anunciados errados no `printUsage` (código + teste, EN+PT).** Achado na varredura de listas CLI: o `printUsage` do `Main` anunciava `build`/`run` como `jvm|native|js|android` (omitia `native.risc|native.arm`, aceitos por `KofCliSupport.parseTarget` e ratificados em `docs/status.md:88-89`), `bench`/`profile` como `jvm|native|js|android` (android é REJEITADO por `BenchDiscovery.parseTarget`/`Profile.parseTarget` → `unknown target`), e `test` como `jvm|native` (omite `js`, ratificado em `status.md:566`). Fix (só string de help, zero mudança de contrato — regra 6): `Main.printUsage` (build/run/test/bench/profile) + `CmdBuild` (2 sítios) + `CmdRun` (2 sítios) + `CmdTest` (usage interno, que divergia do Main: dizia `jvm|native|js|android`). Prova (Q1, mesmo commit): `CliUsageTargetsTest` 3/3 — build/run anunciam `native.risc`+`native.arm`; bench/profile NÃO anunciam android; test anuncia `js`. Q0: as 3 asserções falhavam no código antigo. Escopo declarado: `script`/`config` mantidos conservadores (`jvm|native|js`) por não confirmar risc/arm no interpretador. `docs/status.md` já era o correto — nenhuma edição .md de conteúdo além deste registro. Guard `docs-lang.sh check` 0/0/0.

> **✅ FEITO (17/09 ~03:10, dono = 192.168.100.15, lane bugs-and-gaps): corpus sync #330 (`fn`/`fun`/`func` em posição de NOME) — 4 pares EN+PT.** A varredura pós-land (`git log 2d1efec6..origin/beta-0.4.0`) achou o fix do #330 (`80b04b10`) e o drift que ele deixou: a doc canônica ainda dizia que `fn`/`fun`/`func` **são identificadores válidos como nome** ou que falhavam com o `PARSE037` genérico. Medição própria (probe temporário `TmpProbe330Test`, removido) confirma o contrato real: **`PARSE085` em TODA posição de nome** (função, variável, parâmetro, método, campo, classe, record, enum). Corrigidos: `language-reference/grammar` (linha que dizia "As a function *name* they remain valid identifiers" — FALSO), `language-reference/lexical-structure`, `bugs-and-gaps/specification-gaps` (+ contagem `FunctionSyntaxTest` 12→**15**) e a mini-face (b) do §232 em `known-bugs` (anotada **FIXED 17/09 #330**). Prova: probe mediu os 8 casos (todos PARSE085) + `FunctionSyntaxTest` 15/15 do próprio fix; docs-lang 0/0/0. Regra 4/Q0: doc alinhada ao comportamento esperado (SG-001), não o inverso.

> **✅ FEITO (16/09 ~21:08→23:30 local (-0300) = 17/09 00:08→02:30 UTC; dono = 192.168.100.15, lane bugs-and-gaps): D-DIAG-EN — 4 batches de código PT→EN + doc-sync (5 commits na `beta-0.4.0`).** Unidade da decisao da mantenedora (DECISIONS `221cb305`, #324): tooling 100% EN, codigos SEM/PARSE/WASM NUNCA mudam, so texto; docs seguem EN+PT. Lands: (1) `9bc9f60f` parser — PARSE073/076/078/085/094 PT→EN + `FunctionSyntaxTest.reservedWordMessageIsEnglish` (assert EN + regex anti-diacritico); (2) `8edfe970` switch — SEM031/032/035 + `missingDefaultFailsToCompile` reforcado; (3) `d993a6f4` runtime/CLI — "await: handle inválido"→"invalid handle", "selectAny: argumento não é Handle", "lambda invoke não encontrado", "(classe 'X' não achada)", JvmFrameDiagnostics inteiro ("last Kof construct"/"phase:"/"IR (last N ops)"/"bytecode ASM (last N lines)"/"unavailable"), Bench "invalid value for X"; prova: `JvmFrameDiagnosticsTest` asserta labels EN + ausencia PT (assert antigo `fase`→`phase`), `BenchTest` guard→"invalid value". **LIÇÃO (registrada): varrer so por diacrítico PERDE labels PT sem acento ("fase", "linhas", "erro") — revisao arquivo-por-arquivo no batch**; (4) `621ce9fd` colecoes/target — SEM055 "takes an Int INDEX"/SEM056 "Kof collections are homogeneous" (CollectionCallLowerer 4 sitios + ExpressionStaticCallLowerer 2 — contrato honesto entre rotas), SEM033 "received a void value", SEM029/SEM034 "is not supported on collections", TargetMatrix "cannot be backend/frontend" + honest-gap WASM001 "does not exist yet — planned in Phase 6" (caminho DECISIONS.md na mensagem preservado); prova: `SemanticResolutionTest.heterogeneousWriteToPinnedCollectionRejected` guard de `contains("homog")` (casava PT e EN — false-green risk) → `contains("Kof collections are homogeneous")` + ausencia "coleções"; `contains("ÍNDICE")`→`"INDEX"`. (5) `13343543` doc-sync fila: `known-bugs.md:5859` (EN) passa a citar o SEM033 EM texto real — era o ponto nomeado pela fila da mantenedora. **Politica de citacao PT-vs-EN no doc PT** (`known-bugs.pt_BR:3588/5853/3939` citam tooling PT) = convencao de doc NAO adjudicada (rule 6) — lacuna registrada, PT docs intocados. Suite apos cada batch: compiler+cli 2263 / 1F (§252 flake, nao-tocado) / 0E; compiles verde. RESTANTE (inventariado por lista de palavras + diacritico, nao-so-diacritico): ~150 strings PT em SemanticAnalyzer/ExpressionInstanceCallLowerer/PrimitiveNumericFormatters/CompilerImports/ReturnPathAnalyzer/Translate*(CLI)/LspHover/CmdEditor etc. — continua por familia NAO-colidindo com o footprint da .22/.17 (typer-files MethodCallTyper/ExpressionTyper FORA ate o land #334; MemberCallTyper/SemExpressionTyper/StatementAnalyzer/SymbolTableBuilder FORA). **PROXIMO PASSO (esta lane):** batch 6 = `ExpressionInstanceCallLowerer` (8 strings: "nao tem metodo", "pode truncar... forma explicita", "colecao; a linha crua de db.query e JSON String") + `PrimitiveNumericFormatters` ("Kof nao tem metodo") + `CompilerImports` ("nao corresponde ao diretorio do import", "nao encontrado no modulo") + `ReturnPathAnalyzer` ("declara retorno ... mas pode terminar sem return/throw") — antes de cada: grep de testes asserindo o PT; depois doc-sync EN das citas que mudarem; `docs-lang.sh check` + suite antes do push; footprint re-checado via `git log --oneline -10` da beta + `git status` do worktree .22.

> **✅ FEITO (17/09 ~04:00, dono = 192.168.100.15, lane bugs-and-gaps): alinhamento sistemático de `language-reference/*` com os SG-00x APLICADOS (EN+PT, 11 pares + spec-gaps).** A varredura pós-land (#322/#330) revelou que as tabelas de estado do `language-reference/` ficaram **presas no estado de 06/09** — declaravam Unspecified/Absent itens já corrigidos há dias. Medido por probe (temporário, removido) e cruzado com os §-registros de `specification-gaps.md`: **subtyping** (`SEM021`, SG-009), **deref `T?`** (`SEM049`, SG-005), **wildcard** (`PARSE086`, SG-007), **elem de `list.add`/`set`/`map.put`** (`SEM056`), **cobertura de interface** (`SEM043`, SG-015), **instanciação de abstract** (`SEM041`, SG-017), **private/protected em MÉTODOS** (`SEM046`, SG-013) × campos ainda Unspecified, **classes aninhadas** (`SEM042`, SG-016), **`Int main()` removido** (`SEM044`, SG-018), **`throws`** (`SEM045`, SG-019), **modelo de memória** (SC + 6 HB, SG-020), **overload top-level + de método** (§131/SG-011, `SEM047`/`SEM057`), **inferência de lambda** (SG-012), **função aninhada tipada** (`outer__inner`) × `f(){}`→`SEM015`, **`val` imutável** (`SEM037`, SG-010), **short-circuit em JS** (SG-006 FIXED), **`sealed`/`permits`** (`PARSE010`, SG-002). Corrigidos: `specification-status`, `semantics`, `type-system` (§7 reescrito), `classes`, `functions`, `closures`, `expressions`, `grammar`, `statements`, `syntax`, `README` + header `Re-synced 17/09/2026`. Regra 4/Q0: doc alinhada ao comportamento medido, nunca o inverso; nenhuma mudança de contrato. `docs-lang.sh check` 0/0/0.

> **✅ FEITO (17/09 ~05:00, dono = 192.168.100.15, lane bugs-and-gaps): `docs/development/future/README` — índice desatualizado + contradição interna (EN+PT).** A varredura dos índices de `docs/development/` achou 4 defeitos reais no README de `future/`: (a) título dizia `docs/future/` (caminho antigo; o real é `docs/development/future/`); (b) linha da tabela com `**` e `|` NÃO fechados (quebra de tabela); (c) **contradição**: `DECOMPILER`/`TRANSLATOR`/`LEGACY_MIGRATION` apareciam em "já caíram para docs/development/" enquanto a linha de cima diz que voltaram p/ `future/` em 15/09 — e `PLATFORM-PLAN`/`APPLICATION_MODEL`/`planning-stdlib-time-design` (apagados, consolidados no `DECISIONS.md` 13/09) seguiam listados como se estivessem em development/; (d) **`PLAN-MULTIPARADIGMA.md` ausente do índice** (arquivo real na pasta, design-only 16/09). Correções: caminho do título; linha fechada; `PLAN-MULTIPARADIGMA` adicionado às duas tabelas ("o que fica aqui"); seção renomeada p/ **"Histórico: snapshot 12/09 — NÃO é o estado atual"** com banner + coluna "casa atual" apontando `DECISIONS.md`/`docs/audits/`/`future/`; §4.3 do `docs/development/README` ganhou `PLAN-MULTIPARADIGMA` + o cluster de migração. Prova: checagem de integridade de tabela (nenhuma linha com `|` desbalanceado); `docs-lang.sh check` 0/0/0. Também verificado nesta rodada (plano confiável): **2.1.5 FFI001 real** (`CompilerPipeline.java:234/456` emite o gap honesto) e **1.5 export OTel realmente pendente** (zero `otlp`/`opentelemetry` no runtime). Docs-only.

> **✅ FEITO (16/09 ~19:45, dono = 192.168.100.15, lane bugs-and-gaps): 3ª rodada da varredura doc×código — índices/versão/toolchain (11 unidades, EN+PT).** A varredura de índices achou drift real em vários canônicos: (1) `docs/development/future/README` — título `docs/future/` (caminho antigo), linha de tabela com `**`/`|` não fechados, contradição interna (migração listada como 'caiu p/ development' embora tenha VOLTADO p/ `future/` 15/09; `PLATFORM-PLAN`/`APPLICATION_MODEL`/`planning-stdlib-time-design` apagados no `DECISIONS.md` 13/09 ainda listados) + **`PLAN-MULTIPARADIGMA.md` ausente do índice** (`d5e4febd`); (2) `docs/development/README` — contagem de bugs abertos `8` (canônico = 32) `b1c8d3c7`, faixa SG `020`→`022` em 2 lugares `defb8ac5`; (3) **toolchain/JDK embutido**: `package.sh` embute **Temurin 25** e `pom release=25` (D-BASELINE 14/09, README) mas distribution/tooling/philosophy/roadmap diziam `Temurin 21`/`JDK 21+` → sincronizado mantendo o contrato verbatim `KofVersion.TOOLING_API=21` (`1e830207`), incluindo `LICENSING` (também `0.2.6-beta`→`0.4.0-beta`, contagens → `docs/status.md`) `a7180d3d`; (4) claims de versão corrente `0.2.6-beta`→`0.4.0-beta`: INSTALL/stdlib-README/release-naming `a41611b2` + headers `Version:` de ARCHITECTURE/stdlib-logging/stdlib-web/http/DATABASE_VISION `56eb89be` + exemplos do INSTALL `kof 0.4.0-beta`/Adoptium 25.x; (5) **corrupção de encoding** `x{a7}`→`§` no `docs/audits/README` `110186c4`; (6) `docs/status.md` listava **19** comandos CLI (faltavam decompile/translate/compare/migrate/deps/editor/new) → **26** `039dc2d5`; (7) `docs/tooling/README` dizia `kof fmt` **planned** e LSP **minimal** — ambos implementados (`Fmt.run`, `LspServer` hover/definition/completion/references/rename) `3d23677d`. Prova: `docs-lang.sh check` 0/0/0 em toda unidade; comandos/constantes conferidos no código (`Main.java` switch=26, `KofVersion.TOOLING_API=21`, `kof-cli` @Test=265). Dead-link scan docs/learn/training = 0 (só placeholders `X.md` do I18N). > **✅ FEITO (16/09 ~20:05, dono = 192.168.100.15, lane bugs-and-gaps): corpos do version-bump 0.3.22→0.4.0 completados (176 marcadores, 104 arquivos, EN+PT) + JDK embutido no `training/` + CHANGELOG.** O bump `e8a8aeea` (13/09) foi **INCOMPLETO**: `VERSION`/`pom` = `0.4.0-beta` mas `training/`, `learn/` e `docs/` ainda carregavam **176** marcadores `0.3.22-beta` (headers `Status:` do learn, tags de seção de idioms/anti-patterns, `learn/33/38/39`, exemplos `kof version`/`Compiler/Runtime/Stdlib`, `learn/README` 'what works today'). Varredura completa → `0.4.0-beta`, **preservando proveniência**: `known-bugs.md` Issue #101 (a release do reportante EXTERNO era 0.3.22-beta) e snapshots `docs/history/` (`80c6efdd`). Também: `training/distribution/install`/`releases/versioning`/`tooling/cli` ainda diziam **Temurin 21** → 25 (tooling API level 21 fica) `0d6f72d3`; CHANGELOG 'unreleased' dizia `0.3.0-beta`/branch `beta-0.3.0` (branch duplicada, já existe seção released `0.3.0-beta` 05/09) → `0.4.0-beta`/`beta-0.4.0` `fea97a11`. Prova: `docs-lang.sh check` 0/0/0; residual `0.3.22-beta` = só known-bugs (proveniência); residual `Temurin/OpenJDK 21` no corpus = 0.

> **✅ FEITO (16/09 ~20:20, dono = 192.168.100.15, lane bugs-and-gaps): listas de comandos CLI + capacidades do LSP (EN+PT).** Varredura de listas de comandos achou mais 3 canônicos presos no passado: `training/tooling/cli.md` dizia **'Comandos oficiais (18)'** e faltavam profile/inspect/decompile/translate/compare/migrate/deps/new/install → **26** (`b3eeb4a0`, + cláusula tooling-API sincronizada com D-BASELINE); `README.md` (raiz) lista condensada sem os 7 de migração/PM/editor/new `ee65dd0e`; `docs/tooling/LSP.md` listava **hover/definition/completion/references/rename como 'planned'** quando o `LspServer` já os expõe (`completionProvider`/`hoverProvider`/`definitionProvider`/`referencesProvider`/`renameProvider`) `abc4c4b9`. Prova: `Main.java` switch = 26; `LspServer.java:93-119`; `docs-lang.sh check` 0/0/0. Residual '18 commands' current-state = 0.

> **✅ FEITO (16/09 ~20:40, dono = 192.168.100.15, lane bugs-and-gaps): refs `file:line` pós-REFACTOR-500 (EN+PT).** Varredura automática de 446 refs `arquivo:linha` em `docs/` achou **12 still-current** apontando p/ símbolos que migraram nos splits de 13/09: `IRNodes.java:99-252` (agora 1 record `Kof*.java` por op implementando `KofOperation`) em `compiler-architecture`; `AstNodes.java:147-158` → `EntityDeclarationNode.java`/`KofOrm.java` (`classes.md`); `AstNodes.java:358-363` → `AstNodes.java:63-64` (`semantics.md`); `JvmBackend.java:913-946` → `JvmOpCollections.java:95-112` (`type-system.md`); linhas `IRNodes.java` → arquivos `Kof*` reais (`ARRAY_MODEL`/`EXCEPTIONS_MODEL`); §31 `known-bugs` anotado como linha pré-split (`b0f1d26a`). Residual past-EOF = 0 em docs current-state (só `docs/history/*` datados + `docs/decisions/planning-switch-expr.md` que descreve o estado PRÉ-mudança). Prova: re-scan = 0; `docs-lang.sh check` 0/0/0.

> **✅ FEITO (16/09 ~21:00, dono = 192.168.100.15, lane bugs-and-gaps): índices + exemplos de release (EN+PT).** 3 achados: (a) `docs/language-reference/README.md` não indexava `concurrency.md` nem `concurrency-memory-model.md` (ambos normativos, existem) → linhas adicionadas na tabela `c52db641`; (b) `docs/distribution/RELEASES.md` mostrava exemplo absurdo de bump **`0.2.6-beta → 0.2.6-beta`** (mesmo p/ mesmo) 2x + tarballs `kof-0.2.6-beta-*`; `PACKAGING.md` idem → **`0.4.0-beta`** com exemplo real `0.3.22-beta → 0.4.0-beta` `d27693f6`; (c) classe-existência scan (`.java` citados em docs/learn/training) = 0 fantasma real (restantes são gerados/narrados: `KofRuntime.java` é GERADO; `AbstractMethodImplementationTest`/`Jdk*.java` são nota histórica/exemplo). Varredura de subcomandos CLI falsos (`kof <x>` em docs) = 0 (só `kof doctor` documentado como inexistente). Prova: re-scan README-coverage + `docs-lang.sh check` 0/0/0.

> **✅ FEITO (16/09 ~21:15, dono = 192.168.100.15, lane bugs-and-gaps): `kof.cli` row 13→26 (EN+PT) `170defcb`.** A tabela de módulos do `docs/stdlib/stdlib.md` listava só 13 comandos; alinhada aos 26 do `Main.java`, com ponteiro p/ `docs/status.md`. Varredura complementar sem drift: dead-links docs/learn/training = 0 reais (4 placeholders `X.md` no `I18N.md`); test-class refs ausentes = só nomes de teste PROPOSTOS em entradas "Pointer (lane compiler)" OPEN e em planos futuros (sem false-green); claims de versão `0.2.6-beta` restantes = provenance datada ("since/closed in 0.2.6-beta", snapshots com data) ou CHANGELOG histórico. Prova: `docs-lang.sh check` 0/0/0.

> **PRÓXIMO PASSO (16/09 ~19:50, dono = 192.168.100.15, lane bugs-and-gaps):** a varredura de **índices/READMEs + versão/JDK** está esgotada nesta rodada. Restante possível: (a) `roadmap-audit.md` 'CLI 20 commands'/'23 namespaces' = **snapshot datado** (regra 3 estados) → NÃO tocar; (b) claims `0.2.6-beta` em seções datadas (`runtime/*` 'Updated (0.2.6-beta)', `history/*`) = proveniência → NÃO tocar; (c) re-rodar quando outra lane landar fix (`git log dbf1a775..origin/beta-0.4.0`); (d) suite: re-medição é da lane `.22` (não tocar o número). NÃO tocar §259/§255/§256b/§258/DECISIONS.md/§248/§252/#273/§253-B.

> **PRÓXIMO PASSO (17/09 ~04:15, dono = 192.168.100.15, lane bugs-and-gaps):** varredura doc×código **2ª rodada pós-land concluída** (#330 + #322 + todos os SG-00x aplicados): 3 commits (`5167d172` corpus #330, `427af859` language-reference vs SG-00x, `cdc4daf9` compiler-architecture). **Nova rodada quando outra lane landar fix.** **GAP ANALYSIS (o que falta nos planos, para a mantenedora decidir):** (a) **TIER 1 SYSTEMS** (pré-requisito dos Tiers 6+) ainda aberto: 1.2 GC mark-sweep Native x86 (G-1..G-5, lane nat `.17`/`.18` — frente 2 **aguarda decisão A/B/C** da mantenedora, rule 6); 1.5 export OTel pendente; 1.6 bare-metal = plano só. (b) **TIER 2 compilador**: 2.1.5 FFI Native (`FFI001` honesto — `dlopen` segfault sem init glibc), 2.2.2 hook formal `CodegenStep` ausente (2.2.3/2.2.4 bloqueados por ele), 2.4.1 scoped resources = design. (c) **2.6 nullability D-NULL-INTENT** N1–N4 em curso (lane compiler `.22`/`.17`); **2.7 value records** planejado, gate R12. (d) **Bugs abertos todos com dono**: §248/§252/§253-B/§255/§256b/§258/§259/§262/§263 (**§262 `Record T? == null` NPE no JVM** e **§263 parser descarta IDENT antes de anotação** são os 2 catalogados mais recentes, fixes = lane compiler; não toco). **Sem trabalho de implementação LIVRE na minha lane** — o drift de docs está esgotado nesta passada. **NÃO TOCAR:** §259/§255/§256b/§258 (lane `.17`/`.18`), `docs/development/DECISIONS.md` (lane .17), §248 (escopo), §252/#273 + face B §253 (nat/.18), #266/#259/#275 (mantenedora).

> **PRÓXIMO PASSO (16/09 ~18:20, dono = 192.168.100.15, lane bugs-and-gaps):** o fix de help do CLI (16/09 ~18:10) fechou o item (1) da fila anterior. Fila re-avaliada: (2) "23 namespaces" e "CLI 20 commands" em `roadmap-audit.md` = SNAPSHOT datado (09/06, re-sync 09/12) → intocados (regra dos 3 estados); (3) contagem de targets 6 vs 7 = decisão de design (não minha, rule 6); (4) contagens por módulo nos canônicos são "measured at this HEAD" com data → snapshots válidos. **Drift de docs novamente ESGOTADO.** Nota de contagem: minha unidade +3 testes → suíte 2221 (baseline datado 2218/15:54 preservado como proveniência). Próxima unidade: repetir a varredura quando outra lane landar fix (`git log <last>..origin/beta-0.4.0`). **NÃO TOCAR:** §259/§255/§256b/§258 (lane `.17`/`.18`), `docs/development/DECISIONS.md` (lane .17), §248 (escopo), §252/#273 + face B §253 (nat/.18), #266/#259/#275 (mantenedora).

> **PRÓXIMO PASSO (16/09 ~17:10, dono = 192.168.100.15, lane bugs-and-gaps):** a auditoria **NÃO estava esgotada** — o ciclo re-trigger achou o drift CLI 18→26; continuar varrendo claims de CONTAGEM de features/arquivos canônicos. **PRÓXIMAS UNIDADES (ordem):** (1) listas de comandos CLI em `docs/distribution/INSTALL.md`/`docs/tooling/*` vs `Main.printUsage` (26); (2) número de módulos stdlib citado (`kof.*`) vs `KofStd`/namespace real; (3) número de targets (6 emitidos: jvm/native/riscv64/aarch64/js/android — conferir que não há "7" nem "3" stale); (4) contagens "N tests" por módulo fora dos registros datados. **NÃO TOCAR:** §259/§255/§256b/§258 (lane `.17`/`.18`), `docs/development/DECISIONS.md` (lane .17), §248 (escopo), §252/#273 + face B §253 (nat/.18), #266/#259/#275 (mantenedora).

> **PRÓXIMO PASSO (16/09 ~11:15, dono = 192.168.100.15, lane bugs-and-gaps):** §259 catalogado (`http.timeout/retry/circuit` no Native = no-ops silenciosos; `HTTP003` fantasma removido de `ecosystem-coverage`/`backend-parity`; `KofHttp.java` anotado; `eea7af94` pushado). **PRÓXIMA UNIDADE:** continuar a varredura de gap codes doc×código — conferir códigos citados em `docs/` que NÃO aparecem no grep de emissão do código (ex.: `NATIVE002` só em comentários; `CONC003` existe como `CONC003-JS-01`), catalogar/corrigir EN+PT com nota de registro. **NÃO TOCAR:** fix do §259 (lane native/compiler `.17`/`.18`), docs/development (lane .17), §248 (escopo), §252/#273 + face B §253 + §255 + §256b (nat/.18), #266/#259/#275 (mantenedora), §258/.18.
> **PRÓXIMO PASSO (16/09 ~10:15, dono = 192.168.100.15, lane bugs-and-gaps):** varredura de drift **ESGOTADA** nesta sessão (7 commits): §211 leading-Status, fila §186/§187, WEB002 ws/sse (stdlib-web/backend-parity/ecosystem-coverage/status/README/training), §258 linha+justificativa, SHAs mortos (`4408eb66` typo; `c4356881`/`eaba0f24` inexistentes = ⚠️ anotados), issues `closed` conferidas na API (148/159/133/145/101/207/213/209/261/267/268/269/256/151/160), 4 abertas com dono (#275/#266/#259 mantenedora; #273 nat/.18). **Nenhum drift real restante nos canônicos** (`docs/`, `training/`, README, AGENTS). **PRÓXIMA UNIDADE (re-trigger):** repetir a varredura quando outra lane landar fix (`git log <last>..origin/beta-0.4.0` por fix sem virar header/corpo/fila do §-registro; checar SHA de CÓDIGO citado, teste citado existe, issue citada closed confere na API). **Se nada novo e o portão seguir verde** → tick reativo não-inventivo (registrar "aguardando fechamento X"; `auto-loop.sh stop` só quando as 3 condições STABILITY segurarem — hoje NÃO seguram: §248/§252/§253-B/§255/§256b/§258 abertos, todos com dono). **NÃO TOCAR:** docs/development (lane .17), §248 (decisão escopo), §252/#273 + face B §253 (nat/.18), #266/#259/#275 (mantenedora).
...
>
 ...
 > (linha JS + nota de topo), security (Transactions JS `DB001`→`untyped 16/09`
 ...
 > formato), ecosystem-coverage (matriz bugs-and-gaps = outra lane), DB002 (já
...
>
> **✅ FEITO (16/09 ~11:00, dono = 192.168.100.18, lane development): WEB001 residual — SSE handler-scoped no host GraalJS (`7cd69a7b`) + CRON001 registrado nas docs.** Escopo confirmado com a mantenedora (chat, 16/09): handler-scoped honesto — eventos escritos DURANTE o corpo do handler, stream fecha no retorno (pump single-thread). `JsRuntimeUiWebSse.java` (novo bloco concatenado ao `ui-web`, padrão crypto+chacha): `kofWebSseRoute`/`kofWebSseSend`/`kofWebSseMakeConn` + conexão com `send/event/close/isOpen`, framing idem JVM, headers `text/event-stream`/`no-cache`/`keep-alive`/`X-Accel-Buffering: no`; pump `JsRuntimeUiWeb` com `sendResponseHeaders(200, 0)` (chunked real — `-1` = "sem corpo" no javadoc e engolia os writes, medido). Gates abertos só p/ o que roda (`jsWebT1` aceita `kof_web_sse_route`; `contextJsSupported` aceita `kof_web_sse_send`); wsSend/wsMessage/stats seguem compile-time; residual honesto JS: push pós-return/multi-cliente = `WEB003`. Prova: `KofWebJsE2ETest` +3 (single-data framing+headers; member-style send/event/multi-line; scoped-close com assert `isOpen` no corpo — as 2 formas cobertas); suíte completa 1907/0/0/190; check_500 sem ≥600; docs-lang 0/0/0. CRON001 (mesma rodada de gap analysis): `scheduler.at(cron, fn)` IGNORA o cron nos 5 alvos (`kof_time_interval(60000)`) — stub, não feature; stdlib/roadmap/status/backend-parity EN+PT corrigidos; gate de compile no JS AVALIADO E RECUSADO (criaria divergência fake num stub já par-plataforma). Docs EN+PT no lockstep: backend-parity (2 células × 2 línguas), stdlib-web (4 células × 2), stdlib index (2 × 2), README (1 × 2), ecosystem-coverage (1 × 2). Rebase duplo resolvido (lane docs tinha reformatado as MESMAS células no sweep R6 — format deles + semântica minha, célula a célula).

> **✅ FEITO (16/09 ~13:30, dono = 192.168.100.18, lane development): varredura de gaps da lib kof-ui-widgets ⇒ UIW005/UIW001 entregues + §261 da plataforma + §255/#773/#777 fechados.** (1) Sondas compiladas+rodadas nos 3 alvos acharam 2 células mortas no ledger da lib: UIW001 (vincular ação pós-criação — o `.on` pós-criação já roda nos 3) e UIW005 (o ponteiro mousedown/mousemove/mouseup + `Event.x()` já vive no runtime). ENTREGUE na lib (`1391f59`, main): `Slider` arrastável (transição PURA `sliderValue`, clamp/âncora/trunc exatos nos 3 alvos; +4 testes de valor) + `ReconfigButton` (troca o próprio rótulo ao clicar, mirror `ReconfigState`) + oráculo novo `scripts/browser-drag.mjs` no **Chrome 153 real** (drag +40px leva o Slider 20→70; botão desligado→LIGADO→desligado; zero exceções) + `examples/slider.kf` fixture; suíte da lib jvm/native/js 24/24 suites + check.sh limpo; gaps/roadmap/learn-01/api.md sincronizados (roadmap também tinha UIW031 pendurado já fechado — corrigido). (2) O oráculo achou bug REAL na plataforma: §261 (Kof4j `4ea099b3`) — window.bind resolvia component-first com DOIS contadores de handle partindo de 0 (`kofUiSeq` vs `kofNodeSeq`): Component criado antes de widget cru ROUBAVA o handle e o widget nunca renderizava (silencioso). Fix: contador único compartilhado; prova `KofJsBrowserE2ETest.componentAndRawWidgetIdsNeverCollide` (VERMELHO pré-fix medido re-introduzindo o bug; browser 26/26; suíte 1908/0/0/168). Renumerado de §260 (colidiu com o §260 GC x86 da lane `.17` no remoto — convenção §176). (3) §255 fechado (`06e77e94`): `crossNativeSqliteNowCompiles` ganhou o trio de guards do irmão → SKIP honesto sem sysroot sqlite. (4) §258→🟡 PARTIAL (`c2300df6`): #773 bound check no `listValues` (precedente `d6eaae0c`) + #777 valida token hex no de-chunker do E2E SSE; restam #775 (`.22`) + #776 (§259). Chrome: instalar o `google-chrome-stable` destravou a prova de browser que os gates pulavam. NÃO TOCAR: fila (2) ws/TLS/sse-push no JS = decisão de escopo (regra 6, mantenedora); §252/#273, §253 face B, §192 (nat); #775/#776 (donas `.22`/§259).

> **✅ FEITO (16/09 ~19:20, dono = 192.168.100.18, lane development): varredura de gaps da plataforma ⇒ §235 (face JS) + §264 (novo, formatação Double/Float) + §265 (novo, status()/headerSet() web JS) fechados.** (1) §235 `0212cca2`: o dispatch de estáticos de wrapper faltava no JS — `Int.parseInt`/`Long.parseLong`/`Double/Float.parseDouble/parseFloat`/`Bool.parseBoolean` + o estático `Double/Float.toString` caíam no genérico e abortavam em runtime com `ReferenceError: java_lang_Integer is not defined` (R6). Roteados p/ os helpers que JÁ existiam no runtime (github #51/§81); sem helper novo p/ parse. `WrapperStaticCallsE2ETest.wrapperIsAndParseStatics` saiu de JVM-only p/ runBoth. **Face NATIVE ainda ABERTA** — `println(Int.parseInt("42"))` dá link error `undefined reference to java_lang_Integer_parseInt` (medido; lane nat, NÃO fechada — a auditoria a chamou de "JS-only", errado). (2) §264 `0212cca2` (novo): o JS imprimia Double/Float com o `Number.toString` CRU — `4` (JVM `4.0`), `10000000` (JVM `1.0E7`), `0` (JVM `-0.0`) — em TODO caminho de display (println/print/concat/`String.valueOf`/`.toString()`). O §44/§180 chamou isso de "esperado no JS" e 5 células da matriz EXCLUÍAM o js p/ ficarem verdes = false-green Q5. Conserto: fatia de runtime nova `num-fmt`/`kofNumFmt` (round-trip curto via `toExponential` + `fround` na precisão de Float, limiar `E` do JDK `exp<-3||exp>=7`, null→"null") + passthrough do tipo flutuante REAL ao `String.valueOf` só no alvo JS (espelha o do Native) em `ExpressionPrintLowerer`/`ExpressionBinaryLowerer`/`ExpressionInstanceCallLowerer`. **Oracle: kofNumFmt validado contra `Double.toString` do JVM em 500 valores fuzz + as células valor-por-valor** — idêntico. As 5 células (floatprint/doubleprint/infinityprint/negzero/doublemod) viraram paridade 4-alvos; `double-print` entrou como gate em `BackendParityTest`; docs da matriz EN+PT PARTIAL→DONE em lockstep (ConformanceMatrixDocTest é o guard). ESCOPO HONESTO: print de coleção BOXED (`List<Double>`→`[4,2.5]`) NAO é isto — é §104b-ii (lane nat, Number cru sem tag de tipo); `mapgetprim` segue no predicado p/ nao colidir. (3) §265 `22fdd679` (novo, achado ao verificar se as células "custom status/header" do ecosystem-coverage estavam STALE — a auditoria disse que sim, MENTIRA): `status(201,body)`/`headerSet()` em handler web JS eram no-ops silenciosos. Raiz decisiva = CODEGEN: `JsRuntimeOps.handleRuntimeOp` tinha 2 ramos colapsando a chamada no 2º arg (descartava o efeito — `invoke()` saía `return "made"`; a IR tinha os ops, o pump nao recebia) e SOMBRAVA o ramo correto abaixo. Raiz latente = runtime: os helpers liam `kofWebRequest.response` (campo que nunca existiu; o `response` vive no `ctx`) + o `ctx.response.status` escrevia ADIANTADO (double sendResponseHeaders) com branch de deteccao morto (testava `result.status===function`, a fn devolve String). Conserto = remover os 2 colapsos + estado DEFERIDO `_status`/`_headerQueue` aplicado pelo pump antes do `sendResponseHeaders` (idem JVM thread-local; o HttpServer do JDK exige headers pre-envio). A célula "JS ✅ 08/27" era FALSE-GREEN (compile-support ≠ efeito); virou ✅ genuíno só com E2E no FIO VIVO: `KofWebJsE2ETest.jsWebStatusAndHeaderReachTheWire` (VERMELHO pré-fix medido `200`+header ausente no servidor rodando; mesma sonda verde JVM `201 Created`; o `– WEB001` do Native permanece honesto). **MENSAGEM P/ AS LANCES: a PRÓXIMO PASSO desta lane dizia "path-params/status/header/404/500 JÁ verdes no browser" — era FALSO (status/header). Uma auditoria de sub-lane chamou as células de stale com base em `contextJsSupported` (gate de compile) SEM rodar E2E na porta — suporte de compilação NAO prova efeito de runtime (regra 6 + Q5). Corrigido aqui.** Prova: suíte completa 1913/0/0/169 (era 1908; +5 = 1 §264 + 1 §265 + 3 renomeações/células que saíram de exclusão); lib kof-ui-widgets 12 suites js 0 fail pós-toolchain fresca; docs-lang 0/0/0. ⚠️ DÍVIDA herdada p/ as lanes compiler/android: o check_500 (`ci.yml:41`) está VERMELHO no tip por `ExpressionMethodCallLowerer.java` 593→**603** (o split REFACTOR-500 da lane compiler `c9a4060f` — que CORRETAMENTE fechou os dois vermelhos que esta lane anotou, SemanticAnalyzer 719→566 e SemExpressionTyper 618→537 — + o commit android `a1a8573a` empurraram a classe p/ cima da linha vermelha; fica anotado p/ o dono partir/update-baseline, esta lane NAO toca). O vermelho MEU (ExpressionInstanceCallLowerer 608) foi FECHADO por esta lane (split StringReceiverGuards 478+153, ea89e658). ⚠️ NOVO GAP medido por esta lane (17/09): `§266` codegen JS — loop (while OU for) com if no corpo cujos statements posteriores ao if caem na cláusula update do `for(;…;…)` reconstruído mas cujos `let _scopedVar$…` ficam no bloco → `ReferenceError: _scopedVar$v is not defined` em runtime, SILENCIOSO p/ UI (kofUiRender engole em catch → render vazio sem erro). Repro headless no §266. Achado ao montar o widget ReorderList da lib (17/09); NÃO corrigido aqui (unidade grande, face do reconstrutor de CFG da lane JS/compiler — R6-honesto catalogado com prova; workaround do widget: if fora do corpo do loop). UNIDADE SEGUINTE DESTA LANE (17/09, rastreada): FECHAR o §266 — op de IR `KofContinueLabel(LabelId)` emitido SÓ pelo caso ForStmt do `StatementLowerer` (precedente `KofTryEnd`), `JsControlFlowParser.parseLoop` usa-o como fronteira corpo/update em VEZ da varredura-para-trás ambígua (linha ~293); consumidores do KofOperation recebem o case no default-passthrough (JvmOpEmitter/NativeMethodEmitter/Interpreter/Optimizer). Q0: repro while+if+trailing == 100/102/102 nos 4 alvos; view-loop com if renderiza não-vazio no Chrome (scripts/browser-reorder.mjs volta à forma `if` direta). Q1: `JsLoopIfTailE2ETest` runBoth jvm+js (while/for, head+tail do corpo, if/else aninhado, continue/break) + suíte completa obrigatória (arquivo mais denso em casos-especiais da lane JS; §147/§149/§174 tocaram ali).  NÃO TOCAR: §235-face-native (§ acima, lane nat), §104b-ii (boxed storage, lane bugfixer), §252/#273 + §192 + face B §253 (nat), §248/CRON001/ws-TLS (escopo, mantenedora), #775/#776 (.22/§259).

> **✅ FEITO (16/09 ~21:30, dono = 192.168.100.18, lane development): §193 (face crua do db.query) fechada na sua meta de diagnóstico — `5dd748cf`.** Re-medida no tip atual: desde DB001 a linha crua é String JSON e `rows.get(0).get("col")` compilava SILENCIOSO e morria em runtime `NoSuchMethodError: String.get` (JVM; JS undefined; Nativo undefined-reference — regra 6). Fix no frontend único: SEM066 rejeita acessores de coleção (`get/put/remove/size/keys/values/containsKey/entries`) com parênteses num receptor String, apontando o caminho canônico (`db.query<Record>` tipado / `json.decode<Map<String,Object>>(row)`); escopo estreito — os 0 nomes na StringMethodRegistry, métodos JDK reais via classpath externo (getBytes/isBlank/strip/repeat) e `xs.get(0)` em List/`m.get(k)` em Map intocados. Prova: `RawRowCollectionAccessE2ETest` 4/4 (VERMELHO pré-fix medido via teste-sonda: compile success + NoSuchMethodError no fio); suíte completa 1933/0/0/169; docs-lang 0/0/0. O `db.query<Record>` canônico JÁ funciona em JVM/Native; a face JS dele segue DB002 (erro alto honesto). ⚠️ DÍVIDA herdada (lane compiler, não tocada por esta lane): SemanticAnalyzer 514→706 cruzou o check_500 no tip da origem. NÃO TOCAR: §235-face-native (lane nat), §104b-ii (lane bugfixer), §252/#273 + §192 + face B §253 (nat), §248/CRON001/ws-TLS (escopo, mantenedora), #775/#776 (.22/§259), DB002/JS-typed-query (precisa bridge de dados no host — unidade nova). > **PRÓXIMO PASSO (re-dispatch lê isto, lane development):** **REGRA ABSOLUTA NOVA da mantenedora (16/09, chat): o roadmap "Em desenvolvimento" é prioridade, e a frente UI/web/db dele é desta lane.** Ordem: (1) ~~DB001 no JS~~ **FECHADO 16/09** (`3e55df51`/`eb9140cb`). (2) **WEB001 residual** — fatia de HONESTIDADE FECHADA 16/09 (`555d2afe`: context-fns web sem runtime no JS [sse/wsSend/wsMessage/stats] agora FALHAM em compile-time — fim do no-op silencioso R6 do kofWebStub; gate `KofWeb.contextJsSupported`); (2a) ~~SSE no JS~~ **FECHADO 16/09** (`7cd69a7b`: SSE handler-scoped no host GraalJS — `app.sse` + `send/event/close/isOpen` + `sse()`, framing/headers idem JVM, 3 E2Es; escopo confirmado com a mantenedora). RESIDUO REAL remanescente: IMPLEMENTAR ws/TLS no host GraalJS + SSE push pós-return/multi-cliente (pump single-thread; unidade nova, escopo/regra 6 — decidir antes de abrir; path-params/404/500 JÁ verdes no browser `JsRuntimeUiWeb`; status/header eram no-op até §265 16/09, agora ✅). (3) ~~**§253 face A — ASSUMIDA por esta lane**~~ **FECHADA 16/09 (codigo landado, bateria completa 1901/0/0/190)** — self-cancel compila+roda em jvm/js/script; NATIVE* falha SEM092 em compile-time ate a face B (lane nat, NAO TOCAR). `var id = time.interval(…, () -> time.cancel(id))` → SEM011 pois `StatementAnalyzer` tipa o inicializador ANTES de `scope.define` (:177/:202). **Provas da secao (rode `/tmp/opencode/pre`):** P2 (handle-sombra t5, `myId` capturada, `id` atribuida depois) roda js/jvm/**native rc=0**; P1 (`var id=""; id=interval(()=>cancel(id))` — o shape que a face A VIRA depois do desugar) roda js/jvm/script mas **SIGSEGV 139 no native x86** (face B real, confirmada hoje de novo). **Logo a face A nao pode so liberar o analyzer:** (a) StatementAnalyzer: pre-`define` UNKNOWN/quando `vds.initializer()` contem lambda que le `vds.name()` (helper recursivo em LambdaExpr, ~igual o `collectLambdasExpr` do scanner), atualizar tipo pos-infer via `updateLocalType`; (b) NOVO SEM092 (livre) nos alvos NATIVE* mantendo o erro em compile-time apontando §253-face-B — abrir neles converteria SEM011 em SIGSEGV (R6 proibido); fechar junto com a face B da lane nat DEPOIS; (c) lowering: `collectMutatedCapturesStmt` VarDeclStmt case (:43) precisa enxergar o self-name como capturado-mutavel (hoje `shadowed.add` antes de descer) → `driver.selfCapturedNames`; (d) `CapturedVarBox.emit`: na forma self-capture o box tem de ser **criado+guardado no slot ANTES de avaliar o inicializador** (a lambda do inicializador captura a caixa vazia; o `KofStoreField(value)` vem depois da avaliacao) — inversao de ordem em ~10 linhas, os 4 backends herdam via IR; (e) ExpressionTyper (:28-45) ja desembrulha box p/ locals — sem mudanca. Q0: repro `time.interval`/`scheduler.every` self-cancel verde em js/jvm/script com cancel real; native falha ALTO SEM092. Q1: E2E `§253` nos 3 + assercao SEM092 no native + Spinner/Toast regressao. Custo: multi-tick; comecar por (a)+(d) no mesmo commit (sem (d) vira runtime NPE no cancel — nunca commitar (a) sozinha). Plano: pre-declarar a var no escopo antes de tipar/abaixar o inicializador (face frontend compartilhada; NAO toca face B = SIGSEGV de leitura do handle capturado no native x86, lane nat). Q0: repro que hoje da SEM011 deve compilar+rodar nos 3 alvos com cancel funcional; Q1: teste `§253SelfCancelE2ETest`-like nos targets + nao-regressar `Spinner`/`ToastAutoDismiss` (handle-sombra segue valido). Face B do §253 = nat, NAO TOCAR. NÃO TOCAR: §252/#273 + §192 + face B do §253 (nat), §248 (decisão de escopo, mantenedora), UI007 (Q1–Q5 abertos no KOFUI-AUDIT §6 — decisão de API, regra 6), D-PRINT/#168 (lane .15), §205 fatia 2, docs/development/ (lane .17), §184 (FECHADO pela .22 16/09).
...
lane; registrado em `docs/bugs-and-gaps/known-bugs.md` CANVAS001. Sem item pendente na lane
...
> **⚠️ SNAPSHOT HISTÓRICO (03–13/09) — NÃO é a fila viva.** Esta tabela é a foto do
> arranjo multi-agente de 03–13/09: os donos são **nomes** (`agente-*`, `lane *`),
> sem `IP` (o modelo de identidade por IPv4 só entrou 13/09, regra 9), e a branch
> citada (`beta-0.3.0`) está **morta** — tudo hoje entra por `beta-0.4.0`.
> **A fila viva são as entradas datadas no topo deste arquivo** (com `dono = <IPv4>`).
> Reconciliação 16/09 (lane bugs-and-gaps, dono = 192.168.100.15) contra o CÓDIGO:
> **já concluídos e provados** → `EDI001` (`docs/tooling/PLAN-EDITOR-INTEGRATION.md`
> `CONCLUÍDO 13/09`, degraus 0–13 + `EditorIntegrationTest`), `REFACTOR-500`
> (`docs/architecture/PLAN-SOLID-500.md` `CLOSED 13/09`, gate `check_500.sh`),
> `STDLIB` (`docs/stdlib/PLAN-STDLIB-EXPANSION.md` `CONCLUDED 14/09`), `KOFSCRIPT`
> (`kof script`/`kof repl` implementados + `KofScriptTest`), `SYN001`/`NATIVE002`
> core/`GITHUB-P0`/`WEB-BUGS` (`FEITO`). **Ainda vivos:** `NATIVE002-stdlib`
> residual (ver `docs/development/native-multiarch.md`), `F9-CONFORMANCE`
> (`docs/bugs-and-gaps/conformance-matrix.md`), `APP-MODEL` (`D-APP` `PARTIAL` em
> `DECISIONS.md`). O conteúdo abaixo fica **preservado como histórico** (regra 8 —
> nunca descartar trabalho alheio); para o estado corrente, ler as entradas datadas.
...
> **Reconciliado 16/09 (lane bugs-and-gaps, dono = 192.168.100.15) contra o CÓDIGO/`known-bugs.md`.**
> Itens provadamente concluídos saíram da lista; códigos fantasma foram trocados pelo
> item real. A fila viva segue sendo as entradas datadas no topo do arquivo.

| Gap/Item | Prioridade | Escopo | Notas |
|---|---|---|---|
| **~~CANVAS001~~** | ~~alta~~ | ~~Canvas arc() ClassFormatError~~ | ✅ **CLOSED** (JVM 06/09; JS 06/09 `5a9cac46`, re-aprovado verde 12/09 — `UiE2ETest` 29/29 sem exclusões). Ver `docs/bugs-and-gaps/known-bugs.md` §CANVAS001. |
| **§259** — `kof.http` Native: `timeout/retry/circuit` são no-ops silenciosos | média | TLS `https` + DNS real + knobs de resiliência reais no Native | HTTP/1.1 get/post/status ✅ 03/09 (`NativeHttpRuntime.java`); delete/put/patch/options compilados; `HTTP003` é **código fantasma** (não emitido — removido das matrizes). Real residual = §259 (lane `.17`/`.18`). |
| **WEB002/WEB003/WEB004 residual** — kof.web Native avançado | média | TLS `listenSecure` (`WEB002`), sse (`WEB003`), ws (`WEB004`), path params, keepalive no `NativeWebRuntime` | server base ✅ 03/09 (accept/route/lambda/body — `KofWebNativeE2ETest` 4/4); ws = gate compile-time `WEB004` 16/09; sse = handler-scoped 16/09. Mapeamento por FUNÇÃO (não `WEB002` genérico). |
| **WEB001 residual** — kof.web JS avançado | média | ws/sse + TLS no JS | GraalJS HttpServer real ✅ 03/09 (`bc577aa`); sse ✅ handler-scoped 16/09 (`7cd69a7b`); ws = gate `WEB004`; TLS = `WEB002`. |
| **MEDIA001/2/3** | baixa | paridade media Native/JS | gap documentado |
| **SECPQ** | baixa | PQC via liboqs FFI | Tier 9 (futuro) |
| **~~MySQL query binário~~** | ~~baixo~~ | ~~`kof_db_mysql_prep_query`~~ | ✅ feito |
...
- **Congelamento de comportamento** (AGENTS.md, obrigatório): zero regressão (suíte **910** é gate de merge), features novas **aditivas** (retrocompatibilidade), refactor de 500 linhas preserva semântica (mesma suíte + golden E2E; output mudou = bug do refactor), bugs em `docs/bugs-and-gaps/known-bugs.md` são corrigidos **no código** para atingir o comportamento previsto (nunca "documentar em volta"), paridade JVM/Native/JS é regra.
...
> **⚠️ SNAPSHOT HISTÓRICO (03–07/09)** — mesmas ressalvas da tabela "Em curso agora"
> acima: dono por nome, branch `beta-0.3.0` morta. O caminho `docs/known-bugs.md`
> **não existe** desde a reorganização de 13/09 (o canônico é
> `docs/bugs-and-gaps/known-bugs.md`); a linha abaixo fica preservada como histórico.

| Frente | Estado | Dono | Branch | Arquivos | Notas |
|---|---|---|---|---|---|
| **Bug-hunt + `known-bugs.md`** | `EM CURSO` | agente-idiomatic | beta-0.3.0 | `docs/bugs-and-gaps/known-bugs.md`, `docs/status.md` | **26 bugs corrigidos com teste de regressão** (1–26 exceto nenhum; 04/09 fechou 19, 26, 16-sublist e 8-invocação). Suíte completa 913 testes verde. |
