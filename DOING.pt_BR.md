...
> **✅ FEITO (17/09 10ª rodada, dono = 192.168.0.131, lane bug-hunt externo — JVM target 0.4.2-beta): 110 bugs reais reportados (#400–#409 nesta sessão). Bugs desta rodada: SEM021 falso positivo — instância de classe concreta não atribuível à sua interface genérica implementada (#400), argumentos de tipo genérico não verificados — `List<Int>` silenciosamente atribuída a `List<String>` → ClassCastException (#401), campo de tipo-função chamado de método com retorno tipado → VerifyError bad-return-type (#402), campo chamado `log` causa descarte silencioso de todos os corpos de método → VerifyError stack-underflow (#403), `catch (e: Exception)` rejeitado — cláusula catch usa sintaxe invertida Type-nome vs. resto da linguagem (#404), construtores `Exception`/`RuntimeException` indefinidos — throw/catch inutilizável (#405), operador de verificação de tipo `is` não parseado em expressões condicionais → PARSE029 (#406), `val`/`var` no nível de módulo não suportados → PARSE007 (#407), parâmetro `Int?` compilado como `int` primitivo — argumento null gera bytecode inválido → VerifyError (#408), guarda de null `if (x == null) { return }` não estreita `T?` para `T` → SEM049 (#409). Apoiador criado em Kof e publicado em https://github.com/PublioSantos/kofbughunter; migrado para ~/apoiador. NÃO TOCAR: issues #400–#409 abertas, aguardam triagem.**

> **✅ FEITO (17/09 9ª rodada, dono = 192.168.0.131, lane bug-hunt externo — JVM target 0.4.2-beta): 100 bugs reais reportados (#390–#399 nesta sessão [10 issues]; #380–#389 na 8ª; #369–#376 + #378–#379 na 7ª; #359–#368 na 6ª; #347–#357 na 5ª; #337–#346 na 4ª; #328–#336 na 3ª; #318–#327 na 2ª; #308–#317 na 1ª; #287–#306 antes).** Bugs desta sessão: override de método genérico abstrato rejeitado quando subclasse substitui tipo concreto por T → SEM043 (#390), `Map.put()`/`Map.remove()` substituem retorno null por constante zero para valores de tipo primitivo — mesma raiz que #376 (#391), tipo de retorno de `List.set()` declarado void — SEM033 ao atribuir, elemento antigo sempre descartado via `pop` (#392), campos primitivos anuláveis `Long?`/`Bool?`/`Double?` compilados como tipos primitivos JVM — null sempre lido como zero (#393), resultado de `List.reduce()` sem `checkcast` antes de `areturn` em método com retorno tipado → VerifyError (#394), `List.reduce()` cross-type: tipo de retorno inferido como tipo do elemento em vez do acumulador → SEM021 (#395), método de classe genérica com parâmetro de tipo-função não substitui args de tipo da classe → SEM014 (#396), variável local chamada `args` em `main()` gera `aload_0` (carrega `String[]` oculto, slot 0) em vez de `aload_1` → VerifyError (#397), parâmetros primitivos anuláveis `Long?`/`Double?`/`Bool?` compilados como tipos primitivos — crash COMP002 / VerifyError (#398), método genérico retornando T emite `checkcast T` com nome literal da variável de tipo → NoClassDefFoundError em runtime (#399). **NÃO TOCAR:** Issues #390–#399 abertas, aguardam triagem. PR #302 no fork aguarda review. Nenhum arquivo de código tocado.

> **✅ FEITO (17/09 8ª rodada, dono = 192.168.0.131, lane bug-hunt externo — JVM target 0.4.2-beta): 90 bugs reais reportados (#380–#389 nesta sessão [10 issues]; #369–#376 + #378–#379 na 7ª; #359–#368 na 6ª; #347–#357 na 5ª; #337–#346 na 4ª; #328–#336 na 3ª; #318–#327 na 2ª; #308–#317 na 1ª; #287–#306 antes).** Bugs desta sessão: `Map<K,Double>.get()` chave ausente substitui `dconst_0` por null (#380), lambda retornada de método de classe capturando campo de instância gera Lambda0 sem campos — `aload_0` no corpo em vez de `getfield` → VerifyError (#381), List sem métodos: `sort()/indexOf()/lastIndexOf()/subList()/addAll()` → SEM025 (#382), `Map<K,Bool>.get()` chave ausente substitui `iconst_0` (false) por null (#383), `Map<K,Long>.get()` chave ausente substitui `lconst_0` (0L) por null (#384), descritor de método em interface genérica emite `LT;` literal em vez de apagar para `Ljava/lang/Object;` → NoSuchMethodError + sem bridge (#385), Map sem métodos: `containsValue()/getOrDefault()/putIfAbsent()` → SEM025 (#386), tipo de retorno de `String.charAt()` é primitivo Java `char` — SEM021 ao atribuir, imprime code point inteiro (#387), campo de instância com tipo-função chamado dentro de método de classe → `invokestatic` fantasma → VerifyError (#388), tipo de função com múltiplos parâmetros do tipo função `((A)->B,(C)->D)->E` analisado como função de 1 arg → SEM013 (#389). **NÃO TOCAR:** Issues #380–#389 abertas, aguardam triagem. PR #302 no fork aguarda review. Nenhum arquivo de código tocado.

> **✅ FEITO (17/09 7ª rodada, dono = 192.168.0.131, lane bug-hunt externo — JVM target 0.4.2-beta): 80 bugs reais reportados (#369–#376 + #378–#379 nesta sessão [10 issues; #377 fechado como duplicata de #375]; #359–#368 na 6ª; #347–#357 na 5ª; #337–#346 na 4ª; #328–#336 na 3ª; #318–#327 na 2ª; #308–#317 na 1ª; #287–#306 antes).** Bugs desta sessão: interpolação de string `${expr}` não avaliada — template impresso literalmente (#369), campo/parâmetro `Int?` compilado como primitivo `int` — `aconst_null` em `int` → VerifyError (#370), tipo de retorno `Int?`/`Bool?` compilado como primitivo — `return null` emite `iconst_0`/`ireturn` (#371), `String.split()` retorna `String[]` em vez de `List<String>` → SEM021 (#372), genérico aninhado `List<X<A,B>>` (tipo interno com 2+ args) → ICE COMP002 em `Type.of()` StringIndexOutOfBoundsException (#373), `List<TipoFunção>` como campo de classe ou parâmetro → SEM011 (#374), acesso a campo em parâmetro genérico limitado `T : Bound` emite `getfield "?"` → NoClassDefFoundError (#375), `Map<K,Int>.get()` chave ausente: auto-unboxing substitui null por 0 (#376), tipo de retorno `Long?` compilado como primitivo `long` — `return null` emite `lconst_0; lreturn` (#378), declaração `operator+` dentro de classe dispara recursão infinita no TypeParser → OOM/travamento (#379). **NÃO TOCAR:** Issues #369–#376/#378–#379 abertas, aguardam triagem. PR #302 no fork aguarda review. Nenhum arquivo de código tocado.

> **✅ FEITO (17/09 6ª rodada, dono = 192.168.0.131, lane bug-hunt externo — JVM target 0.4.2-beta): 70 bugs reais reportados (#359–#368 nesta sessão [10 issues]; #347–#357 na 5ª rodada; #337–#346 na 4ª; #328–#336 na 3ª; #318–#327 na 2ª; #308–#317 na 1ª; #287–#306 antes).** Kof atualizado de 0.4.0-beta para 0.4.2-beta (SHA256 verificado). Bugs desta sessão: `println(Char)` imprime code point inteiro em vez do caractere (#359), `listOf()` subtipos heterogêneos → ClassCastException (#360), `list.reduce()` com lambda tipado → ICE COMP002 (#361), métodos built-in em campo `List<T>` emitem `invokevirtual "".method` → ClassFormatError (#362), `List<T>.get()` em classe genérica emite `checkcast T` literal → NoClassDefFoundError (#363), string `"""..."""` compilada como vazia — conteúdo descartado (#364), bridge de override genérico com retorno primitivo sem boxing → VerifyError (#365), `extends Interface<T>` emite `extends` em vez de `implements` → IncompatibleClassChangeError (#366), argumento nomeado `f(param = valor)` analisado como atribuição → SEM027/SEM020 (#367), método em parâmetro genérico `T` emite `invokevirtual "".method` → ClassFormatError (#368). **NÃO TOCAR:** Issues #359–#368 abertas, aguardam triagem. PR #302 no fork aguarda review. Nenhum arquivo de código tocado.

> **✅ FEITO (16/09 5ª rodada, dono = 192.168.0.131, lane bug-hunt externo — JVM target 0.4.0-beta): 60 bugs reais reportados (#347–#357 nesta sessão [10 issues, excl. #348]; #337–#346 na 4ª; #328–#336 na 3ª; #318–#327 na 2ª; #308–#317 na 1ª; #287–#306 antes) + PR #302 no fork.** Bugs desta sessão: interface static method ACC_ABSTRACT|ACC_STATIC → ClassFormatError (#347), instance.staticMethod() → invokevirtual → IncompatibleClassChangeError (#349), 0b.../1_000 mal-analisados → statements seguintes sumem do bytecode (#350), campo genérico Int sem Integer.valueOf → VerifyError (#351), campo estático de classe com putfield em `<init>` → IncompatibleClassChangeError (#352), retorno genérico sem checkcast → VerifyError (#353), chamada de método em campo genérico emite classe "" vazia no Methodref → ClassFormatError (#354), inicializador estático não-nulo antes de super() → VerifyError (#355), Char em concatenação de String boxeado como Integer imprime código numérico (#356), método default de interface não reconhecido pelo SEM → SEM043 (#357). **NÃO TOCAR:** PR #302 aguarda review. Issues abertas, aguardam triagem.

> **✅ FEITO (16/09 4ª rodada, dono = 192.168.0.131, lane bug-hunt externo — JVM target 0.4.0-beta): 50 bugs reais reportados (#337–#346 nesta sessão [10 issues]; #328–#336 na 3ª; #318–#327 na 2ª; #308–#317 na 1ª; #287–#306 antes) + PR #302 no fork.** Bugs: `for-in` sobre Set/Map/String usa `arraylength` → VerifyError (#337), `Char == String` emite int desboxeado em Objects.equals → VerifyError (#338), extends de `final` class sem SEM error → IncompatibleClassChangeError (#339), `new Interface()` sem SEM error → InstantiationError (#340), `final abstract class` aceito sem erro (#341), método `abstract` em classe concreta → AbstractMethodError (#342), `if (cond) return` gera branch invertido (#343), método duplicado → ClassFormatError em vez de SEM error (#344), método `static` acessa campo de instância → VerifyError (#345), descriptor do catch no `LocalVariableTable` sem `java/lang/` → NoClassDefFoundError (#346). **NÃO TOCAR:** PR #302 aguarda review. Issues abertas, aguardam triagem.

> **✅ FEITO (16/09 3ª rodada, dono = 192.168.0.131, lane bug-hunt externo — JVM target 0.4.0-beta): 40 bugs reais reportados (#328–#336 nesta sessão; #318–#327 na 2ª; #308–#317 na 1ª; #287–#306 antes) + PR #302 no fork.** Bugs: catch tipo primitivo → `java/lang/Int` na exception table (#328), `enum.ordinal()` → SEM025 (#329), `var fn=42` → PARSE037 em vez de PARSE085 (#330), campo private/protected sem check → IllegalAccessError (#331), catch com classe do usuário → package errado na exception table (#332), `void f(){ return 42 }` → NoSuchMethodError (#333), `list.map` tipo errado → ClassCastException (#334), enum==String sem SEM062 (#335), `list.add(index,val)` → VerifyError (#336). **NÃO TOCAR:** PR #302 aguarda review. Issues abertas, aguardam triagem.
