...
> **PRÓXIMO PASSO (17/09 ~07:30, dono = 192.168.100.15, lane bugs-and-gaps):** varredura doc×código (claims 'planned/pending' ⇄ código real) rendeu 3 commits EN+PT: `09391aa4` (`await`/resultado de task + suíte de teste estruturada + `channel<T>()` + `application{onStart/onShutdown}` já implementados; `kof.concurrent.Queue`/`kof.test` estavam como planned), `45d54358` (source maps JS por linha implementados `KofJsSourceMapTest`; `traceId`/`spanId` W3C existem; versão `kof.ui` 0.2.6→0.4.0) e `ea58056d` (GC mark-sweep **implementado** 03/09 — `kof_gc_mark`/`kof_gc_sweep` + `kof_gc_collect_now` manual; só auto-collect sob exaustão pendente §260; bit MARKED fica reservado — marcação é no prefixo do bloco do allocator). Próximo: (1) **§262 face (b)** igualdade de conteúdo null-safe nos 4 alvos — **bloqueado p/ prova Native** (sem qemu) → repassar à lane compiler; (2) seguir a varredura `***Status:** planned` (restante = `comparison/kof-vs-java.md` l.260 'Proposal only' do `service`/DI, legítimo) e commands §264; (3) re-rodar doc×código quando outra lane landar fix (`git log <head>..origin/beta-0.4.0`).
