...
>
> **PRÓXIMO PASSO (re-dispacho lê isto, lane development):** **REGRA ABSOLUTA NOVA da mantenedora (16/09, chat): o roadmap "Em desenvolvimento" é prioridade, e a frente UI/web/db dele é desta lane.** Ordem: (1) **DB001 no JS** — `kof.db` no KofJS via delegate `kof_platform` (precedente exato: `io` — `JsRuntimeIo.java` delega readFile/writeFile/readText ao host GraalJS que roda na JVM; SQLite via JDBC no mesmo host; gate atual `KofDb.supportedOn:51` nega JS; precisa `kof_platform.db*` no host + `KofJsRunner` + supportedOn + `KofDbE2ETest` cross-js). (2) **WEB001 residual** (ws/sse/TLS no browser KofJS — path params JÁ existe no browser, `paramsMap` `JsRuntimeUiWeb:92-125`; confirmar o que sobra antes de abrir unidade). (3) §253 face A (escopo da var do inicializador em lambda — frontend; se a .22 não reivindicar até lá, assumir com Q0/Q1). NÃO TOCAR: §252/#273 + §192 + face B do §253 (nat), §248 (decisão de escopo, mantenedora), UI007 (Q1–Q5 abertos no KOFUI-AUDIT §6 — decisão de API, regra 6), D-PRINT/#168 (lane .15), §205 fatia 2, docs/development/ (lane .17), §184 (FECHADO pela .22 16/09).
