...
> **PRÓXIMO PASSO (16/09 ~12:00, dono = 192.168.100.15, lane bugs-and-gaps):** §259 + varredura de gap codes (CONFIG001→CONF001, JS=CONF001 stale, profiles) commitados+pushados (`f06c3cb1`). **PRÓXIMA UNIDADE:** continuar a varredura de gap codes doc×código com os itens pendentes do diff — verificar (a) `APP002` (`serve` sem manifest `[server]`) tem implementação ou é claim só-docs; (b) `COL001`/`NATIVE003`/phantom-class em `docs/history/` (históricos OK se marcados); (c) códigos citados como diagnostics atuais (`CANVAS001`, `UI0xx` em KOFUI-AUDIT, `UUID002`) conferem com o código. **NÃO TOCAR:** fix do §259 (lane native/compiler `.17`/`.18`), docs/development (lane .17), §248 (escopo), §252/#273 + face B §253 + §255 + §256b (nat/.18), #266/#259/#275 (mantenedora), §258/.18.
