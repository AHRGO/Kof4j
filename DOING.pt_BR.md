...
>
> **✅ FEITO (15/09, dono = 192.168.100.15, lane bugs-and-gaps): §216 (Char) — causa raiz MEDIDA + escalação de regra 6 (NÃO corrigido por decisão de contrato).**
...
> - **⚠️ REGRA 6 — NÃO corrigir sem decisão:** `println(char)` é contrato **congelado** (`training/language/strings.md:25` `// 72 (H)`; §27 do known-bugs). #168/#153 pedem o OPOSTO (mostrar o caractere) = mudança de semântica, exige ratificação da mantenedora + bump + corpus + migração. A face que É bug de paridade (`String.valueOf(char)` standalone → `"h"`) já foi corrigida no §27 (07/09). Storage de coleção DEVE manter char boxado como `Integer` (§104b-ii depende).
> - **Docs:** §216 EN+PT sincronizados com a causa raiz + o bloqueio de regra 6. Escalado à mantenedora 15/09.
>
