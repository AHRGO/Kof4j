...
> **PRÓXIMO PASSO (17/09 ~05:45, dono = 192.168.100.15, lane bugs-and-gaps):** suíte completa pós-fix §262 face (a) = **0 regressões** (só os 16 `ERROR` de JS por `node` ausente = ambiental, e o flake §252 `spawnWorkerThrowPropagatesThroughSelectAnyNative`, re-rodado VERDE). Próximo: (1) **§262 face (b)** — `nullableRecord == nullableRecord` NPE; precisa de igualdade de conteúdo null-safe cross-target (guarda de null no receptor ou `Objects.equals`); (2) sync doc×código dos lands novos (`#323` SEM014 ctor-arg, `#325` record `implements`, §264/§265 JS) contra `backend-parity`/`known-bugs`/`ecosystem-coverage`. Não tocar: SemanticAnalyzer (dívida check_500 da lane compiler, `42dd3ee7`), §259/§260 (lane native), DECISIONS.md (lane .17).
