...
> **✅ FEITO (16/09 ~15:20, dono = 192.168.100.15, lane bugs-and-gaps): `training/` — contagem de suíte stale (EN+PT, 10 arquivos).** Os 5 pares `training/` (README, language/overview, reference/compiler, reference/targets, tooling/cli) diziam `**Version:** 0.4.0-beta (Sep 2026) — 810 tests` — ou seja, **versão corrente com contagem de 02/09**. O nº autoritativo medido é **2214** (1907 compiler + 38 script + 7 kof-c + 262 cli; `AGENTS.md:1091`/`docs/development/README.md:6`/`docs/status.md:87`). Corrigido o número (e o breakdown em `overview`). **Escopo consciente:** `CONTRIBUTING.md`/`.pt_BR` **não** foram tocados — são um snapshot datado de 02/09 (`Kof 0.2.6-beta — 02 Sep 2026 — 810 tests`), auto-consistente; `CHANGELOG`/`docs/history/`/`docs/audits/`/`docs/development/future/DECOMPILER` idem (histórico/auditoria datada, o número é o do momento). Guard `docs-lang.sh check` 0/0/0.
> **PRÓXIMO PASSO (16/09 ~15:25, dono = 192.168.100.15, lane bugs-and-gaps):** continuar a auditoria doc×código nos **docs canônicos** (foco da mantenedora: "todos os .md revisados contra o estado atual do código"). **PRÓXIMA UNIDADE:** varredura de **SHAs mortos** citados em `docs/` (o método: `git cat-file -e <sha>` em cada hash de 7–40 hex citado; os inválidos são ponteiros quebrados) + ponteiros `file:line` para linhas inexistentes. **NÃO TOCAR:** fix do §259/§255/§256b/§258 (lane `.17`/`.18`), docs/development/ (lane .17), §248 (escopo), §252/#273 + face B §253 (nat/.18), #266/#259/#275 (mantenedora).
