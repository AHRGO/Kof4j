...
> **PRÓXIMO PASSO (17/09 ~09:40, dono = 192.168.100.15, lane bugs-and-gaps):** re-varredura doc×código dos lands novos: `79163241` sync `kof.observability`→`OBS002` (`histogram`/`metrics()`/`spanStart`/`spanEnd` estavam na tabela? não — adicionados; `KofObservabilityTest` 7/7; §5 próximos passos reduzidos a rota `/metrics` + export OTel) + `KOFUI-AUDIT` recontagem (UI002 FEITO, UI001 única face silent-no-op, `isUiType`=36); `68713423` sync do land `7c1305ab` (#331/#327): SG-013 estendido a CAMPOS — `classes.md` dizia 'field access is NOT checked' (falso agora), `type-system.md` ganhou `SEM063`, `specification-status` = métodos+campos; prova `FieldAccessControlTest` 7/7. i18n `8edfe970` (switch SEM031/032/035→EN) = **sem drift de docs** (as citações PT batem com o código ainda PT; a fila D-DIAG-EN sincroniza doc por pacote). Próximo: (1) seguir a fila **D-DIAG-EN** (docs learn/training que citam mensagens PT — sincronizar quando o pacote landar); (2) re-varredura quando outra lane landar (`git log <head>..origin/beta-0.4.0`); (3) §262 face (b) repassada à lane compiler. NÃO tocar `docs/status.md` (lane `.22`).
