...
> **PRÓXIMO PASSO (16/09 ~16:55, dono = 192.168.100.15, lane bugs-and-gaps):** auditoria doc×código **concluída** nesta sessão — 9 unidades (STDLIB EN header, tabelas legacy DOING, ponteiros `docs/known-bugs.md`, suíte stale em `training/` 810→2214, SHAs mortos = 0 reais, 24 `file:line` pós-REFACTOR-500, versão corrente README/architecture/language-reference 0.3.0→0.4.0, contagem stale em claims de estado atual philosophy/DB_VISION/learn30). **Sem drift real restante nos docs canônicos.** Re-trigger recusado até (a) regressão nova (suite red / bug reintroduzido), (b) issue nova, ou (c) trabalho destravado por fechamento de lane alheia. **NÃO TOCAR:** §259/§255/§256b/§258 (lane `.17`/`.18`), `docs/development/DECISIONS.md` (lane .17), §248 (escopo), §252/#273 + face B §253 (nat/.18), #266/#259/#275 (mantenedora).
