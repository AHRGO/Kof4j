...
> **PRÓXIMO PASSO (17/09 ~09:10, dono = 192.168.100.15, lane bugs-and-gaps):** re-varredura doc×código pós-lands (`8edfe970` i18n switch + `2ddd7ab4`) = **sem drift de i18n** (docs que citam mensagens PT ainda batem com o código ainda PT; a fila D-DIAG-EN sincroniza doc por pacote traduzido). ACHEI e CORRIGI drift real de `kof.observability` (`79163241`): a tabela de API omitia `histogram`/`metrics()`/`spanStart`/`spanEnd` e o §5 listava export Prometheus + spans como 'próximos passos' — mas estão implementados (`OBS002` fechado 16/09; `KofObservabilityTest` **7/7**, era citado 3/3). Sync EN+PT em `observability.md`, `ecosystem-coverage`, `stdlib.md`, `security.md`; `KOFUI-AUDIT` header: recontagem 17/09 fixa UI002 FEITO e UI001 como única face silent-no-op (`isUiType` = 36 tipos). Próximo: (1) seguir a fila **D-DIAG-EN** — quando o pacote i18n landar, sincronizar os docs (learn/training) que citam o texto PT correspondente (varredura pronta: só `known-bugs.md:5859` cita SEM033 PT, ainda correto); (2) re-varredura doc×código no próximo land (`git log <head>..origin/beta-0.4.0`); (3) §262 face (b) já repassada à lane compiler. NÃO tocar `docs/status.md` (lane `.22`).
