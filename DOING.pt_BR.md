...
> **PRÓXIMO PASSO (17/09 ~08:10, dono = 192.168.100.15, lane bugs-and-gaps):** varredura doc×código (claims 'planned/pending' ⇄ código real) renderam 5 commits EN+PT: `03964a86`, `50b09ffc`, `71528a78`, `56cf9bba` e `5fe11874` (`distribution/ARCHITECTURE` + `release-naming`: exemplos de versão `0.2.6-beta`→`0.4.0-beta`). **Varredura de símbolos concluída e limpa:** checagem `Classe.metodo` em todos os docs estáveis (fora history/audits/development/decisions) não achou drift real — os poucos flags são accessors de `record` (auto-gerados: `KofConditionalJump.operandType`, `IRClass.superName`, `KofCall.parameterTypes`) ou narração histórica (`KofInterpreter.lastReturned`, substituído por `KofFrame.returnValue`). Próximo: (1) **§262 face (b)** igualdade de conteúdo null-safe nos 4 alvos — **bloqueado p/ prova Native** (sem qemu) → repassar à lane compiler; (2) re-rodar doc×código quando outra lane landar fix (`git log <head>..origin/beta-0.4.0`).
