...
1. **Zero regressão.** Nenhum commit pode fazer um teste existente passar a
   falhar. A suíte completa (`mvn test`, hoje **2215** nos 4 módulos — ver
...
> é **2215 testes** (compiler 1908 + script 38 + kof-c 7 + cli 262, medição
> 16/09 ~15:09 — cresce com cada commit): **0 regressões / 0 erros** (a única falha que a
> suíte já mostrou é o flake intermitente do §252 nativo — calado de novo nesta corrida, último disparo 09:44)
> (node agora presente
...
> (`4408eb6`) + os outros guards de toolchain/BD externo + o guard de sysroot do §255 (`06e77e94`) → `2215/0-1/192-skip` (o flake §252 dispara em ~1/2 das corridas completas: disparou às 09:44, calou às 11:38 e 15:09)
> (MEDIDO 16/09 ~15:09, clone limpo). Com qemu, **tudo executa** — os 84 cross rodam
> verdes e o total fica `2215` com a contagem de skip caindo para o
> resíduo externo de BD/ambiente `node`. Estado correto HOJE (16/09 ~15:09, clone limpo de `97425541`):
> **0 regressões / 0 erros** (2215 = 1908+38+7+262, 192 skip) — a corrida completa das 09:44 teve o flake INTERMITENTE
> conhecido do §252 nativo (`spawnWorkerThrowPropagatesThroughSelectAnyNative`, dona lane
> nativa `.18`/nat; às 11:38 e 15:09 ele ficou calado — frequência ~1/2, ver §252), que
