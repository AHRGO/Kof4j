...
>
> **PRÓXIMO PASSO (re-dispatch lê isto, lane development):** fila PLAN-STDLIB — `ulid`/`isUlid` (P1, RFC-like: Crockford base32 26 chars + 48-bit timestamp) sem algoritmo ratificado no corpus: preparar briefing p/ mantenedora ANTES de implementar (regra 6 — formato/variante é decisão de contrato). Alternativa sem decisão: auditar `docs/bugs-and-gaps/known-bugs.md` p/ item OPEN da lane development reproduzível. NÃO TOCAR: D-ENUM207 fatia 2 (lane .15), D-PRINT/#168, §192/§205 fatia 2 (nat), N1→N4 (compiler).
