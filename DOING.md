...
> **PRÓXIMO PASSO (re-dispacho le isto, lane CodeQL):** fila CodeQL **zerada** (8 restantes = fix no HEAD, dependem so do proximo scan). Ao re-trigger: (1) re-contar via `gh api .../code-scanning/alerts?state=open` — se o scan rodou e baixou p/ ~0, confirmar e registrar; se aparecerem alerts NOVOS, atacar (metodo: bucket por regra → ler cada instancia → fix na raiz c/ teste no MESMO commit OU dismiss c/ evidencia <=280 chars). (2) **NAO inventar trabalho CodeQL** — condicao de estabilidade da lane atingida; watchers de issue (#185 coord lanca, sem acao pendente minha) e filas de codigo das outras lanes NAO sao desta lane. (3) Se um scan novo trouxer regra nunca vista, comecar por `git log -5 -- <arq>` p/ nao colidir c/ lanes quentes (.22 MethodCallTyper/enum, .17 stdlib, .18 development).
