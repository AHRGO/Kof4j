...
> **✅ FEITO (16/09 2ª rodada, dono = 192.168.0.131, lane bug-hunt externo — JVM target 0.4.0-beta): 30 bugs reais reportados como issues no upstream KofLang/Kof4j (#318–#327 nesta sessão; #308–#317 na 1ª rodada; #287–#306 em sessões anteriores) + PR #302 aberto no fork PublioSantos/Kof4j.** Bugs desta sessão cobrem: `this()` constructor delegation → SEM015 (#318), tipos java.lang (Integer/Math/etc.) não reconhecidos → SEM011 (#319), `Char` armazenado como `int` → `println('A')` imprime `65` (#320), `interface extends ConcreteClass` sem erro → IncompatibleClassChangeError (#321), SEM043 falso-positivo em `abstract class` parcialmente implementando interface (#322), mismatch no tipo de argumento do construtor não detectado → VerifyError (#323), mensagens SEM em português misturadas com toString() Java (#324), `record X(...) implements Interface` → PARSE007 em cascata (#325), override com return type incompatível aceito silenciosamente — dispatch errado em runtime (#326), atribuição a campo `final` fora da classe sem erro → IllegalAccessError (#327). **NÃO TOCAR:** PR #302 aguarda review da mantenedora. Issues #318–#327 abertas, aguardam triagem. Nenhum arquivo de código tocado. PRÓXIMO PASSO desta lane: triagem/follow-up ou 3ª rodada de bug-hunt (nullable primitives, JS/Native targets).
