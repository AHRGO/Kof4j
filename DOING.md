...
> **PRÓXIMO PASSO (re-dispacho lê isto, lane bugs-and-gaps):** re-medir o portão 4-módulos no tip fresco (pós `9efba38d` §243 + `7137d978` §244 + meu §245) com jar NOVO; esperado = só §205. Se confirmado, a fila desta lane = regra 6 / lane alheia / novo vermelho; re-varrer issues abertas (#259/#266/#261/#267) p/ reconhecer os fixes da lane compiler e fechar as que caíram, senão **recusar re-trigger** (estabilidade).
...
> **PORTÃO DA RELEASE (RE-MEDIDO 15/09 ~07:30 no tip `80edc90a`, jar fresco 4 módulos):** o revert `6553ac2e` (§241) **limpou os 14 vermelhos de `c0cf805e`** — agora kof-compiler **1769 run / 2 fail**, kof-script 38/0, kof-c 7/0, kof-cli 252/0 (err de `node` = 0–1 conforme o host; v22 presente aqui). Os 2 fails são AMBOS pré-existentes, **confirmados idênticos no tip limpo `d776e9c1` sem meus arquivos** (worktree dedicado): `ConformanceMatrixTest.conformanceCoreControl` = §205 (célula `ifexpr-heterogeneous-direct` NATIVE exit=139, lane #183) + `ConformanceMatrixDocTest.matrixDocMatchesTestExclusions` (mesma causa — a matriz lista `ifthrowelse`/`nullnarrow` etc. que o teste exclui por §205). **§240 ✅ fechado (`5e996312`); §241-como-regressão ✅ fechado pelo revert; o 2º vermelho do MEU gate (tip `bad95c5e`) NÃO era o `ConformanceMatrixDocTest` e sim `KofConcurrency2Test.stopFlagFieldWriteObservedBySpinReader` — teste mal dimensionado, catalogado+corrigido §245; §243 ✅ (`9efba38d`) e §244 ✅ (`7137d978`) fechados pela lane compiler.** O que resta no portão é só §205. Re-rodar com jar novo antes de declarar.
> **NÃO TOCAR:** #148/#233 família Jvm*Descriptor (lane .17); `DecompileTest.wideParamsMapToCorrectSlots` (lane decompiler); §205 (lane #183); **§241 = código da lane compiler (catálogo é meu, fix não — §240 foi corrigido por mim `5e996312` porque bloqueava o gate e o próprio registro pedia a separação)**; **§243 ✅ FECHADO upstream pela lane compiler (`9efba38d`, #261) — o guard §179 foi aplicado aos pins; §244 ✅ FECHADO upstream (`7137d978`, #267) — o ramo de concat agora cobre dois operandos apagados**; **§241-front-boxed (face parâmetro #266) = decisão de contrato que reabre §125 (regra 6)**; #213/#207/#159/#160/#151 = regra 6/decisão/lane alheia (re-confirmado hoje). Método: worktree NOVO por unidade + `git fetch`/`gh issue view`/repro-no-jar-do-TIP ANTES de tocar. Se não houver novidade na fila nem regressão verde: **recusar re-trigger** (estabilidade desta lane).
>
