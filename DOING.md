...
> **EM CURSO (15/09 ~20:00, dono = 192.168.100.15, lane bugs-and-gaps): D-ENUM207 — #207 (identidade de enum) ATRIBUÍDA a esta lane pela mantenedora (`DECISIONS.md` D-ENUM207, `3629a13b`).** A mantenedora encerrou o agente anterior; a ratificação da mudança semântica (`Dir.N == "N"` hoje `true` → `false`/erro) está na própria D-ENUM207 (regra 6 satisfeita). **Recon medida (o que o próximo passo precisa):** hoje `Dir.N` vira `ldc "N"` (String), **nenhum `Dir.class` é emitido** (`CompilerPipeline.lowerToIR` não tem branch p/ `EnumDeclarationNode` — cai no `default` em `:242`), `JvmTypeMapper:70-73/166` apaga o enum p/ `Ljava/lang/String;`, `==` vira `kof_string_equals` (`ExpressionBinaryLowerer:309-320`), `getClass()` retorna `java.lang.String`, `instanceof Dir` → `instanceof java/lang/String`. O corpus documenta "enum value IS the name (String)" em 5 arquivos (`classes.md:139`, `grammar.md:117`, `semantics.md:66`, `syntax.md:84`, `type-system.md:255`) — **todos exigem bump de corpus na mesma unidade** (D-PRINT/D-ENUM207). **Sítios (JVM vertical):** `CompilerClassLowering` (novo `lowerEnum`) + `CompilerPipeline:204-244` (branch) + `AccessFlags` (`ACC_ENUM`), `ExpressionLowerer:46-57/424-445` (constante → `KofGetStatic`), `ExpressionStaticCallLowerer:346-379` (`values()`/`valueOf()`), `ExpressionBuiltinInstanceCalls:21-59` (`name`/`toString`/`ordinal`/`compareTo` — hoje identidade/lista-String), `JvmTypeMapper:70-73/166` (parar de apagar p/ String), `CompilerComparisons:32-33` + `ExpressionBinaryLowerer:309-320` (`==` identidade). **Paridade (freeze 5):** o interpretador roda o MESMO IR (`KofInterpreter.java:245` `KofGetStatic` + `ensureInit`) → o Script herda a mudança; Native/JS precisam de paridade OU gap diagnosticado `XXX00x` (R6) — **não half-landing (lição §241)**. **Plano de fatias:** (1) JVM: enum como classe real (`java/lang/Enum`, `static final` fields, `<clinit>` com `getstatic`, `name`/`ordinal`/`toString`/`valueOf`/`values` reais) + `getstatic`/`==`/`instanceof`/`getClass()`; teste novo `EnumIdentityE2ETest` RED antes com o repro da issue. (2) Script (herda IR) medir. (3) Native/JS paridade ou gap. (4) corpus + §211 + DECISIONS + fechar #207. **NÃO TOCAR:** `docs/development/DECISIONS.md` já tem a D-ENUM207 (registro, sem edição). **FATIA 1 ✅ FEITA 15/09 (esta lane):** `enum == String` (qualquer ordem) agora é ERRO DE TIPO `SEM062` no frontend COMPARTILHADO (`TypeChecker.inferBinaryResultType` + `isEnumRef`, usando `BuiltinTypes.isEnumType` já registrado em `analyzeAndLower`); mesmo diagnóstico nos 4 alvos (freeze 5). `Dir.N == "N"` (antes `true`) rejeitado; enum×enum e enum×enum-nulável seguem compilando. Prova `EnumIdentityE2ETest` 4/4 (VERMELHO 2/4 pré-fix). Corpus EN+PT atualizado (`classes.md`/`semantics.md`/`syntax.md`/`expressions.md`/`type-system.md`) no mesmo commit. Sítios que FALTAM (fatia 2): `CompilerClassLowering.lowerEnum` + `CompilerPipeline` branch + `AccessFlags.ACC_ENUM`, `ExpressionLowerer` getstatic, `JvmTypeMapper` parar de apagar, `ExpressionBinaryLowerer` identidade — classe enum real com getstatic.
