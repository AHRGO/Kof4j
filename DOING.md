...
> **🔴 ALERTA DE GATE (15/09 ~00:30, dono = 192.168.100.15, lane bugs-and-gaps): REGRESSÃO de 39 fails na suíte — `8935c8a7` (#237/#231, lane .22 compiler). Catalogada §240 (EN+PT).**
> - **Prova:** suíte 4-módulos verde em `8719e304`/`9115baed`; **39 fails** em `kof-compiler` a partir de `8935c8a7` (reproduzido em checkout LIMPO de `origin/beta-0.4.0` `432cb552` com `mvn clean compile`, SEM commits meus): `SemanticResolutionTest` 2 (`'"abc".indexOf('c')'` → SEM025 em vez de SEM051), `QualifiedCatchE2ETest` 2 (`throw RuntimeException` → SEM026), `KofDbE2ETest` 10 / `KofOrmE2ETest` 14 (`Cannot resolve method 'execute'/'query'/'close' on type 'String'`), `KofStringParseTest` 6, `KofWebE2ETest` 1, `KofJsE2ETest` 1, `NativeE2ETest` 1, `CoreRegressionE2ETest` 1, `ConformanceMatrixTest` 1.
> - **Causa raiz:** `ExternalClasspath.knows()` ganhou `|| JdkReflectionResolver.isJdkClass(internalName)` → todo tipo `java/*` passa a ser "externo conhecido"; `MemberCallTyper`/`SemanticAnalyzer.isExternal` tomam o caminho de reflexão do JDK para `String`/exceções, contornando o registro de builtins (mudança de CONTRATO, regra 6). `knows()` deve seguir escopado às entries reais; a reflexão de JDK pertence só ao call-site `resolveMethodWithArgs`.
> - **Ação:** NÃO corrigido por mim (frente de outra lane + regra 6). **Bloqueia o portão de release.** Dono do `8935c8a7` = lane `.22` (compiler). Meu #161 (`eaab71ce`) está verde sobre essa base (só §205 + 13 err de node além).
>
...
> **PORTÃO DA RELEASE (verificar no próximo tick):** §240 (39 reds, regressão `8935c8a7`, lane .22) e §241 (14 reds + `Int?`-return VerifyError, regressão `c0cf805e`, lane compiler) — rodar num tip fresco com jar novo: `mvn -o -pl kof-compiler -am test -Dtest='KofStringParseTest,KofDbE2ETest,KofOrmE2ETest,KofWebE2ETest,KofJsE2ETest,QualifiedCatchE2ETest,KofInterpreterParityTest,KofMapSetTest,JvmE2ETest,JsonE2ETest,CoreRegressionE2ETest,ConformanceMatrixTest,BackendParityTest,CodegenKitchenSinkTest,SemanticResolutionTest'` — se as lanes reveram/terminaram os commits, **fechar §240/§241 com prova** e só re-abrir a issue-dona se o gap restar.
> **NÃO TOCAR:** #148/#233 família Jvm*Descriptor (lane .17); `DecompileTest.wideParamsMapToCorrectSlots` (lane decompiler); §205 (lane #183); §240/§241 = código da lane .22/compiler (catálogo é meu, fix não); #213/#207/#159/#160/#151 = regra 6/decisão/lane alheia (re-confirmado hoje). Método: worktree NOVO por unidade + `git fetch`/`gh issue view`/repro-no-jar-do-TIP ANTES de tocar. Se não houver novidade na fila nem regressão verde: **recusar re-trigger** (estabilidade desta lane).
>
