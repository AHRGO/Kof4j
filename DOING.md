...
>
> **PRÓXIMO PASSO (re-dispacho lê isto, lane bugs-and-gaps):** **#153/#168 (Char) = causa raiz medida, duas faces distintas (§216):** (1) `Char.toString()` → `"A"` é **bug de paridade real** (a mantenedora na triagem da #153: "`println(Char)` numérico é comportamento documentado; `Char.toString()` não"); sítio `ExpressionInstanceCallLowerer:411`; **a mantenedora adiou a correção para a 0.4.1** (patch de estabilização) — não furar a fila na `beta-0.4.0`. (2) `println(char)`/concat = contrato **CONGELADO** (`training/language/strings.md:25` `// 72 (H)`; §27) — mudança de semântica, regra 6, exige ratificação. Causa raiz em ambos: `TypeEmitter.boxPrimitive` boxa char como `Integer` + força `valueOf(I)`. Seguir para **#213 (interface default method dropado)/#219 (abstract method)/#220/#221**, respeitando: #219 já tem fix da lane .22 (`79ab6e0e`). **NÃO TOCAR:** #148/#233 família Jvm*Descriptor (lane .17); `DecompileTest.wideParamsMapToCorrectSlots` (lane decompiler); §205 (lane #183); **§240 (regressão 39-fails de `8935c8a7`, lane .22)**. Método: worktree NOVO por unidade, jar SEMPRE `clean package` após mexer no compiler, teste em arquivo DEDICADO. **Família interop-descriptor (§224/§225/§234) — ler a assinatura REAL do callee (`ACC_VARARGS` + retorno real) conserta parâmetro E retorno de uma vez; #216 já provou o caminho.**
>
