...
> **✅ FEITO (16/09, dono = 192.168.0.131, lane bug-hunt externo — JVM target 0.4.0-beta): 20 bugs reais reportados como issues no upstream KofLang/Kof4j (#308–#317 nesta sessão; #287–#306 em sessões anteriores) + PR #302 aberto no fork PublioSantos/Kof4j corrigindo o bug "class extending interface emits interface as super_class" (raiz: `lowerClass()` em `CompilerClassLowering.java` não verifica `isInterfaceType()` antes de colocar o nome em `super_class`; fix redireciona para `ifaces[]`).** Metodologia: repro mínimo → `kof build --target jvm` → runtime → `javap -c` (bytecode evidence) → issue publicada em EN com seções Environment/Description/Reproducer/Bytecode Evidence/Root Cause/Suggested Fix. Bugs desta sessão cobrem: SEM043 falso-positivo em `default` method (#308), class nomeada igual a tipo JVM (#309), lambda sem inferência de tipo SAM (#310), abstract method sem override sem erro em compile-time (#311), `super()` com construtor implícito → SEM017 falso-positivo (#312), `throw new ExceptionType()` → nome não qualificado no bytecode (#313), chamadas intra-classe sempre `invokevirtual` (#314), cast C-style `(Int) x` sem instrução de conversão (#315), return type errado em método stdlib (#316), campo de interface inacessível SEM011/SEM025 (#317). **NÃO TOCAR:** PR #302 aguarda review da mantenedora — não re-abrir, não fechar. Issues #308–#317 abertas, aguardam triagem. Nenhum arquivo de código tocado nesta sessão (só issues + PR em sessão anterior). PRÓXIMO PASSO desta lane: triagem/follow-up das issues abertas ou nova rodada de bug-hunt em outras áreas (nullable primitives, JS/Native targets).
