...
> **PORTÃO DA RELEASE (medido 15/09 ~02:55 no tip `5e996312`, jar fresco):** §240 ✅ **FECHADO** (fix `5e996312`, lane bugs-and-gaps — `KofBuiltinJdkSeparationE2ETest` 4/4; os 39 reds do `8935c8a7` sumiram). **Resta só §241** (regressão `c0cf805e`, lane compiler, SEM `IN PROGRESS`): o gate-set acima roda **15 fails** = §241 (14: `KofInterpreterParityTest`×4, `KofMapSetTest`, `JvmE2ETest`, `JsonE2ETest`, `CoreRegressionE2ETest`×2, `ConformanceMatrixTest`×3, `BackendParityTest`, `CodegenKitchenSinkTest`) + §205 (`conformanceCoreControl`, Native ifexpr, lane #183); + 2 err de `node` (ambiental). Nenhum do §240. Se a lane compiler landar §241, **fechar com prova**; senão, o catálogo (EN+PT) é a fonte. Re-rodar com jar novo antes de declarar.
