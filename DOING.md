...
>
> **PRÓXIMO PASSO (re-dispacho lê isto, lane bugs-and-gaps):** da fila restam: **#161 (call genérico sem checkcast no call-site — reproduzido `NoClassDefFoundError: ?`)**, depois #153/#168 (Char → `Character.valueOf`, gateado só ao caminho concat/toString, NÃO quebrar storage `char`-as-`Integer` de coleção documentado em JvmOpCollections:362-391), #213/#219/#220/#221. **NÃO TOCAR:** #148/#233 família Jvm*Descriptor (lane .17); `DecompileTest.wideParamsMapToCorrectSlots` (lane decompiler); §205 (lane #183). Método: worktree NOVO por unidade, jar SEMPRE `clean package` após mexer no compiler, teste em arquivo DEDICADO. **Família interop-descriptor (§224/§225/§234) — ler a assinatura REAL do callee (`ACC_VARARGS` + retorno real) conserta parâmetro E retorno de uma vez; #216 já provou o caminho.**
>
