...
>
> **PRÓXIMO PASSO (re-dispatch lê isto, lane development):** candidato natural = §184 (JS `kofArraySet` sem máscara em `Byte[]`/`Short[]`/`Char[]` — fix proposto (a): lowering passa tag de elemento `"b"/"s"/"c"` e `kofArraySet`/`kofArrayGet` aplicam a máscara; espelho do `kofD2I` de §181). Confirmar no re-dispatch que a lane JS não a reivindicou. Alternativas: `creditCardBrand`/`last4` (PLAN-STDLIB, mas Luhn já existe — brand table = trademark, avaliar), ou aguardar resposta D-STDLIB-ULID. NÃO TOCAR: D-PRINT/#168 (lane .15), §205 fatia 2/§192 (nat), N1→N4 (compiler), #266/#259 (mantenedora).
>
...
> **✅ FEITO (17/09 12ª rodada + PR, dono = 192.168.0.131, lane bug-hunt externo — JVM target 0.4.2-beta): 130 bugs reais reportados (#420–#427 nesta sessão) + PR #428 com correção sugerida. Bugs desta rodada: modificadores `open`/`override` não parseados → PARSE010 (#420), construtor primário Kof-style `class Foo(p: T) {}` não parseado → PARSE015 (#421), `List.any/all/none/count/sorted` ausentes + parâmetro implícito `it` não suportado → SEM025 (#422), crash do compilador em `filter { it }` — MemberCallTyper muta lista imutável → COMP002 (#423), `StringBuilder` indefinida → SEM011 (#424, também cobre `Math`), declaração `object` não parseada → PARSE011 (#425), `Char` impresso como código numérico → WRONGOUTPUT (#426), declaração de propriedade em interface (`val name: String`) rejeitada → PARSE016 (#427). PR #428: fix para #423 (compact constructor copia `arguments` para `ArrayList`); fix verificado localmente — crash COMP002 eliminado; PR aberto em KofLang/Kof4j contra beta-0.4.0. Novo comando `kofbug pr` automatiza build+verify+push+open-PR para próximos fixes. NÃO TOCAR: issues #420–#427 abertas, PR #428 aguarda review da Mel.**

> **✅ FEITO (17/09 11ª rodada, dono = 192.168.0.131, lane bug-hunt externo — JVM target 0.4.2-beta): 120 bugs reais reportados (#410–#419 nesta sessão). Bugs desta rodada: operador de range `0..n` não parseado → PARSE039 (#410), operador safe-call `?.` + tipo nullable `T?` em parâmetro de construtor → PARSE044 (#411), implementação múltipla de interfaces `class Foo: A, B` não parseada → PARSE044 (#412), lambda/closure não implementa interface Function → IncompatibleClassChangeError em runtime (#413), operador Elvis `?:` não parseado → PARSE041 (#414), subscript de String `s[0]` rejeitado → SEM054 (#415), operador `!!` não estreita `Int?` para `Int` → SEM021 (#416), `mutableListOf()` não definida como built-in → SEM015 (#417), construtor `Pair()` não definido → SEM015 (#418), `for ((k, v) in map)` destrutural não parseado → PARSE043 (#419). Pipeline de caça a bugs publicado em https://github.com/PublioSantos/kofbughunter (commit c9c3ba4). NÃO TOCAR: issues #410–#419 abertas, aguardam triagem.**

> **✅ FEITO (17/09 10ª rodada, dono = 192.168.0.131, lane bug-hunt externo — JVM target 0.4.2-beta): 110 bugs reais reportados (#400–#409 nesta sessão). Bugs desta rodada: SEM021 false positive — concrete class not assignable to implemented generic interface (#400), generic type args not enforced — `List<Int>` silently assigned to `List<String>` → ClassCastException (#401), function-type field called from typed-return method → VerifyError bad-return-type (#402), field named `log` causes all method bodies to be silently dropped → VerifyError stack-underflow (#403), `catch (e: Exception)` rejected — catch clause uses reversed Type-name syntax vs. rest of language (#404), `Exception`/`RuntimeException` constructors undefined — throw/catch unusable (#405), `is` type-check operator not parsed in conditional expressions → PARSE029 (#406), top-level `val`/`var` not supported → PARSE007 (#407), `Int?` parameter compiled as primitive `int` — null argument generates invalid bytecode → VerifyError (#408), null guard `if (x == null) { return }` does not narrow `T?` to `T` → SEM049 (#409). Apoiador criado em Kof e publicado em https://github.com/PublioSantos/kofbughunter; migrado para ~/apoiador. NÃO TOCAR: issues #400–#409 abertas, aguardam triagem.**

> **✅ FEITO (17/09 9ª rodada, dono = 192.168.0.131, lane bug-hunt externo — JVM target 0.4.2-beta): 100 bugs reais reportados como issues no upstream KofLang/Kof4j (#390–#399 nesta sessão [10 issues]; #380–#389 na 8ª; #369–#376 + #378–#379 na 7ª; #359–#368 na 6ª; #347–#357 na 5ª; #337–#346 na 4ª; #328–#336 na 3ª; #318–#327 na 2ª; #308–#317 na 1ª; #287–#306 em sessões anteriores).** Bugs desta sessão: abstract generic method override rejected when subclass substitutes concrete type for T → SEM043 (#390), `Map.put()`/`Map.remove()` replace null return with zero constant for primitive-typed values — same root as #376 (#391), `List.set()` return type declared void — SEM033 on assignment, old element always discarded via `pop` (#392), nullable primitive fields `Long?`/`Bool?`/`Double?` compiled as primitive JVM types — null always reads as zero value (#393), `List.reduce()` result missing `checkcast` before `areturn` in typed-return method → VerifyError (#394), `List.reduce()` cross-type accumulator: return type inferred as element type instead of accumulator type → SEM021 (#395), generic class method with function-type parameter does not substitute class type args → SEM014 (#396), local variable named `args` in `main()` generates `aload_0` (loads hidden `String[]` param, slot 0) instead of `aload_1` → VerifyError (#397), nullable primitive parameters `Long?`/`Double?`/`Bool?` compiled as primitive types — COMP002 crash / VerifyError (#398), generic method returning T emits `checkcast T` with literal type-var name → NoClassDefFoundError at runtime (#399). **NÃO TOCAR:** Issues #390–#399 abertas, aguardam triagem. PR #302 no fork aguarda review. Nenhum arquivo de código tocado.

> **✅ FEITO (17/09 8ª rodada, dono = 192.168.0.131, lane bug-hunt externo — JVM target 0.4.2-beta): 90 bugs reais reportados como issues no upstream KofLang/Kof4j (#380–#389 nesta sessão [10 issues]; #369–#376 + #378–#379 na 7ª; #359–#368 na 6ª; #347–#357 na 5ª; #337–#346 na 4ª; #328–#336 na 3ª; #318–#327 na 2ª; #308–#317 na 1ª; #287–#306 em sessões anteriores).** Bugs desta sessão: `Map<K,Double>.get()` absent key substitutes `dconst_0` for null (#380), lambda returned from class method capturing instance field generates Lambda0 with no fields — `aload_0` in lambda body instead of `getfield` → VerifyError (#381), List missing methods: `sort()/indexOf()/lastIndexOf()/subList()/addAll()` → SEM025 (#382), `Map<K,Bool>.get()` absent key substitutes `iconst_0` (false) for null (#383), `Map<K,Long>.get()` absent key substitutes `lconst_0` (0L) for null (#384), generic interface method descriptor emits `LT;` literal type-var instead of erasing to `Ljava/lang/Object;` → NoSuchMethodError at runtime + no bridge method (#385), Map missing methods: `containsValue()/getOrDefault()/putIfAbsent()` → SEM025 (#386), `String.charAt()` return type is Java `char` primitive — SEM021 on assignment, prints integer code point inline (#387), function-type instance field called inside class method → phantom `invokestatic Default/Main.fieldName` → VerifyError (#388), multi-parameter function type with function-type params `((A)->B,(C)->D)->E` misparses as 1-arg function → SEM013 (#389). **NÃO TOCAR:** Issues #380–#389 abertas, aguardam triagem. PR #302 no fork aguarda review. Nenhum arquivo de código tocado.

> **✅ FEITO (17/09 7ª rodada, dono = 192.168.0.131, lane bug-hunt externo — JVM target 0.4.2-beta): 80 bugs reais reportados como issues no upstream KofLang/Kof4j (#369–#376 + #378–#379 nesta sessão [10 issues; #377 fechado como duplicata de #375]; #359–#368 na 6ª rodada; #347–#357 na 5ª rodada; #337–#346 na 4ª rodada; #328–#336 na 3ª rodada; #318–#327 na 2ª rodada; #308–#317 na 1ª rodada; #287–#306 em sessões anteriores).** Bugs desta sessão: string interpolation `${expr}` not evaluated — template printed literally (#369), `Int?` nullable field/constructor param compiled as primitive `int` — `aconst_null` on `int` → VerifyError (#370), `Int?`/`Bool?` function return type compiled as primitive — `return null` emits `iconst_0`/`ireturn` (#371), `String.split()` returns `String[]` not `List<String>` → SEM021 (#372), nested generic `List<X<A,B>>` (inner type 2+ args) → COMP002 ICE in `Type.of()` StringIndexOutOfBoundsException (#373), `List<FunctionType>` as class field or function parameter type → SEM011 (#374), bounded type parameter field access `item.field` (T : Bound) emits `getfield "?"` → NoClassDefFoundError (#375), `Map<K,Int>.get()` absent key auto-unboxing substitutes 0 for null (#376), `Long?` return type compiled as primitive `long` — `return null` emits `lconst_0; lreturn` (#378), `operator+` method declaration inside class triggers infinite recursion in TypeParser → OOM/hang (#379). **NÃO TOCAR:** Issues #369–#376/#378–#379 abertas, aguardam triagem. PR #302 no fork aguarda review. Nenhum arquivo de código tocado.

> **✅ FEITO (17/09 6ª rodada, dono = 192.168.0.131, lane bug-hunt externo — JVM target 0.4.2-beta): 70 bugs reais reportados como issues no upstream KofLang/Kof4j (#359–#368 nesta sessão [10 issues]; #347–#357 na 5ª rodada; #337–#346 na 4ª rodada; #328–#336 na 3ª rodada; #318–#327 na 2ª rodada; #308–#317 na 1ª rodada; #287–#306 em sessões anteriores).** Kof atualizado de 0.4.0-beta para 0.4.2-beta (SHA256 verificado). Bugs desta sessão: `println(Char)` prints integer code point instead of character — Char stored as JVM `int` (#359), `listOf(1, "a", true)` heterogeneous subtypes → ClassCastException (#360), `list.reduce()` with typed lambda → COMP002 ICE (#361), built-in methods (`size()`/`isEmpty()`/`contains()`) called on `List<T>` field emit `invokevirtual "".method` → ClassFormatError (#362), generic class method `List<T>.get()` result emits `checkcast T` (type variable literal) → NoClassDefFoundError (#363), triple-quoted string `"""..."""` compiles to empty string — content silently discarded (#364), bridge method for generic override with primitive return type (`Int`) missing boxing → VerifyError (#365), class implementing generic interface emits `extends` instead of `implements` → IncompatibleClassChangeError (#366), named-argument call syntax `f(param = value)` not recognized — parsed as assignment → SEM027/SEM020 (#367), method call on generic function parameter `T` emits `invokevirtual "".method` → ClassFormatError (#368). **NÃO TOCAR:** Issues #359–#368 abertas, aguardam triagem. PR #302 no fork aguarda review. Nenhum arquivo de código tocado.

> **✅ FEITO (16/09 5ª rodada, dono = 192.168.0.131, lane bug-hunt externo — JVM target 0.4.0-beta): 60 bugs reais reportados como issues no upstream KofLang/Kof4j (#347–#357 nesta sessão [10 issues excl. #348 não criado]; #337–#346 na 4ª rodada; #328–#336 na 3ª rodada; #318–#327 na 2ª rodada; #308–#317 na 1ª rodada; #287–#306 em sessões anteriores) + PR #302 aberto no fork PublioSantos/Kof4j.** Bugs desta sessão: interface static method ACC_ABSTRACT|ACC_STATIC → ClassFormatError (#347), instance.staticMethod() → invokevirtual → IncompatibleClassChangeError (#349), 0b.../1_000 silently mis-parsed → subsequent statements dropped from bytecode (#350), generic field assignment with Int missing Integer.valueOf boxing → VerifyError (#351), static class-type field initializer in `<init>` with putfield instead of `<clinit>` putstatic → IncompatibleClassChangeError (#352), generic method return type missing checkcast → VerifyError (#353), method call on generic field emits empty class name "" in Methodref → ClassFormatError (#354), non-null static field initializer emitted before super() → VerifyError (#355), Char string concat boxes as Integer prints code point not character (#356), interface default method not recognized by SEM type checker → SEM043 (#357). **NÃO TOCAR:** PR #302 aguarda review. Issues #347–#357 abertas, aguardam triagem. Nenhum arquivo de código tocado.

> **✅ FEITO (16/09 4ª rodada, dono = 192.168.0.131, lane bug-hunt externo — JVM target 0.4.0-beta): 50 bugs reais reportados como issues no upstream KofLang/Kof4j (#337–#346 nesta sessão [10 issues]; #328–#336 na 3ª rodada; #318–#327 na 2ª rodada; #308–#317 na 1ª rodada; #287–#306 em sessões anteriores) + PR #302 aberto no fork PublioSantos/Kof4j.** Bugs desta sessão: `for-in` sobre Set/Map/String usa `arraylength` → VerifyError (#337), `Char == String` emite int desboxeado em Objects.equals → VerifyError (#338), extends de classe `final` sem SEM error → IncompatibleClassChangeError (#339), `new Interface()` sem SEM error → InstantiationError (#340), `final abstract class` aceito silenciosamente (#341), método `abstract` em classe concreta → AbstractMethodError (#342), `if (cond) return` gera branch invertido no bytecode (#343), método duplicado na mesma classe → ClassFormatError em vez de SEM error (#344), método `static` acessa campo de instância → VerifyError (`aload_0` em contexto estático) (#345), descriptor do `LocalVariableTable` do catch sem prefixo `java/lang/` → NoClassDefFoundError (#346). **NÃO TOCAR:** PR #302 aguarda review. Issues #328–#346 abertas, aguardam triagem. Nenhum arquivo de código tocado.

> **✅ FEITO (16/09 3ª rodada, dono = 192.168.0.131, lane bug-hunt externo — JVM target 0.4.0-beta): 40 bugs reais reportados como issues no upstream KofLang/Kof4j (#328–#336 nesta sessão [9 issues]; #318–#327 na 2ª rodada; #308–#317 na 1ª rodada; #287–#306 em sessões anteriores) + PR #302 aberto no fork PublioSantos/Kof4j.** Bugs desta sessão: catch com tipo primitivo (Int/Bool) → `java/lang/Int` na exception table → NoClassDefFoundError (#328), `enum.ordinal()` não implementado → SEM025 apesar de estar na spec (#329), `var fn = 42` (keyword reservada como variável) → PARSE037 em vez de PARSE085 (#330), campo private/protected lido/escrito de fora da classe sem SEM error → IllegalAccessError (#331), catch com classe do usuário → `java/lang/Foo` em vez de `Default/Foo` na exception table (#332), `void f() { return 42 }` compila com tipo int, call site usa `()V` → NoSuchMethodError (#333), `list.map` com lambda de tipo diferente → checkcast errado → ClassCastException (#334), enum comparado com String via `==` sem SEM062 — retorna true (#335), `list.add(index, value)` gera bytecode errado com single-arg add → VerifyError (#336). **NÃO TOCAR:** PR #302 aguarda review. Issues #328–#336 abertas, aguardam triagem. Nenhum arquivo de código tocado.
