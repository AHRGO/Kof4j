...
>
> **PRÓXIMO PASSO seguinte (lane compiler):** §242 residual (`MemberResolver.checkAbstractClassImplementation` casa override por NOME+ARIDADE, não assinatura → `Int run(Int)` satisfaz `abstract Int run(String)`; usar `TopLevelOverload.sigTag`, +teste `wrongSignatureDoesNotSatisfy`). Depois §243/#261 (classe do usuário com nome de builtin — aplicar a regra ratificada: user shadowing wins; o builtin `List`/`String`/`Set`/`Map` só vale se o módulo não declarar o nome).
