...
> **EM CURSO (15/09, dono = 192.168.100.15, lane bugs-and-gaps): D-ENUM207 — #207 (identidade de enum). ATRIBUÍDA a esta lane pela mantenedora (`DECISIONS.md` D-ENUM207, `3629a13b`). A mudança semântica é ratificada (regra 6 satisfeita).** **FATIA 1 ✅ (`58e2e320`):** `enum == String` (qualquer ordem) = ERRO DE TIPO `SEM062` no frontend COMPARTILHADO (`TypeChecker.inferBinaryResultType` + `isEnumRef`) — mesmo diagnóstico nos 4 alvos; `Dir.N == Dir.M`/enum-nulável seguem compilando. **FATIA 2 ✅ (esta unidade):** enum agora é CLASSE REAL (`CompilerEnumLowering.lowerEnum`): constantes `static final LDir;` no `<clinit>` (`KofNewObject`+`KofPutStatic`, o caminho genérico de classe dos 4 alvos — sem ABI novo, sem half-landing §241); branch `EnumDeclarationNode` em `CompilerPipeline.lowerToIR`; `AccessFlags.ENUM (0x4000)`; `JvmTypeMapper` para de apagar enum→String; `ExpressionLowerer` `Dir.N`/`N` → `KofGetStatic Dir.N:LDir;`; `==`/`!=` e `switch` de enum por IDENTIDADE (`if_acmp`); `name`/`ordinal`/`toString`/`compareTo` métodos reais + `values()`/`valueOf()` estáticos (List<Dir>, null no miss); `println` imprime o nome via `toString()` (D-PRINT); `CollectionWrites.isStringLike` (§150) parou de marcar enum como String (membership nativa por ponteiro — corrige o SIGSEGV). **Prova:** `EnumIdentityE2ETest` 6/6 (`getClass()`→`class Dir`, `javap` mostra `getstatic Dir.N:LDir;` e nenhum `ldc // String N`, `Dir.class` emitida); `KofEnumTest` 6/6 (4 alvos), `KofMapSetTest` 14/14, `KofEnumSwitchTest` 4/4, `CoreRegressionE2ETest` 102/102, `BackendParityTest` 19/19, `JvmE2ETest` 35/35, `KofInterpreterParityTest` 25/25, `KofJsE2ETest` 40/40; suíte do compilador **1877/0/16-node/186 skip**; corpus EN+PT atualizado. **PRÓXIMO PASSO (re-dispatch lê isto):** (1) fechar a issue **#207** no GitHub com a prova (comentário + close); (2) rodar o gate dos 4 módulos (`kof-script,kof-c-compiler,kof-cli`) antes do push final; (3) provar Script (herda IR — deve passar) e as 3 arquiteturas nativas; (4) reconciliar §216 face 2 / #168 (D-PRINT destravou: `println(Char)` deve imprimir `A`, não `65`) como próximo item da lane. **NÃO TOCAR:** `docs/development/DECISIONS.md` (registro da D-ENUM207; lane .17).
