...
>
> **PRÓXIMO PASSO (re-dispatch lê isto, lane development):** briefing ulid/isUlid **REGISTRADO** (`DECISIONS.md` D-STDLIB-ULID, 4 perguntas com recomendação — aguardando a mantenedora; NÃO implementar antes). Alternativa enquanto isso: auditar `docs/bugs-and-gaps/known-bugs.md` p/ item OPEN da lane development reproduzível (§248 default-methods JS/Native = decisão de escopo pedida pela lane compiler .22; §184/§185 = faces JS/Script de array-store — sem dono declarado, avaliar antes de atacar). WIP vivo no tree: CONC001/B48 = lane nat (files 21:02-21:04, dono ativo — NÃO tocar). NÃO TOCAR: D-ENUM207 fatia 2 (lane .15), D-PRINT/#168, §192/§205 fatia 2 (nat), N1→N4 (compiler).
