...
>
> **PRÓXIMO PASSO seguinte (lane compiler):** §243/#261 (classe do usuário com nome de builtin — aplicar a regra ratificada em DECISIONS §4/§179: **user shadowing wins**; os pins de `CompilerTypes.toType`/`exceptionType` e os paralelos em `ExpressionLowerer:143`/`ExpressionTyper:150-154`/`SemExpressionTyper:294-298` precisam do guard `!unitDeclaresType && sa.getClass == null`). Teste `UserClassShadowsBuiltinE2ETest` (`class List`/`class String` campo write/read JVM).
...
> **PRÓXIMO PASSO (re-dispacho lê isto, lane bugs-and-gaps):** fila desta lane = regra 6 / lane alheia / portão vermelho.
...
> **PORTÃO DA RELEASE (RE-MEDIDO 15/09 ~07:30 no tip `80edc90a`, jar fresco 4 módulos):** o revert `6553ac2e` (§241) **limpou os 14 vermelhos de `c0cf805e`** — agora kof-compiler **1769 run / 2 fail**, kof-script 38/0, kof-c 7/0, kof-cli 252/0 (err de `node` = 0–1 conforme o host; v22 presente aqui). Os 2 fails são AMBOS pré-existentes, **confirmados idênticos no tip limpo `d776e9c1` sem meus arquivos** (worktree dedicado): `ConformanceMatrixTest.conformanceCoreControl` = §205 (célula `ifexpr-heterogeneous-direct` NATIVE exit=139, lane #183) + `ConformanceMatrixDocTest.matrixDocMatchesTestExclusions` (mesma causa — a matriz lista `ifthrowelse`/`nullnarrow` etc. que o teste exclui por §205). **§240 ✅ fechado (`5e996312`); §241-como-regressão ✅ fechado pelo revert; o que resta no portão é só §205.** Re-rodar com jar novo antes de declarar.
> **NÃO TOCAR:** #148/#233 família Jvm*Descriptor (lane .17); `DecompileTest.wideParamsMapToCorrectSlots` (lane decompiler); §205 (lane #183); **§241 = código da lane compiler (catálogo é meu, fix não — §240 foi corrigido por mim `5e996312` porque bloqueava o gate e o próprio registro pedia a separação)**; **§243 = resolução de nomes (BUG contra contrato já ratificado em DECISIONS §4 — fix multi-site nos arquivos quentes da lane compiler; sinalizado ao dono, não editado por mim)**; **§244 = lowering de operador (lane compiler; sinalizado, não editado)**; **§241-front-boxed (face parâmetro #266) = decisão de contrato que reabre §125 (regra 6)**; #213/#207/#159/#160/#151 = regra 6/decisão/lane alheia (re-confirmado hoje). Método: worktree NOVO por unidade + `git fetch`/`gh issue view`/repro-no-jar-do-TIP ANTES de tocar. Se não houver novidade na fila nem regressão verde: **recusar re-trigger** (estabilidade desta lane).
>
