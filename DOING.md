...
>
> **PRÓXIMO PASSO (re-dispacho lê isto, lane bugs-and-gaps):** da fila restam: **#153/#168 (Char) = BLOQUEADO na regra 6** — `println(char)` é contrato CONGELADO (`training/language/strings.md:25` `// 72 (H)`; §27); o pedido das issues (mostrar o caractere) é mudança de semântica, exige decisão da mantenedora; causa raiz já MEDIDA e registrada no §216 (`TypeEmitter.boxPrimitive` boxa char como `Integer` + força `valueOf(I)`; sítios `ExpressionPrintLowerer:63`, `ExpressionBinaryLowerer:253/261`, `ExpressionInstanceCallLowerer:411`) — escalado 15/09. Seguir para **#213 (interface default method dropado)/#219 (abstract method)/#220/#221**, respeitando: #219 já tem fix da lane .22 (`79ab6e0e`). **NÃO TOCAR:** #148/#233 família Jvm*Descriptor (lane .17); `DecompileTest.wideParamsMapToCorrectSlots` (lane decompiler); §205 (lane #183); **§240 (regressão 39-fails de `8935c8a7`, lane .22)**. Método: worktree NOVO por unidade, jar SEMPRE `clean package` após mexer no compiler, teste em arquivo DEDICADO. **Família interop-descriptor (§224/§225/§234) — ler a assinatura REAL do callee (`ACC_VARARGS` + retorno real) conserta parâmetro E retorno de uma vez; #216 já provou o caminho.**
>
