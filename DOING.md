...
> - **Catalogado §236 (EN+PT):** o backend JS não tem lowering de `String.format` — `JsEmitter` monta `JsMember` com alvo null (`jsClassName` devolve null p/ classe JDK) → `COMP002 unknown JS expression: null`; **PRÉ-EXISTENTE** (idêntico em `aa78eba0` e `59359935`), lane JS. Teste intencionalmente **só JVM** (não é verde-falso).
> - **Suíte 4-módulos (worktree em `aa78eba0`):** run=1996 fail=2 err=13 skip=167 — **2 fail = §205 (`ConformanceMatrixTest.conformanceCoreControl`, Native ifexpr, lane #183) + `DecompileTest.wideParamsMapToCorrectSlots` (kof-cli, pré-existente, lane decompiler); 13 err = node ausente (ambiental)**. Gate `check_500` OK.
>
